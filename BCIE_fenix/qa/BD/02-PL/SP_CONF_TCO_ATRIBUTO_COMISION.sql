-- ##--------------------------------------------------------------------------------------------##
-- Descripción: Script que permite agregar, modificar, desactivar/activar la configuración de los atributos de la  Comisión, así mismo, permite listar la información de dicha tabla.
/*
...
CREATE OR REPLACE PROCEDURE SP_CONF_TCO_ATRIBUTO_COMISION (
	...
	P_COLUMNA              IN VARCHAR2 := NULL,			 -- Nombre de la columna, de la tabla de Comisión (COMISION)
    ...
    P_ID_TCA_COMISION  	   IN NUMBER   := NULL,         -- Id. del catálogo de Comisión (TCA_COMISION)
	...
	P_ID_TCA_TIPO_COMISION IN NUMBER   := NULL,			-- Id. del registro de comisión. (TCA_ COMISION) 
...
BEGIN
	...
	THEN
		...
		IF  (P_ID_TCA_COMISION > 0) THEN
			...
			END IF;			            
		ELSE
			v_Error_msg := 'No se ha indicado el Id. del registro del catálogo de Comisión (TCA_COMISION).';
		END IF;
