...
declare variable $outInvokeConsultarProductoByIdOperacion.response as element() (:: schema-element(pro:ConsultarProductoByIdOperacionResponse) ::) external;
declare variable $outInvokeConsultarRolPorProceso.response as element() (:: schema-element(con:responseConsultarRolesPorProcesoMessage) ::) external;

declare function local:funcXq_configuracionprepago_response($outInvokeConsultarProductoByIdOperacion.response as element() (:: schema-element(pro:ConsultarProductoByIdOperacionResponse) ::), 
                                                            $outInvokeConsultarRolPorProceso.response as element() (:: schema-element(con:responseConsultarRolesPorProcesoMessage) ::)) 
                                                            as element() (:: schema-element(con1:ConfiguracionPrepagoResponse) ::) {
    <con1:ConfiguracionPrepagoResponse>
        <con1:configuracionPrepago>
            <con2:Header>
                {
                    if (fn:true()) then 
                        <ns1:Operacion>
                            <ns2:CodigoOperacion>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:idOperacion)}</ns2:CodigoOperacion>
                            <ns2:NombreOperacion>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:nombre)}</ns2:NombreOperacion>
                            {
                                if ($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:responsable)
                                then <ns2:ResponsableOperacion>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:responsable)}</ns2:ResponsableOperacion>
                                else ()
                            }
                            <ns2:CodigoCliente>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:cliente/cli:idCliente)}</ns2:CodigoCliente>
                            <ns2:NombreCliente>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:cliente/cli:razonSocial)}</ns2:NombreCliente>
                            <ns2:CodigoProducto>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Producto/pro1:idProducto)}</ns2:CodigoProducto>
                            <ns2:NombreProducto>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Producto/pro1:descripcion)}</ns2:NombreProducto>
                            <ns2:MontoSolicitado></ns2:MontoSolicitado>
                            <ns2:Pais>{fn:data($outInvokeConsultarProductoByIdOperacion.response/pro:Operacion/ope:cliente/cli:pais/cat:Descripcion)}</ns2:Pais>
                        </ns1:Operacion>
                    else
                        <ns1:Cliente>
            ...
            <con2:RolesEquipoTrabajo>
                {
                    for $listadoRoles in $outInvokeConsultarRolPorProceso.response/con:ListadoRoles/con3:listadoRoles
                    return 
                    <con2:Rol>
                        {
...
local:funcXq_configuracionprepago_response($outInvokeConsultarProductoByIdOperacion.response, $outInvokeConsultarRolPorProceso.response)
