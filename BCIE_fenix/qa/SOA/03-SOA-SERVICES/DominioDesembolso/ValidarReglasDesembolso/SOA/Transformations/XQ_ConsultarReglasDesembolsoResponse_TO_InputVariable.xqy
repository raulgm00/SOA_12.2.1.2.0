...
declare function local:funcXq_consultarreglasdesembolsoresponse_to_inputvariable($inputVariable.request as element() (:: schema-element(des:ValidarReglasDesembolsoRequest) ::), 
    ...
    <des:ValidarReglasDesembolsoRequest>
        ...
        <des:TareaReglas>
            ...
            }
            {
                ...
                <reg:ReglasEvaluacion>
                    ...
                    <reg1:Estado>
                          ...
                          <reg1:Estatus>{fn:data($ReglasEvaluacion/reg1:Estatus)}</reg1:Estatus>
            ...
            {
                for $ReglasDesembolso in $outConsultarReglasDesembolso.response/des:ReglasEvaluacion
                return 
                <reg:ReglasEvaluacion>
                    <reg1:Id>{fn:data($ReglasDesembolso/reg1:Id)}</reg1:Id>
                    {
                        if ($ReglasDesembolso/reg1:Descripcion)
                        then <reg1:Descripcion>{fn:data($ReglasDesembolso/reg1:Descripcion)}</reg1:Descripcion>
                        else ()
                    }
                    {
                        if ($ReglasDesembolso/reg1:Transaccion)
                        then <reg1:Transaccion>{fn:data($ReglasDesembolso/reg1:Transaccion)}</reg1:Transaccion>
                        else ()
                    }
                    {
                        if ($ReglasDesembolso/reg1:Tipo)
                        then 
                            <reg1:Tipo>
                                {
                                    if ($ReglasDesembolso/reg1:Tipo/cat:Id)
                                    then <cat:Id>{fn:data($ReglasDesembolso/reg1:Tipo/cat:Id)}</cat:Id>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Tipo/cat:Descripcion)
                                    then <cat:Descripcion>{fn:data($ReglasDesembolso/reg1:Tipo/cat:Descripcion)}</cat:Descripcion>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Tipo/cat:DescripcionCorta)
                                    then <cat:DescripcionCorta>{fn:data($ReglasDesembolso/reg1:Tipo/cat:DescripcionCorta)}</cat:DescripcionCorta>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Tipo/cat:estatus)
                                    then <cat:estatus>{fn:data($ReglasDesembolso/reg1:Tipo/cat:estatus)}</cat:estatus>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Tipo/cat:codigoExterno)
                                    then <cat:codigoExterno>{fn:data($ReglasDesembolso/reg1:Tipo/cat:codigoExterno)}</cat:codigoExterno>
                                    else ()
                                }
                            </reg1:Tipo>
                        else ()
                    ...
                    {
                        if ($ReglasDesembolso/reg1:PosibleExceptuar)
                        then <reg1:PosibleExceptuar>{fn:data($ReglasDesembolso/reg1:PosibleExceptuar)}</reg1:PosibleExceptuar>
                        else ()
                    }
                    {
                        if ($ReglasDesembolso/reg1:Exceptuado)
                        then 
                            <reg1:Exceptuado>
                                {
                                    if ($ReglasDesembolso/reg1:Exceptuado/cat:Id)
                                    then <cat:Id>{fn:data($ReglasDesembolso/reg1:Exceptuado/cat:Id)}</cat:Id>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Exceptuado/cat:Descripcion)
                                    then <cat:Descripcion>{fn:data($ReglasDesembolso/reg1:Exceptuado/cat:Descripcion)}</cat:Descripcion>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Exceptuado/cat:DescripcionCorta)
                                    then <cat:DescripcionCorta>{fn:data($ReglasDesembolso/reg1:Exceptuado/cat:DescripcionCorta)}</cat:DescripcionCorta>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Exceptuado/cat:estatus)
                                    then <cat:estatus>{fn:data($ReglasDesembolso/reg1:Exceptuado/cat:estatus)}</cat:estatus>
                                    else ()
                                }
                                {
                                    if ($ReglasDesembolso/reg1:Exceptuado/cat:codigoExterno)
                                    then <cat:codigoExterno>{fn:data($ReglasDesembolso/reg1:Exceptuado/cat:codigoExterno)}</cat:codigoExterno>
                                    else ()
                                }
                            </reg1:Exceptuado>
                        else ()
                    ...
                    {
                        if ($ReglasDesembolso/reg1:UsuarioExceptua)
                        then <reg1:UsuarioExceptua>{fn:data($ReglasDesembolso/reg1:UsuarioExceptua)}</reg1:UsuarioExceptua>
                        else ()
                    }
                    {
                        if ($ReglasDesembolso/reg1:FechaExcepcion)
                        then <reg1:FechaExceptua>{fn:data($ReglasDesembolso/reg1:FechaExcepcion)}</reg1:FechaExceptua>
                        else ()
                    }
                  <reg1:Estado>
                          <cat:Id>{fn:data($ReglasDesembolso/reg1:Estado/cat:Id)}</cat:Id>
                          <cat:Descripcion>{fn:data($ReglasDesembolso/reg1:Estado/cat:Descripcion)}</cat:Descripcion>
                          <cat:DescripcionCorta>{fn:data($ReglasDesembolso/reg1:Estado/cat:DescripcionCorta)}</cat:DescripcionCorta>
                          <cat:estatus>{fn:data($ReglasDesembolso/reg1:Estado/cat:estatus)}</cat:estatus>
                          <cat:codigoExterno>{fn:data($ReglasDesembolso/reg1:Estado/cat:codigoExterno)}</cat:codigoExterno>
                    </reg1:Estado>
                    {
                        if ($ReglasDesembolso/reg1:Estatus)
                        then <reg1:Estatus>{fn:data($ReglasDesembolso/reg1:Estatus)}</reg1:Estatus>
                        else ()
                    }
                    <reg:EsAdvertencia>{fn:data($ReglasDesembolso/reg:EsAdvertencia)}</reg:EsAdvertencia>
                    <reg:EsError>{fn:data($ReglasDesembolso/reg:EsError)}</reg:EsError>
                    <reg:Mensaje>
                        <cat:Id>{fn:data($ReglasDesembolso/reg:Mensaje/cat:Id)}</cat:Id>
                        <cat:Descripcion>{fn:data($ReglasDesembolso/reg:Mensaje/cat:Descripcion)}</cat:Descripcion>
                        <cat:DescripcionCorta>{fn:data($ReglasDesembolso/reg:Mensaje/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        <cat:estatus>{fn:data($ReglasDesembolso/reg:Mensaje/cat:estatus)}</cat:estatus>
                        <cat:codigoExterno>{fn:data($ReglasDesembolso/reg:Mensaje/cat:codigoExterno)}</cat:codigoExterno>
                    </reg:Mensaje>
                </reg:ReglasEvaluacion>
            }
        </des:TareaReglas>
...
local:funcXq_consultarreglasdesembolsoresponse_to_inputvariable($inputVariable.request, $consultarReglasDesembolsoAux.response, $outConsultarReglasDesembolso.response)
