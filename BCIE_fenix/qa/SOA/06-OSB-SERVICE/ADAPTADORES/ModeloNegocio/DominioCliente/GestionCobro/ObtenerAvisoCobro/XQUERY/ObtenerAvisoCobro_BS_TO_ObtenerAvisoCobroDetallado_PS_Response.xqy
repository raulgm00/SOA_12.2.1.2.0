    ...
    <ns3:ObtenerAvisoCobroResponse>
        ...
        <ns3:AvisoCobro>
            ...
            <ges:aviso>
                        ...
                        <ges:detallePago>
                             ...
                             <lin:Fondo>{fn:data($fondo)}</lin:Fondo>
                         {
                          ...
                          return
                            ...
                            <ges:ContratoDesembolso>
                                <des:referencia>{fn:data($monto/IDENTIFICADOR)}</des:referencia>
                                    {
                                    for $valor in $monto/*
                                    return
                                    if(fn:name($valor) != "IDENTIFICADOR")then
                                    <des:monto>
                                        <com:tipo>
                                            <cat:Descripcion>{fn:name($valor)}</cat:Descripcion>
                                        </com:tipo>
                                        <com:importe>{fn:data($valor)}</com:importe>
                                    </des:monto>
                                    else()
                                    }
                            </ges:ContratoDesembolso>
                            ...
                            <ges:tablasDetalle>
                            
                             {
                                    ...
                                    for $desembolso in $contratoDetalle[CODIGO_FONDO=$fondo]
                                    where $fecha = $desembolso/LISTA_INTERESES/INTERES/FECHA_VENCIMIENTO
                                    return
                                    
                                      for $interes in $desembolso/LISTA_INTERESES/INTERES
                                      return    
                                        <ges:detalleIntereses>
                                          <des:referencia>{fn:data($desembolso/DESEMBOLSO)}</des:referencia>
                                          <des:monto>
                                                <com:tipo>
                                                    <cat:Descripcion>SALDO</cat:Descripcion>
                                                </com:tipo>
                                                <com:importe>{fn:data($interes/SALDO)}</com:importe> 
                                         </des:monto>
                                          <des:monto>
                                                <com:tipo>
                                                    <cat:Descripcion>MONTO_INTERES</cat:Descripcion>
                                                </com:tipo>
                                                <com:importe>{fn:data($interes/MONTO_INTERES)}</com:importe> 
                                         </des:monto>
                                      <des:condicionesFinancieras>
                                          <des:tasa>
                                              <des:fija>
                                                    <des:valor>{fn:data($interes/TASA)}</des:valor>
                                               </des:fija>
                                          </des:tasa>
                                          <des:baseCalculo>
                                              <cat:DescripcionCorta>{fn:data($interes/BASE)}</cat:DescripcionCorta>
                                          </des:baseCalculo>
                                      </des:condicionesFinancieras>
                                      <ges:periodo>
                                          <com:inicio>{fn:data($interes/FECHA_DESDE)}</com:inicio>
                                          <com:fin>{fn:data($interes/FECHA_HASTA)}</com:fin>
                                          <com:dias>{fn:data($interes/DIAS)}</com:dias>
                                      </ges:periodo>
                                      <ges:pagosAplicados>
                                          <com:importe>{fn:data($interes/LISTA_INTERESES/TOTAL_INTERES_PAGO)}</com:importe>
                                      </ges:pagosAplicados>
                                  </ges:detalleIntereses>
                                }
                             ...
                             <ges:tablasDetalle>
                             {
                                    ...
                                    return
                                       ...
                                       for $desembolso in $contratoDetalle[CODIGO_FONDO=$fondo]
                                        for $fechaDesde in distinct-values($desembolso/LISTA_INTERESES/INTERES/FECHA_DESDE)
                                          return
                                            let $intereses := if($fecha = $desembolso/LISTA_INTERESES/INTERES/FECHA_VENCIMIENTO
                                                             and $fechaDesde = $desembolso/LISTA_INTERESES/INTERES/FECHA_DESDE) 
                                                then $desembolso/LISTA_INTERESES/INTERES[FECHA_DESDE = $fechaDesde]
                                                else()          
                                            return
                                      <ges:detalleCC>
                                        ...
                                        <ges:periodo>
                                         ...
                                         <ges:pagosAplicados>
                                          <com:importe>{fn:data($desembolso/LISTA_INTERESES/TOTAL_INTERES_PAGO)}</com:importe>
                                      </ges:pagosAplicados>
                           ...
                           <ges:tablasDetalle>
                            ...
                            {
                            for $detalleNota in $aviso/LISTA_DETALLES/DETALLE[@TIPO_DETALLE = "SALDO_NO_VENCIDO"]
                              let $contratoConsolidado := if ($detalleNota/MONEDA = $moneda and $detalleNota/LINEA= $linea)
                              then $detalleNota/LISTA_CONTRATOS/CONTRATO
                              else()
                              return
                            
                               if ($contratoConsolidado[CODIGO_FONDO=$fondo and FECHA_VENCIMIENTO = $fecha]) then
                                 if (fn:data($AvisosCliente[1]/REVISION_TASA = 'S'))then
                                  <ges:comentario2>{fn:data($detalleNota/LISTA_NOTAS[@CLASE = 'NOTA_DETALLE_HEADER']/NOTA/TEXTO)}</ges:comentario2>
                                 else
                                 
                                  <ges:comentario2></ges:comentario2>
                               else
                               ()
                             }
                ...
                if($ObtenerAvisoCobroRequest/ns3:tipoAviso = 'D')then
					
				
                if ($aviso/LISTA_DETALLES/DETALLE[@TIPO_DETALLE = "DETALLE_CALCULO"]) then

                for $moneda in distinct-values($aviso/LISTA_DETALLES/DETALLE[@TIPO_DETALLE = "DETALLE_CALCULO"]/MONEDA)
                return
                <ges:avisoOperacion>
                    <ges:Moneda>
                        <cat:Descripcion>{fn:data($moneda)}</cat:Descripcion>
                    ...
                    {
                    for $fecha in distinct-values($aviso/LISTA_DETALLES/DETALLE[MONEDA=$moneda]/LISTA_CONTRATOS/CONTRATO/LISTA_INTERESES/INTERES/FECHA_VENCIMIENTO)
                    return
                    <ges:informacionPago>
                        <ges:fechaPago>{fn:data($fecha)}</ges:fechaPago>
                        ...
                        {
                        for $detalle in $aviso/LISTA_DETALLES/DETALLE[@TIPO_DETALLE = "DETALLE_CALCULO"]
                        let $linea := if ($detalle/MONEDA = $moneda and $detalle/LISTA_CONTRATOS/CONTRATO/LISTA_INTERESES/INTERES/FECHA_VENCIMIENTO = $fecha)
                        then $detalle/LINEA
                        else()
                        return
                        
                        for $fondo in distinct-values($detalle/LISTA_CONTRATOS/CONTRATO/CODIGO_FONDO)
                        where $detalle/LINEA = $linea 
                        return
                        ...
                        <ges:detallePago>
                            ...
                            <ges:tablasDetalle>
                             ...
                             <ges:tablasDetalle>
                             {
                                    ...
                                    return
                                       ...
                                       for $desembolso in $contratoDetalle[CODIGO_FONDO=$fondo]
                                        let $intereses := $desembolso/LISTA_INTERESES/INTERES
                                        where $fecha = $desembolso/LISTA_INTERESES/INTERES/FECHA_VENCIMIENTO
                                        return
                                      <ges:detalleCC>
                           ...
                           <ges:tablasDetalle>
                            ...
                            <ges:comentario1></ges:comentario1>
                ...
                <ges:avisoOperacion>
                    ...
                    <ges:informacionPago>
                        ...
                        for $fondo in fn:distinct-values($detalle/LISTA_CONTRATOS/CONTRATO/CODIGO_FONDO)
                        where $detalle/LINEA = $linea 
                        ...
                        <ges:detallePago>
                          ...
                          return
                           ...
                           <ges:Mora>
                                ...
                                <des:referencia>{fn:data($monto/DESEMBOLSO)}</des:referencia>
                                {  
                                for $valor in $monto/LISTA_VENCIMIENTOS/VENCIMIENTO[FECHA_VENCIMIENTO = $fecha]/*
                                return
                                <des:monto>
                                    <com:tipo>
                                        <cat:Descripcion>{fn:name($valor)}</cat:Descripcion>
