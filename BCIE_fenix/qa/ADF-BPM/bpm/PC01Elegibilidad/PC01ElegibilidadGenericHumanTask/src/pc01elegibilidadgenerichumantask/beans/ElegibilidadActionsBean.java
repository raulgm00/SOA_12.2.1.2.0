...
import com.bcie.xmlns.cuestionariomo.CrearReporteNotaConceptoRequestType;
import com.bcie.xmlns.cuestionariomo.CrearReporteNotaConceptoResponseType;
...
public class ElegibilidadActionsBean extends FenixActionBean
{
    ...
    private final String REPORTE_NOTA_FILE_NAME ="ReporteNotaDeConcepto.pdf";
    ...
    private RichSelectBooleanCheckbox sbcbOEJ;
  ...
  private Boolean guardarElegibilidad(Long idOperacion, Boolean esElegible, String strElegibilidad,  String strDeterminar)
    ...
    public String invokeFinalizarTarea()
    {
        ...
        isValidDocumentos = validarDocumentosTarea(getIdOperacion(), codigoTarea,accion); //JURBINA@16062021 validar los documentos por operación, tarea y accion 
        ...
        switch (codigoTarea)
        {
            ...
            case FenixConstants.PC01_INCORPORARCONTEXTOMACRO:
                LOGGER.fine("PC01_INCORPORARCONTEXTOMACRO");
                isValidFinalizarTarea = validarRespuestasCuestionario();
                if(!isValidFinalizarTarea)
                {
                    bundleKeyError = "MSG_CUESTIONARIO_INCOMPLETO";
            ...
            case FenixConstants.PC01_RECOPILARINFOOEJ:
                LOGGER.fine("PC01_RECOPILARINFOOEJ");
        ...
        if (!isValidFinalizarTarea)
        {
            ...
            {
                if(!bundleKeyError.isEmpty() && !bundleKeyError.equals(""))
                {
    ...
    public String invokeMasInformacion()
    {
        ...
        switch (codigoTarea)
        {
            case FenixConstants.PC01_ANALIZARPERFILRIESGO:
                ...
                isValidMasInformacion = isValidMasInformacion && isValidDocumentos;
    ...
    public String aceptarRetornar()
    {
        ...
        switch (codigoTarea)
        {
            case FenixConstants.PC01_ANALIZARELEFIBILIDAD: 
              ...
              Boolean retOEJ = getRetOEJ();
              if(retASJUR || retGERIES || retOEJ)
              {
                ...
                if(retOEJ)
                {
                  actualizarPayloadElement("requiereAjustesOej",true);
                }
    ...
    public void downloadReporteNotaConcepto(FacesContext facesContext, OutputStream outputStream)
    {
        LOGGER.warning("Ingresa Metodo: downloadReporteNotaConcepto");  
        downloadReporteNota(facesContext,outputStream,Boolean.FALSE);
    }    
  /**
   * Descarga el reporte
   */
  ...
  private void downloadReporte(FacesContext facesContext, OutputStream outputStream, Boolean enviarOnBase)
  {
      ...
      try
      {
          ...
          reporteElegibilidad = createPdf(idOperacion, enviarOnBase, false);
    ...
    private void downloadReporteNota(FacesContext facesContext, OutputStream outputStream, Boolean enviarOnBase)
    {
           LOGGER.warning("Ingresa Metodo: downloadReporteNota");
           Long idOperacion = getIdOperacion();
           LOGGER.warning("idOperacion: " + idOperacion);
           byte[] reporteNotaConcepto = null;
           try
           {
               applyIE11Fix(REPORTE_NOTA_FILE_NAME);
               reporteNotaConcepto = createPdf(idOperacion, enviarOnBase, true);

               if (reporteNotaConcepto != null && reporteNotaConcepto.length > 0)
                   outputStream.write(reporteNotaConcepto);
    ...
    /**
        ...
        */
        private byte[] createPdf(long idOperacion, Boolean enviarOnBase, Boolean isNotaConcepto) {
            LOGGER.warning("Ingresa metodo createPdf: ");
            LOGGER.warning("enviarOnBase: " + enviarOnBase);
            LOGGER.warning("isNotaConcepto: " + isNotaConcepto);
            ...
            if (!isNotaConcepto){
                ...
                LOGGER.warning("Es nota de concepto");
                //Reporte Nota de Concepto
                CrearReporteNotaConceptoRequestType request = new CrearReporteNotaConceptoRequestType();
                request.setIdOperacion(idOperacion);
                request.setGuardarReporte(enviarOnBase);
                Archivo file = new Archivo();
                LOGGER.warning("REPORTE_FILE_NAME" + REPORTE_NOTA_FILE_NAME);
                //file.setArchivo(new StringBuilder().append(idOperacion).append(".pdf").toString());
                file.setArchivo(REPORTE_NOTA_FILE_NAME);
                request.setNombreArchivo(file);
                ...
                request.setTipoGeneracion(tipoGen);
                try{LOGGER.log(ADFLogger.WARNING,">HNWS"+ writeXMLRequest(request,request.getClass()).toString());}catch(Exception ex){;}
                 CrearReporteNotaConceptoResponseType response = port.crearReporteNotaConcepto(request);
                try{LOGGER.log(ADFLogger.WARNING,">HNWS"+ writeXMLRequest(response,response.getClass()).toString());}catch(Exception ex){;}
                return response != null ? response.getCuestionario() : null;                
            }          
    ...
    public String getReporteNotaConceptoFileName()
    {
        return REPORTE_NOTA_FILE_NAME;
    }
  ...
  public void setSbcbOEJ(RichSelectBooleanCheckbox sbcbOEJ)
  {
    this.sbcbOEJ = sbcbOEJ;
  }
  
  public RichSelectBooleanCheckbox getSbcbOEJ()
  {
    return sbcbOEJ;
  }
  ...
  private Boolean getRetOEJ()
  {
      
    return sbcbOEJ.isSelected();
  }
  ...
  private Cuestionario12BndQSService initCuestionarioService() {
      ...
      LOGGER.warning("wsdl cuestionario: " + wsdl);
  ...
  public RichPoll getDescargarReportePoll()
  {
    ...
    public Boolean getTieneRiesgoCredito(){
        LOGGER.warning("Incia metodo tieneRiesgoCredito.");
        Integer codigoProducto = getCodigoProducto();
        LOGGER.warning("codigoProducto:" + codigoProducto);
        BindingContainer bindings = getBindings();
        OperationBinding operationBinding = bindings.getOperationBinding("tieneRiesgoCredito");
        operationBinding.getParamsMap().put("idProducto", codigoProducto);
        Boolean result = (Boolean) operationBinding.execute();
        LOGGER.warning("Finaliza metodo tieneRiesgoCredito.");
        return result;
    }
