...
public class FormalizacionActionBean extends FenixActionBean implements Serializable {
   ...
   private List<String> listaAdvertencias = new ArrayList<String>(); 
            ...
            if (condicionesValidas) {
                ...
                if (blnContinuar) {
                    ...
                    if (actualizarComisionesMontoFormalizado()) {
                        ...
                        logger.log(ADFLogger.WARNING, "Inicia validacion aceptarFinalizarTareaContratoFirmadoAction modificaMontoAmpliacion"); 
                        
                        if(operationBinding != null)
                        {
                            logger.log(ADFLogger.WARNING, "modificaMontoAmpliacion encontrada"); 
                            Long idOperacion = formalizacionBean.getOperacionId() != null? Long.valueOf(formalizacionBean.getOperacionId().longValue()) : 0L;
                            ...
                            if(idOperacion != 0L)
                            {
                                operationBinding.getParamsMap().put("idOperacion",idOperacion);
                                operationBinding.execute();
                                logger.log(ADFLogger.WARNING, "aceptarFinalizarTareaContratoFirmadoAction operationBinding de modificaMontoAmpliacion se ejecuto");
                            } 
                         }
                        else
                        {
                            logger.log(ADFLogger.WARNING, "aceptarFinalizarTareaContratoFirmadoAction operationBinding de modificaMontoAmpliacion no existe");
                            JSFUtils.addFacesErrorMessage("aceptarFinalizarTareaContratoFirmadoAction operationBinding de modificaMontoAmpliacion no existe");
                        }
                     } else {
    ...
    /*** START Pantalla Registrar datos de la lï¿½nea de crï¿½dito ***/
    public String aceptarFinalizarTareaRegistrarLineaCreditoAction() {
        ...
        // Validaciï¿½n de datos en header
    ...
    public String aceptarFinalizarTareaValidarLineaCreditoAction() {
        ...
        //    bindings.getOperationBinding("consultarLineaCredito").execute();
        
        ADFContext adfCtx = ADFContext.getCurrent();
        Map sessionScope = adfCtx.getSessionScope();
        Object val = sessionScope.get("fechaValorContrato");
        
        
        Timestamp fechaValorCon = val != null ? (Timestamp)val : null;
                        
        logger.warning("- fechaValor: " + fechaValorCon + " - es esta.");
        
        // Fecha Valor aplica solo para Validar datos de línea de crédito

        logger.warning("idProducto: " + idProducto);
        if(fechaValorCon == null && idProducto == FenixConstants.ID_AMPLIACION_LINEA_GLOBAL_CREDITO_IFI){
