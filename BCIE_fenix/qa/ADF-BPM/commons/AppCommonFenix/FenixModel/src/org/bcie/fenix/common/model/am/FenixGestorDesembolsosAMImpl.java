                                        ...
                                        if (null != estado.getDescripcionCorta() &&
                                            ...
                                            if (null != reglaEvaluacion.getTransaccion()) {
                                                if (reglaEvaluacion.getTransaccion().equalsIgnoreCase(FenixModelConstants.TRANSACCION_REGLA_SCR)) {
                                                    ...
                                                    logger.warning("Creando un nuevo registro en reglasConsultadasMap: " + reglaEvaluacion.getId(), ", " + descripcionEstado);
                                                    ...
                                                    if(reglaEvaluacion.getId() == FenixModelConstants.ID_REGLA_TRANSACCION_MONTO_MAXIMO_OCHO_VECES_SALDO){
                                                        if(descripcionEstado.equals("NO_CUMPLIDA")){
                                                           
                                                            String mensajeErrorRegla23 = (null == reglaEvaluacion.getEstado().getDescripcion())
                                                                                    ? "El monto de desembolso ingresado no debe superar 8 veces la cantidad registrada en los Certificados de depósito del cliente."
                                                                                    : reglaEvaluacion.getEstado().getDescripcion().toString();
                                                            
                                                             reglasConsultadasMap.put("mensajeErrorRegla23", mensajeErrorRegla23);
                                                        } 
                                                    }
                                                    
                                                    if(reglaEvaluacion.getId() == FenixModelConstants.ID_REGLA_VALIDACION_TEMPRANA_FLEXCUBE){
                                                        if(descripcionEstado.equals("NO_CUMPLIDA")){
                                                           
                                                            String mensajeErrorRegla24 = (null == reglaEvaluacion.getEstado().getDescripcion())
                                                                                    ? "El desembolso no se ha validado correctamente contra FLEXCUBE, favor verifique los datos ingresados."
                                                                                    : reglaEvaluacion.getEstado().getDescripcion().toString();
                                                            
                                                             reglasConsultadasMap.put("mensajeErrorRegla24", mensajeErrorRegla24);
                                                        } 
                                                    }
