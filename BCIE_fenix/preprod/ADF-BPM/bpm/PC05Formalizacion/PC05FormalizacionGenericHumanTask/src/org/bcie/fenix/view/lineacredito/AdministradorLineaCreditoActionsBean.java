...
public class AdministradorLineaCreditoActionsBean {
    ...
    private static int countLineasNuevas = 0;
    ...
    public AdministradorLineaCreditoActionsBean() {
        ...
        logger.log(ADFLogger.WARNING, "Valor countLineasNuevas: " + countLineasNuevas); 
    }
    ...
    public void lineaCreditoSelectionListener(SelectionEvent selectionEvent) {
        ...
        // Llenamos datos de la l?nea de cr?dito seleccionada en la tabla DatosLineaCreditoVO
    ...
    private Boolean consultarLineaCredito() {
        ...
        // Se inicializan variables que se envian como parametros para el m?todo consultarLineaCreditoByIdLineaCredito
        ...
        if (null != JSFUtils.resolveExpression(
            ...
            instanciaProceso = (String) JSFUtils.resolveExpression(
                    "#{pageFlowScope.pInstanciaProceso}");
        ...
        if (null != JSFUtils.resolveExpression("#{pageFlowScope.pIdTarea}")) {
            idTarea = (String) JSFUtils.resolveExpression(
                    "#{pageFlowScope.pIdTarea}");
        }
    ...
    public void recuperarIdFlexByLinea() {
        ...
        try {
            ...
            if (operationBinding.getErrors().isEmpty()) {
                ...
                logger.warning(
                    "Error al ejecutar operBinding getIdFlexcubeByIdLinea ->" +
                    operationBinding.getErrors().toString());
                JSFUtils.addFacesErrorMessage(
                    "Ha cocurrido un error al ejecutar el metodo getIdFlexcubeByIdLinea");
            }
        } catch (Exception e) {
            logger.warning("Error al ejecutar operBinding getIdFlexcubeByIdLinea ->",
                e);
            JSFUtils.addFacesErrorMessage(
                "Ha cocurrido un error al ejecutar el metodo getIdFlexcubeByIdLinea");
        }
        ...
        logger.warning("Datos Linea Editale: " +
            administradorLineaCreditoBean.getEsEditable());
    }
    ...
    public void agregarModificarVencimientoPlazoActionListener(
        ...
        if ((opAdjuntarModificar != null) &&
            ...
            // Asignaci?n de variables para modificar row
    ...
    public void agregarModificarComisionActionListener(ActionEvent actionEvent) {
        ...
        // Asignaci?n de variables para row de captura
        voAgregarComision = ADFUtils.getDCBindingContainer()
                                    .findIteratorBinding("AgregarComisionVOIterator");
        rowAgregarComision = voAgregarComision.getRowAtRangeIndex(0);

        // Filtramos el combo con los Tipos de Comisi?n
        ...
        // Ejecutamos query en combo Tipos de Comisi?n (debido a que se filtr?)
        voTiposComision = ADFUtils.getDCBindingContainer()
                                  .findIteratorBinding("TiposComisionLOVIterator");
        voTiposComision.executeQuery();

        // Inicializamos el VO AgregarComisionVO con el valor seleccionado por defecto en Tipos de Comisi?n
        ...
        if ((opAdjuntarModificar != null) &&
            ...
            // Asignamos valor en combo Tipos de Comisi?n
    ...
    public void aceptarAgregarModificarComisionActionListener(
        ...
        // Insertamos/modificamos en VO la Comisi?n
        if (opAdjuntarModificar.equalsIgnoreCase("agregar")) {
            //--------------------------------------------------------------------------------------------------------------
            try {
    ...
    public void aceptarEliminarVencimientoComisionActionListener(
        ...
        if ((opEliminarOrigen != null) &&
            ...
            // Si el Id Comisi?n no es null, agregamos Id a lista de eliminados
            if (idComision != null) {
                administradorLineaCreditoBean.getIdsComisionesEliminadas()
                                             .add(idComision);
            }

            // Eliminamos Comisi?n seleccionada de tabla
    ...
    public void guardarDatosLineaCreditoActionListener(ActionEvent actionEvent) {
        ...
        if (validarDatosLineaCredito(false)) {
            ...
            // Actualiza FechaValorContrato - 42472
            Integer idProducto =(Integer)JSFUtils.resolveExpression("#{pageFlowScope.pIdProducto}");
            if(idProducto == FenixConstants.ID_AMPLIACION_LINEA_GLOBAL_CREDITO_IFI){
				Timestamp fechaValorCon = (Timestamp) JSFUtils.resolveExpression("#{bindings.FechaValorContrato.attributeValue}");
				logger.warning("- fechaValor: " + fechaValorCon + " - 42472");
				
				operationBinding = bindings.getOperationBinding("actualizarFechaValorContrato");
				...
				if(null != JSFUtils.resolveExpression("#{bindings.IdContrato1.inputValue}")){
					...
					operationBinding.getParamsMap().put("idContrato", idContratoLong.longValue());
				}
				
				operationBinding.getParamsMap().put("idOperacion", JSFUtils.resolveExpression("#{pageFlowScope.pIdOperacion}"));
				operationBinding.getParamsMap().put("instanciaProceso", JSFUtils.resolveExpression("#{pageFlowScope.pInstanciaProceso}"));
				operationBinding.getParamsMap().put("fechaValorContrato", JSFUtils.resolveExpression("#{bindings.FechaValorContrato.attributeValue}"));
            ...
            // Actualizaci?n exitosa de la L?nea de cr?dito
            if (operationBinding.getErrors().size() < 1) {
                ...
                // Limpiamos tabla de L?neas de cr?dito
                ...
                // Actualizamos datos del encabezado y L?neas de cr?dito (se cargan todos los datos de nuevo)
                operationBinding = bindings.getOperationBinding(
                        "consultarLineaCredito");
                operationBinding.getParamsMap()
                                .put("idOperacion",
                ...
                // Inicializamos pantalla con datos de la l?nea de cr?dito seleccionada en la tabla DatosLineaCreditoVO
                if (esGuardarExitoso) {
                    ...
                    if (!(countLineasNuevas == 0)){
                        setCambioLineasCredito("S");
                        logger.log(ADFLogger.WARNING, "Set CambioLineasCredito: S >> Numero de lineas modificadas: " + countLineasNuevas);
                        countLineasNuevas = 0;
                        logger.log(ADFLogger.WARNING, "Set Numero de lineas modificadas: " + countLineasNuevas);
                    }
                    if (!(countLineasNuevas == 0)){
                        setCambioLineasCredito("S");
                        logger.log(ADFLogger.WARNING, "Set CambioLineasCredito: S >> Numero de lineas modificadas: " + countLineasNuevas);
    ...
    private void setCambioLineasCredito(String valor)
    {
        ADFContext adfCtx = ADFContext.getCurrent();
        Map sessionScope = adfCtx.getSessionScope();
        Object val = sessionScope.put("CambiolineasCreditos", valor);
    }
    ...
    public Boolean validarDatosLineasCredito(Integer indiceLineaActual) 
    {
        ...
        if (dlcRowCount == 0) {
            ...
            int actualIndex = (indiceLineaActual != null) ? indiceLineaActual
                                                          : voDatosLineaCredito.getCurrentRowIndexInRange();
    ...
    private Boolean validarDatosLineaCredito(Boolean actualizaFlagLineasValidas) {
        ...
        if (JSFUtils.resolveExpression(
            ...
            if ((cantidadDeciInt != 0) && (cantidadDeciInt != 2) &&
                    (cantidadDeciInt != 3) && (cantidadDeciInt != 4)) {
        ...
        if (JSFUtils.resolveExpression("#{bindings.SpreadTasa.inputValue}") != null) {
            String candidadDecimalesStr = JSFUtils.resolveExpression(
                    "#{bindings.SpreadTasa.inputValue}").toString();
            Integer cantidadDeciInt = Integer.valueOf(candidadDecimalesStr);

            if (!(((cantidadDeciInt >= 2) && (cantidadDeciInt <= 5)) ||
                    (cantidadDeciInt == 0))) {
                esDatosCorrectos = Boolean.FALSE;
                JSFUtils.addFacesErrorMessage(ADFUtils.getStringFromBundle(
                        "CANTIDAD_DECIMALES_SPREAD", BUNDLE));
            }
        }
        ...
        // Fecha valor contrato es obligatorio
        if( idProducto == FenixConstants.ID_AMPLIACION_LINEA_GLOBAL_CREDITO_IFI && 
                ((JSFUtils.resolveExpression("#{bindings.FechaValorContrato.attributeValue}") == null) 
                || (JSFUtils.resolveExpression("#{bindings.FechaValorContrato.inputValue}").toString().trim().length() < 1))) {

            esDatosCorrectos = Boolean.FALSE;
            JSFUtils.addFacesErrorMessage(getStringFromBundle("CAMPO_FECHA_VALOR_ES_REQUERIDO") );
        }
        ...
        //De esta secci?n hacia abajo se envian como es datos correctos dependiendo si es validarLineaCredito el llamadoDirecto o validarLineasCredito
        if (!poseeDesembolsoTotalidadRecursos) {
    ...
    public RichOutputText getOtInitForm() {
        ...
        // Inicializamos datos de la l?nea de cr?dito seleccionada en la tabla DatosLineaCreditoVO
        consultarLineaCredito();
     ...
     * Post-PROD-Formalizaci?n-KMBZ-3794-Campos vac?os en Registrar L?nea de Cr?dito para Ayuda de Emergencia
    ...
    public void aceptarAgregarLineaCreditoActionListener(
        ...
        try {
            sIdTipoMonedaMontoLinea = (String) JSFUtils.resolveExpression(
                    "#{bindings.MonedaLinea11.inputValue}");
        } catch (Exception e) {
            logger.log(ADFLogger.WARNING,
        ...
        try {
            if (null == sIdTipoMonedaMontoLinea) {
                bcIdTipoMonedaMontoLinea = (Integer) JSFUtils.resolveExpression(
                        "#{bindings.MonedaLinea11.inputValue}");
            ...
            logger.log(ADFLogger.WARNING,
                "Error no se puede castear el valor id Tipo Moneda MontoLinea1");
        }
        ...
        try {
            ...
            logger.log(ADFLogger.WARNING,
                "Error no se puede castear el valor MontoLinea1 1");
            e.getStackTrace();
        }
        ...
        try {
            ...
            logger.log(ADFLogger.WARNING,
                "Error no se puede castear el valor MontoLinea1 2");
            e.getStackTrace();
        }
        ...
        if (null != sMontoLinea) {
            try {
                ...
                montoLinea = NumbersUtil.stringToBigDecimal(FormalizacionConstants.FORMATO_MONTO_LINEA,
                        sMontoLinea);
        ...
        operationBinding.getParamsMap()
                        .put("idTipoMonedaMontoLinea", bcIdTipoMonedaMontoLinea);
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            if ((result != null) && result) {
                countLineasNuevas = countLineasNuevas + 1;
    ...
    public void aceptarEliminarLineaCreditoActionListener(
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            //Actualizamos conteo de numero de lineas
            countLineasNuevas = countLineasNuevas - 1;
        ...
        if (null != valueChangeEvent) {
            ...
            if (null != valueChangeEvent.getNewValue()) {
                ...
                case "Primer desembolso":
                    tipofechaInicio = 10;
                    break;
                case "Notificaci?n al Cliente de consecuci?n de recursos":
                    tipofechaInicio = 12;
                    break;
    ...
    public void isLineaChangeListener(ValueChangeEvent valueChangeEvent) {
        // Si hay un cambio en los valores se invalida el estado de las lineas
        setCambioLineasCredito("S");
    }
    
    public void isMultipleChangeListener(ValueChangeEvent valueChangeEvent) {
        // Si hay un cambio en los valores se invalida el estado de las lineas
        setCambioLineasCredito("S");
        logger.warning("Set CambioLineasCredito: S  >> Actualizacion de info lineas de credito");
        setLineasValidasGuardadas("N");
    }
    ...
    public void montoLineavalueChangeListener(ValueChangeEvent valueChangeEvent) {
        logger.warning("Inside montoLineavalueChangeListener: " +
            valueChangeEvent.getComponent().getId());
    }

    public void montoLinea1valueChangeListener(
        ValueChangeEvent valueChangeEvent) {
        logger.warning("Inside montoLinea1valueChangeListener: " +
            valueChangeEvent.getComponent().getId());
    }
    ...
    public void seleccionaOpcionMora(ValueChangeEvent valueChangeEvent) {
        ...
        if (opcionMora != null) {
            if (opcionMora.equals(opcion1) || opcionMora.equals(opcion2)) {
                AttributeBinding ValorTasa = (AttributeBinding) bindings.getControlBinding(
                        "ValorTasa");
                ValorTasa.setInputValue("");

                AttributeBinding CodigoTasaReferencia = (AttributeBinding) bindings.getControlBinding(
                        "CodigoTasaReferencia");
                CodigoTasaReferencia.setInputValue("");

                AttributeBinding Frecuencia1 = (AttributeBinding) bindings.getControlBinding(
                        "Frecuencia1");
                Frecuencia1.setInputValue("");

                AttributeBinding SpreadTasa = (AttributeBinding) bindings.getControlBinding(
                        "SpreadTasa");
    ...
    public void vaciaCantidadDecimales(ValueChangeEvent valueChangeEvent) {
        ...
        valueChangeEvent.getComponent()
                        .processUpdates(FacesContext.getCurrentInstance());
        rounding = JSFUtils.resolveExpression(
                "#{bindings.TcaTipoRedondeoVO1.inputValue}").toString();
        ...
        if (rounding.equals(opcion1)) {
            ...
            BindingContainer bindings = BindingContext.getCurrent()
                                                      .getCurrentBindingsEntry();
            AttributeBinding cantidadDecimales = (AttributeBinding) bindings.getControlBinding(
                    "CantidadDecimales");
