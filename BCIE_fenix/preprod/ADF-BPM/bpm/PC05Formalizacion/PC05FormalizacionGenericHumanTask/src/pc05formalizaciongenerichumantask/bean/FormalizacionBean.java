...
public class FormalizacionBean extends FenixPanelBean implements Serializable {
    ...
    public void getPayloadInformation() throws WorkflowException, IOException {
        ...
        if ((nl != null) && (nl.getLength() > 0)) {
            ...
            try {
                if ((idOperacion != null) && (idOperacion.trim().length() > 0)) {
                    operacionId = new oracle.jbo.domain.Number(Long.valueOf(
                                idOperacion));
                }
            } catch (Exception e) {
                logger.log(ADFLogger.ERROR,
                    "Error al inicializar el valor de operacionId: " +
                    e.getMessage());
            }
        }
        ...
        if (nl != null) {
            if (nl.getLength() > 0) {
                setSolicitudContratacion(Boolean.parseBoolean(
        ...
        if (getSolicitudContratacion()) {
            ...
            if (nl != null) {
                if (!nl.item(0).getTextContent().equalsIgnoreCase("") &&
                        (nl.item(0).getTextContent().length() > 0)) {
                    setIdLote(Long.parseLong(nl.item(0).getTextContent()));
                }
            }
        } else {
            logger.warning(
                "Solicitud contratacion es falso, no se obtendra idLote/idEnmienda.");
        }

        instanciaProceso = BPMUtils.getCurrentTask().getProcessInfo()
                                   .getInstanceId();
        //test gabriel
        ...
        logger.log(ADFLogger.WARNING,
            "El producto esta configurado para botones dinamicos: " +
            getEstaConfiguradoProducto());
    ...
    public void obtenerNumeroResolucion() {
        ...
        try {
            logger.warning("Valor de la operacion : " +
                Long.valueOf(getIdOperacion()));
            numResolucionBinding = ADFUtils.findOperation(
                    "obtenerUltimoNumeroResolucion");
            numResolucionBinding.getParamsMap()
                                .put("idOperacion",
                Long.valueOf(getIdOperacion()));
            ...
            if (null != numResolucionObject) {
                ...
                logger.warning("Numero de la resolucion obtenida : " +
                    numeroResolucion);
            } else {
     ...
     * @by Raul Garcia
     */
    public void setConfigurtacionVisibilidad(
        Map<String, TablaDinamicaVOBean> m, Boolean ban) {
    ...
    public void setEsFechaVigenciaIgualFechaFirma(
        Boolean esFechaVigenciaIgualFechaFirma) {
    ...
    public void setMostrarDerivadosImplicitos(
        Boolean mostrarDerivadosImplicitos) {
