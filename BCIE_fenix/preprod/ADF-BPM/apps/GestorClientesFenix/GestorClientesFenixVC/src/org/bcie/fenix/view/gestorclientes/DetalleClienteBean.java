...
public class DetalleClienteBean {
  ...
  public void addPartialTarget(){
    ...
    private void abrirDetalleOperacion() {
        ...
        // Abrimos popup con aplicaci�n del Gestor de Operaciones
        ...
        try{
            ...
            if (roles != null && roles.length>0) {
                for (int i=0;i<roles.length;i++) {
                    logger.warning("Rol del usuario - GPA "+roles[i]);
                    if (roles[i]!=null && roles[i].contains("FENIX_GERENTES") ) {
                        logger.warning("Es GERENTE - gpa! "+roles[i]);
                        setEsResponsableCliente(Boolean.TRUE);
                        break;
                    }
                }
            }
    ...
    public void diaPagoVCListener(ValueChangeEvent valueChangeEvent) {
        if (valueChangeEvent != null && valueChangeEvent.getNewValue() != null) {
            if (valueChangeEvent.getNewValue() instanceof Integer) {
                Integer diaPagoInt = (Integer) valueChangeEvent.getNewValue();
                ADFUtils.setBoundAttributeValue("DiaPago", diaPagoInt.intValue());
            } else if (valueChangeEvent.getNewValue() instanceof BigDecimal) {
