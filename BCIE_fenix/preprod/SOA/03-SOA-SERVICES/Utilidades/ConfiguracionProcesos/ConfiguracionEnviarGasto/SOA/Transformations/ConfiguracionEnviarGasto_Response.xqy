...
declare variable $inputVariable.request as element() (:: schema-element(con:ConfiguracionEnviarGastoRequest) ::) external;
declare variable $outInvokeConsultarRolesPorProceso.response as element() (:: schema-element(con1:responseConsultarRolesPorProcesoMessage) ::) external;

declare function local:funcConfiguracionenviargasto_response($outInvokeConsultarProductoByOperacion.response as element() (:: schema-element(pro:ConsultarProductoByIdOperacionResponse) ::), 
                                                             $inputVariable.request as element() (:: schema-element(con:ConfiguracionEnviarGastoRequest) ::), 
                                                             $outInvokeConsultarRolesPorProceso.response as element() (:: schema-element(con1:responseConsultarRolesPorProcesoMessage) ::)) 
                                                             as element() (:: schema-element(con:ConfiguracionEnviarGastoResponse) ::) {
    <con:ConfiguracionEnviarGastoResponse>
        <con:configuracionEnviarGasto>
            <con2:Header>
                {
                    if (fn:true()) then 
                        <ns1:Operacion>
            ...
            <con2:RolesEquipoTrabajo>
                {
                    for $listadoRoles in $outInvokeConsultarRolesPorProceso.response/con1:ListadoRoles/con3:listadoRoles
                    return 
                    <con2:Rol>
                        {
...
local:funcConfiguracionenviargasto_response($outInvokeConsultarProductoByOperacion.response, $inputVariable.request, $outInvokeConsultarRolesPorProceso.response)
