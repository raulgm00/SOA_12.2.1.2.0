...
declare function local:func($responseMapeoErrorMessage as element() (:: schema-element(ns2:responseMapeoErrorMessage) ::),
                            $message as xs:string) as element() (:: schema-element(ns1:ConsultarGestionCobroResponse) ::) {
    <ns1:ConsultarGestionCobroResponse>
        <ns1:Resultado>
            <res:result>ERROR</res:result>
            <res:message>{fn:data($message)}</res:message>
            <res:error>
                <err:errorCode>{fn:data($responseMapeoErrorMessage/ns2:Result/res:error/err:errorCode)}</err:errorCode>
                <err:errorDescription>{fn:data($responseMapeoErrorMessage/ns2:Result/res:error/err:errorDescription)}</err:errorDescription>
                <err:errorType>{fn:data($responseMapeoErrorMessage/ns2:Result/res:error/err:errorType)}</err:errorType>
            </res:error>
        </ns1:Resultado>
    </ns1:ConsultarGestionCobroResponse>
};
