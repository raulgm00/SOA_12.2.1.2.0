...
declare function local:func($ConsultarDesembolsosConfiguracionById as element() (:: schema-element(ns1:ConsultarDesembolsosConfiguracionByIdResponse) ::), 
                            $ConvertirMonedasImportesResponse as element() (:: schema-element(ns2:ConvertirMonedasImportesResponse) ::)) 
                            as element() (:: schema-element(ns1:ConsultarDesembolsosConfiguracionByIdResponse) ::) {
    <ns1:ConsultarDesembolsosConfiguracionByIdResponse>
        {               
        ...
        <ns1:ContratoDesembolso>
            ...
            <des:referencia>{fn:data($desembolso/des:referencia)}</des:referencia>
            {
                ...
                <des:monto>
                    ...
                    {
                    let $importe := if(count($ConvertirMonedasImportesResponse/ns2:ConvierteMonedasImporte[con:monedaDe/cat:Id = $idDesembolso]/con:montoConvertido)=0)
                    then $desembolso/des:monto[1]/com:importe
                    else $ConvertirMonedasImportesResponse/ns2:ConvierteMonedasImporte[con:monedaDe/cat:Id = $idDesembolso]/con:montoConvertido
                    return
        ...
        <ns1:Resultado>
            <res:result>{fn:data($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:result)}</res:result>
            <res:message>{fn:data($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:message)}</res:message>
            {
                if ($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:error)
                then 
                    <res:error>
                        <err:errorCode>{fn:data($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:error/err:errorCode)}</err:errorCode>
                        <err:errorDescription>{fn:data($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:error/err:errorDescription)}</err:errorDescription>
                        <err:errorType>{fn:data($ConsultarDesembolsosConfiguracionById/ns1:Resultado/res:error/err:errorType)}</err:errorType>
                    </res:error>
                else ()
            }
        </ns1:Resultado>
...
local:func($ConsultarDesembolsosConfiguracionById, $ConvertirMonedasImportesResponse)