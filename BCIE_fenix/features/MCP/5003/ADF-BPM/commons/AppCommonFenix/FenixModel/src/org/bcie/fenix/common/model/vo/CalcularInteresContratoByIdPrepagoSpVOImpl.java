    ...
    public BigDecimal interesContratoByIdPrepago(Long idPrepago){
        logger.warning("Inf, inicia el metodo ejecutarSPCalculoInteres.");   
            ...
            try {
                ...
                callableStatement.setLong(1, idPrepago);       
                ...
                while(rs.next()){
                    ...
                    row.setAttribute("ContractDesembolso",(null != rs.getString("CONTRACT_DESEMBOLSO")) ?
                                                                    rs.getString("CONTRACT_DESEMBOLSO") : null);
                    
                    row.setAttribute("Interes",(null != rs.getBigDecimal("INTERESES")) ?
                                                     rs.getBigDecimal("INTERESES") : null);

                    row.setAttribute("Id_tre_pre_count",(null != rs.getObject("ID_TRE_PRE_CONT")) ?
                                                                 (Integer)rs.getInt("ID_TRE_PRE_CONT") : new Integer(0) );                  
                    id++;
                    this.insertRow(row);
                }
              ...
              Row fila = getRow(new Key(new Object[] {new Long(1L)}));
                ...
                logger.log(ADFLogger.ERROR,"Exception al llenar CalcularInteresByIdPrepagoSpVo : " 
                                                                                      +sqlerror.getMessage());
                throw new JboException(sqlerror);
            }
            

            logger.warning("Inf, Termina el metodo ejecutarSPCalculoInteres. interes recuperado : "
                                                                       +interes+ " del idPrepago : "+idPrepago); 
            return interes;
        }
