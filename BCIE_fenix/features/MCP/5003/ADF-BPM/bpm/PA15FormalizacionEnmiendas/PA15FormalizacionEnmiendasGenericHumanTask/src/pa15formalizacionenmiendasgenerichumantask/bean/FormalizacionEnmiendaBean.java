...
public class FormalizacionEnmiendaBean extends FenixPanelBean implements Serializable {
    ...
    /** 
     * Mapa que contiene las desobligaciones finalizadas de las lineas de credito que se almacenan en el payload.  
     */
    private Map<String, String> mapDesobligacionFin;
    ...
    public void getPayloadInformation() throws WorkflowException {
        LOGGER.warning("Inicia carga del payload: getPayloadInformation");
        
        String instance = BPMUtils.getCurrentTask().getProcessInfo().getInstanceId();
        this.setInstanciaProceso(instance);
        
        LOGGER.warning("Instancia proceso: {0}", instance);
        ...
        this.setMapDesobligacionFin(reader.getMapDesobligacionFin());
    ...
    public Map<String, String> getMapDesobligacionFin() {
        LOGGER.info("getMapDesobligacionFin");
        LOGGER.info("mapDesobligacionFin2: " + this.mapDesobligacionFin);
        return mapDesobligacionFin;
    }

    public void setMapDesobligacionFin(Map<String, String> mapDesobligacionFin) {
        LOGGER.info("setMapDesobligacionFin");
        LOGGER.info("mapDesobligacionFin1: " + mapDesobligacionFin);
        this.mapDesobligacionFin = mapDesobligacionFin;
        LOGGER.info("mapDesobligacionFin2: " + this.mapDesobligacionFin);
    }
    ...
    public void handleUpdateLCEvent(Long idLineaCredito) {
        LOGGER.warning("Inicio handleUpdateLCEvent");
        ...
        LOGGER.warning("Obteniendo monto a desobligar");
        ...
        LOGGER.warning("Invocando actualización de la tarea...");
        writer.updateTask();
        LOGGER.warning("Finalizo handleUpdateLCEvent");
    }

    /**
     * Gestiona el evento lanzado por la región de líneas de crédito cuando se actualizan las líneas y entonces se deben
     * actualizar si el proceso de actualizar informacion en flexcube finalizo correctamente. 
     *
     * @param idLineaCredito ID de la línea de crédito que se actualizó.
     */
    public void handleDesobligacionFinEvent(String esDesobligacionFin) {
        LOGGER.warning("Inicio handleDesobligacionFinEvent");
        
        
        Boolean estaDesobligada = Boolean.FALSE;
        
        LOGGER.warning("validar esDesobligacionFin: " + esDesobligacionFin);
        if(esDesobligacionFin.toLowerCase().equals("true")){
            estaDesobligada = Boolean.TRUE;
        }
        else
        {
            estaDesobligada = Boolean.FALSE;
        }
        String paramName = PA15Constants.PALYOAD_PREFIXDESOBLIGAON_FIN;
        
        LOGGER.warning("paramName: " + paramName);        
        LOGGER.warning("estaDesobligada: " + estaDesobligada);
        
        LOGGER.info("Creando writer para modificar payload de la tarea");
        FormalizacionEnmiendaPayloadWriter writer = new FormalizacionEnmiendaPayloadWriter();
        
        writer.putParameterType(paramName, estaDesobligada.toString());
        LOGGER.warning("Invocando actualizacion de la tarea...");
        writer.updateTask();
        LOGGER.warning("Finalizo handleDesobligacionFinEvent");
    }
    ...
    public void handlePropagarLCEvent(Object eventPayLoad) {
        LOGGER.info("Dentro de handlePropagarLCEvent");
