...
public final class FormalizacionEnmiendaPayloadReader {
    ...
    /** Su valor proviene del Payload de la tarea.*/ 
    private final Map<String, String> mapDesobligacionFin;
    ...
    private FormalizacionEnmiendaPayloadReader(XMLElement xmlPayload) {
        ...
        if (xmlPayload == null) {
            ...
            this.mapDesobligacionFin = Collections.emptyMap();
            ...
            nl = xmlPayload.getElementsByTagName(PA15Constants.PAYLOAD_PARAMETERTYPE_TAGNAME);
            this.mapDesobligacionFin = this.readParameterTypeDesobligacionFin(nl, PA15Constants.PALYOAD_PREFIXDESOBLIGAON_FIN);
        }
    }
    ...
    public static FormalizacionEnmiendaPayloadReader readCurrentTaskPayload() throws WorkflowException {
        LOGGER.warning("Inicia readCurrentTaskPayload: Leyendo FormalizacionEnmiendaPayload");
        ...
        LOGGER.warning("Finaliza readCurrentTaskPayload");
    ...
    private Map<Long, BigDecimal> readParameterTypeDesobligar(NodeList nl, String prefix){
        ...
        if (nl != null && nl.getLength() > 0) {
            LOGGER.warning("{0} ParameterType encontrados", nl.getLength());
            ...
            for (int i = 0; i < nl.getLength(); i++) {
                ...
                if (LOGGER.isInfo()) {
                    if (paramName != null) {
                        ...
                        LOGGER.warning("{0}=>{1}", new Object[] { nameStr, valueStr });
                    }
                }
                ...
                if (paramName != null && paramName.getTextContent() != null &&
                    paramName.getTextContent().contains(prefix) && PA15Constants.PALYOAD_PREFIXDESOBLIGAR.contains(prefix)) {
                    ...
                    if (paramValue != null && paramValue.getTextContent() != null) {
                        try {
                            ...
                            LOGGER.log(Level.WARNING, msg, new Object[] { paramValue.getTextContent() }, ex);
                        }
                    }
     ...
     * montos a desobligar, el mapa se retorna vacío.
     */
    private Map<String, String> readParameterTypeDesobligacionFin(NodeList nl, String prefix){
        LOGGER.warning("Dentro de readParameterTypeDesobligar");
        ...
        if (nl != null && nl.getLength() > 0) {
            ...
            for (int i = 0; i < nl.getLength(); i++) {
                ...
                if (paramName != null && paramName.getTextContent() != null &&
                    paramName.getTextContent().contains(prefix) && PA15Constants.PALYOAD_PREFIXDESOBLIGAON_FIN.contains(prefix)) {

                    //recorta el ID de la línea de crédito
                    String completeName = paramName.getTextContent();
                    String lcStr = "DESOBLIGACION_FIN"; 

                    //Convierte el valor a desobligar, si lo tiene, a BigDecimal
                    String isDesobligacionFin = null;
                    if (paramValue != null && paramValue.getTextContent() != null) {
                        try {
                            isDesobligacionFin =  paramValue.getTextContent(); 
                            
                            LOGGER.warning("isDesobligacionFin=>" + isDesobligacionFin);
                        } catch (Exception ex) {
                            String msg = "Error leyendo Payload. No se pudo convertir a BigDecimal: {0}";
                            LOGGER.log(Level.WARNING, msg, new Object[] { paramValue.getTextContent() }, ex);
                        }
                    ...
                    map.put(lcStr, isDesobligacionFin);
                }
            }
        } else {
            LOGGER.warning("No se encontraron ParameterTypes en el payload");
    ...
    public Map<String, String> getMapDesobligacionFin(){
        return new HashMap<>(this.mapDesobligacionFin);
    }
