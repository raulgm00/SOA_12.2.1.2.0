...
public class TreSolicitudLineaCreditoVOImpl extends ViewObjectImpl implements TreSolicitudLineaCreditoVO {
  ...
  //se modifico metodo el 09/02/2017
    ...
    public Boolean obtenerIdContratoSolicitudCambiarEstado(Long idSolicitud, Long idOperacion){
        logger.warning("Inicio metodo obtenerIdContratoSolicitudCambiarEstado");
        Boolean resultado = Boolean.FALSE;
        ...
        if(getEstimatedRowCount() > 0){
            ...
            try{
                ...
                logger.warning("Cambiando estado de contratos SIN error en servicio de verificarValidacionAsignacion.");
                if (null != iter) {
                    iter.reset();
                    while (iter.hasNext()) {
                        row = iter.next();
                        if (null != row) {
                            if (null != row.getAttribute("IdContratoDesembolso")) {
                                ...
                                if(null != idContrato){
                                    resultado = fenixGestorDesembolsosAM.getContratoDesembolsoVO().cambiarEstadoContratoValidacion(idContrato, requiereValidacionAsignacion);                                    
                                    if(resultado){
                                        logger.warning("Se ha cambiado el estado del contrato");
                                        
                                        logger.warning("Realizando COMMIT para el cambio de estado de los contratos a Validacion de recursos.");
                                        try{
                                            getDBTransaction().commit();
                                            logger.warning("primer commit exitoso");
                                        }catch(Exception ex){
                ...
                logger.warning("Cambiando estado de contratos CON error en servicio de verificarValidacionAsignacion.");
                if(null == requiereValidacionAsignacion || requiereValidacionAsignacion){
                    if (null != iter) {
                        iter.reset();
                        while (iter.hasNext()) {
                            row = iter.next();
                    ...
                    logger.warning("No se pudo consumir correctamente servicio VerificarValidacionAsignacion.");
                    iter.closeRowSetIterator();
                    throw e;
                }
            }
        ...
        this.getViewCriteriaManager().removeApplyViewCriteriaName("TreSolicitudLineaCreditoPorSolicitudVOCriteria");
        logger.warning("Termina metodo obtenerIdContratoSolicitudCambiarEstado");
        return resultado;
    }
