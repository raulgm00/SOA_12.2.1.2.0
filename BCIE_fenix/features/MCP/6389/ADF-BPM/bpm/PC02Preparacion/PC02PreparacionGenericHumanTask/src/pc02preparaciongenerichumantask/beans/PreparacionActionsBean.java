...
public class PreparacionActionsBean extends FenixActionBean {
    ...
    public void desactivaChecks() {
        ...
        OperationBinding operationBinding = bindings.getOperationBinding(
                "activarComisionesValidadas");
        Object result = operationBinding.execute();
    }

    /**
     * Overview: Invocacion de boton Finalizar (comun para todas las pantallas)
     * US: 98979
     * Date: 06-10-2021
     * Developer: @RUGM
     * Step: 1
     * Nota: etiquetas de rastreo
     * */
    public String invokeFinalizarTarea() {
            ...
            if (validarEquipos) {
                ...
                if (validaComisionTramite) {
                    ...
                    if (comisionSugeridaMandatoria) {
                        Boolean requiereIBCIE = obtenerColumna(preparacion.getIdProducto(),
                                FenixConstants.PC03_IBCIE);
                        Boolean requiereSIEMAS = obtenerColumna(preparacion.getIdProducto(),
                                FenixConstants.PC03_SIEMAS);
                        ...
                        if (existeProcesoEvaluacionesIniciado(idOperacion,
                                    ID_PROCESO_EVALUACION)) {
                        ...
                        attributeBinding = ADFUtils.findControlBinding(
                                "requiereProcesoEvaluaciones");
                        ...
                        JSFUtils.addFacesErrorMessage(getStringFromBundle(
                                "MSG_ERROR_MANDATORIA_SUGERIDA"));
                    }
                } else {
                    JSFUtils.addFacesErrorMessage(getStringFromBundle(
                            "CONFIRMA_COMSIONES_TRAMITE"));
                }
            } else {
                JSFUtils.addFacesErrorMessage(getStringFromBundle(
                        "CONFIRMAR_EQUIPO"));
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_ENVIAR_OPERACION_ANALISIS)) {
            ...
            if (preparacion.getValidaAnalisis()) {
                ...
                if (!validaObjecion &&
                        validaNombreObjecion.equalsIgnoreCase("No")) {
                    JSFUtils.addFacesErrorMessage(getStringFromBundle(
                            "CONFIRMA_NO_OBJECION"));
                } else {
                    if (isValidDocumentos) {
                        invocarPopupFinalizarTarea();
                    }
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_EMITIR_OPINION_TECNICA) || preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_VALIDAR_MATRIZ_ACCIONES)) {
            if (isValidDocumentos) {
                invocarPopupFinalizarTarea();
            }
        }
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_HOJA_TERMINOS_SEPRI) || preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC) ||
                preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT) || preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES) ||
                preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_VALIDAR_MATRIZ_ACCIONES) || preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD) ||
                preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD) || preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD)) 
        {
            if (isValidDocumentos) {
            if (preparacion.getCodigoTarea()
                               .equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC)) {
        ...
        /**
         * Overview: Validacion de la actividad en la cual esta parado TAREA_REVISAR_COMISIONES = "15"
         * US: 98979
         * @Feature:6389 US:6415
         * Date: 06-10-2021
         * Developer: @RUGM
         * Step: 2
         * Nota: etiquetas de rastreo
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_REVISAR_COMISIONES))
        {
            ...
            /**
             * Overview: Validacion de montos pagadas
             * US: 98979
             * @Feature:6389 US:6415
             * Date: 06-10-2021
             * Developer: @RUGM
             * Step: 3
             * Nota: se inabilita codigo y s emigra a PA02AdministracionCOmisiones 
             * */
            /*Boolean comisionesPagadas = validarOperacionComisionesPagadas();
            if( comisionesPagadas )
            {*/
        ...
        /*    }else{
                 ...
                 * Step: 4
                 * */
        /*        JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_MONTO_COMISION_NO_PAGADA"));
            }*/
    ...
    /**
     * Overview: Se crea metodo de Validacion de montos pagadas
     * US: 98979
     * @Feature:6389 US:6415
     * Date: 06-10-2021
     * Developer: @RUGM
     * Step: 2.1
     * Nota: codificacion, se inabilita el metodo por cuestiones de equivocacion de implementacion, el codigo se migra a PA02AdministracionComisiones
     * */
    
    public Boolean validarOperacionComisionesPagadas(){
        LOGGER.warning("Inside validarOperacionComisionesPagadas -  " );
        
        Boolean retorna = Boolean.TRUE;
        //Iterador que tiene toda la logica de neogcio abstraida desde base de datos
        DCIteratorBinding voComisionesValidadas = ADFUtils.getDCBindingContainer().findIteratorBinding("CargoVOIterator");
        ViewObject eq = voComisionesValidadas.getViewObject();
        Row[] filas = eq.getAllRowsInRange();
        LOGGER.warning("Total de Filas de CargoVOIterator -  " +filas.length);
        ...
        for (Row row : filas) {
                ...
                LOGGER.warning("Codigo Fila: " + codigoComison);
                LOGGER.warning("Codigo Estado: " + estadoComision);

                    if ((estadoComision.compareTo(FenixConstants.ESTADO_COMISION_PAGADO) == 0)) {
                            
                        LOGGER.warning("Codigo esta actualizado su Estado: " + estadoComision);
                        /**
                         * Comision que buscamos PC02_ESTADO_COMISION_PAGADA = 6;
                         * */
                        LOGGER.warning("Comisiones ESTADO_COMISION_PAGADO : " + estadoComision);
    ...
    public Boolean existeProcesoEvaluacionesIniciado(Long idOperacion,
        Integer idProceso) {
        ...
        try {
            OperationBinding operationBinding = bindings.getOperationBinding(
                    "existeProcesoIniciadoByOperacion");
            ...
            LOGGER.warning(
                "Ha ocurrido un error en el metodo existeProcesoEvaluacionesIniciado ");
            JSFUtils.addFacesErrorMessage(
                "Error al validar proceso de evaluacion existente");
        }

        LOGGER.warning(
            "termina inicia metodo existeProcesoEvaluacionesIniciado");
    ...
    public Boolean validarComisionesValidadas(Boolean finaliza) {
        ...
        LOGGER.warning("Total de Filas de CargoVOIterator - Codigo esta actualizado su Estado: " +comisionValidadoFalso.length);
                ...
                if (finaliza) {
                    if ((estadoComision.compareTo(FenixConstants.ESTADO_COMISION_DISPENSADO) == 0) ||
                        ...
                        if (estadoComision.compareTo(FenixConstants.ESTADO_COMISION_POR_APLICAR) == 0) {
                            retorna = actualizarComisionTCC(FenixConstants.ESTADO_COMISION_VALIDADO.longValue(),codigoComison,FenixConstants.SUBESTADO_COMISION_VALIDADO.longValue());
                        }

                        if (estadoComision.compareTo(FenixConstants.ESTADO_COMISION_PORDISPENSAR) == 0) {
                            retorna = actualizarComisionTCC(FenixConstants.ESTADO_COMISION_DISPENSADO.longValue(),codigoComison,FenixConstants.SUBESTADO_COMISION_VALIDADO.longValue());
                        }
                        ...
                        if (estadoComision.compareTo(FenixConstants.ESTADO_COMISION_MANDATORIO) == 0) {
                            retorna = actualizarComisionTCC(FenixConstants.ESTADO_COMISION_VALIDADO.longValue(),codigoComison, null);
                        }
                    }
                } else {
                    if (validar) {
    ...
    public Boolean cambiarEstadoAplicar() {
        Boolean retorna = Boolean.TRUE;
        DCIteratorBinding voComisionesValidadas = ADFUtils.getDCBindingContainer()
                                                          .findIteratorBinding("CargoVOIterator");

        ViewObject eq = voComisionesValidadas.getViewObject();
        Row[] comisionValidadoFalso = eq.getAllRowsInRange();
        LOGGER.warning("Codigo esta actualizado su Estado: " +
            comisionValidadoFalso.length);
        ...
        if (comisionValidadoFalso.length > 0) {
            for (Row row : comisionValidadoFalso) {
                if (retorna) {
                    ...
                    Integer estadoComision = 0;
                    codigoComison = (Long) row.getAttribute("Id");
                    estadoComision = (Integer) row.getAttribute(
                            "IdTcaEstadoTcc");
                    LOGGER.warning("Codigo: " + codigoComison);
                    LOGGER.warning("Codigo Estado: " + estadoComision);

                    if ((estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_DISPENSADO) == 0) ||
                            (estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_VALIDADO) == 0) ||
                            (estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_PORPAGAR) == 0) ||
                            (estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_PAGADO) == 0) ||
                            (estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_PORDISPENSAR) == 0) ||
                            (estadoComision.compareTo(
                                FenixConstants.ESTADO_COMISION_POR_APLICAR) == 0)) {
                        LOGGER.warning("Codigo esta actualizado su Estado: " +
                            codigoComison);
                    } else {
                        if (estadoComision.compareTo(
                                    FenixConstants.ESTADO_COMISION_SUGERIDO) == 0) {
                            retorna = actualizarComisionTCC(FenixConstants.ESTADO_COMISION_POR_APLICAR.longValue(),
                                    codigoComison, null);
                        }

                        if (estadoComision.compareTo(
                                    FenixConstants.ESTADO_COMISION_MANDATORIO) == 0) {
                            retorna = actualizarComisionTCC(FenixConstants.ESTADO_COMISION_POR_APLICAR.longValue(),
                                    codigoComison, null);
                        }
                    }
    ...
    public Boolean validarComisionesMandatoriasSugeridas() {
        ...
        DCIteratorBinding voComisionesValidadas = ADFUtils.getDCBindingContainer()
                                                          .findIteratorBinding("CargoVOIterator");
        ViewObject eq = voComisionesValidadas.getViewObject();
        Row[] comisionValidadoFalso = eq.getFilteredRows("IdTcaSubEstadoTcc",
                null);
        LOGGER.warning("Codigo esta actualizado su Estado: " +
            comisionValidadoFalso.length);
    ...
    public Boolean validaTramite() {
        ...
        PreparacionBean preparacion = (PreparacionBean) JSFUtils.resolveExpression(
                "#{pageFlowScope.preparacionBean}");
        DCIteratorBinding voComisiones = ADFUtils.getDCBindingContainer()
                                                 .findIteratorBinding("CargoVOIterator");
    ...
    public Boolean guardarRAROC() {
        ...
        try {
            ...
            operationBinding.getParamsMap()
                            .put("idOperacion", preparacion.getIdOperacion());
            ...
            LOGGER.log(ADFLogger.ERROR,
                "Error en guardar RAROC " + e.getClass() + ":" +
                e.getMessage());
    ...
    public String obtenerNombreDocumento() {
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_HOJA_TERMINOS_SEPRI)) {
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT)) {
            return "CONFIRMA_DOCUMENTO_PCT";
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_EMITIR_OPINION_TECNICA)) {
            return "CONFIRMA_DOCUMENTO_OPINION";
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES)) {
            return "CONFIRMA_DOCUMENTO_ADQUISICION";
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_VALIDAR_MATRIZ_ACCIONES)) {
            return "CONFIRMA_DOCUMENTO_VALIDAR";
        }
    ...
    public Boolean validarDocumento() {
        ...
        if (preparacion.getCodigoTarea()
            ...
            if (documentoValidado(
                        FenixConstants.DOCUMENTO_EMITIR_OPINION_TECNICA,
                        Long.parseLong(preparacion.getIdOperacion())) ||
                    documentoValidado(
                        FenixConstants.DOCUMENTO_EMITIR_OPINION_TECNICA2,
                        Long.parseLong(preparacion.getIdOperacion()))) {
                return true;
            } else {
                return false;
            ...
            return documentoValidado(tipoDocumento,
                Long.parseLong(preparacion.getIdOperacion()));
        }
    }
    ...
    public Integer obtenerTipoDocumento() {
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_VALIDAR_MATRIZ_ACCIONES)) {
            return FenixConstants.DOCUMENTO_VALIDAR_MATRIZ;
        }
        ...
        try {
            while (iq.hasNext()) {
                ...
                if (retorno) {
                    ...
                    estadoComision = (Integer) row.getAttribute(
                            "IdTcaEstadoTcc");
                    subEstadoComision = (Integer) row.getAttribute(
                            "IdTcaSubEstadoTcc");
                    LOGGER.warning("SubEstado comision: " + subEstadoComision);

                    if ((subEstadoComision.compareTo(
                                FenixConstants.SUBESTADO_COMISION_VALIDADO) == 0) ||
                            (subEstadoComision.compareTo(
                                FenixConstants.SUBESTADO_COMISION_EDITADO_MANDATORIO) == 0)) {
                        if ((estadoComision.compareTo(
                                    FenixConstants.ESTADO_COMISION_DISPENSADO) == 0) ||
                                (estadoComision.compareTo(
                                    FenixConstants.ESTADO_COMISION_PAGADO) == 0) ||
                            ...
                            if ((estadoComision.compareTo(
                                    ...
                                    (estadoComision.compareTo(
                                        FenixConstants.ESTADO_COMISION_PORPAGAR) == 0)) {
                                LOGGER.warning("Codigo comision: " +
                                    codigoComison +
                                ...
                                OperationBinding operationBinding = bindings.getOperationBinding(
                                        "servicioInicioComisiones");
                                LOGGER.warning("Codigo comision: " + idComi);
                                operationBinding.getParamsMap()
                                                .put("idComision", idComi);
                            ...
                            retorno = Boolean.FALSE;
                            JSFUtils.addFacesErrorMessage(getStringFromBundle(
                                    "MSG_ERROR_ESTADO_COMISION"));
                        }
                    } else {
                        if (subEstadoComision.compareTo(
                                    FenixConstants.SUBESTADO_COMISION_PORVALIDAR) == 0) {
                            if ((estadoComision.compareTo(
                                        FenixConstants.ESTADO_COMISION_PORPAGAR) == 0) ||
                                    (estadoComision.compareTo(
                                        FenixConstants.ESTADO_COMISION_DISPENSADO) == 0) ||
            ...
            LOGGER.log(ADFLogger.ERROR,
                "Error en inicioComisiones " + e.getClass() + ":" +
                e.getMessage());
            retorno = Boolean.FALSE;
        }
    ...
    public Boolean regresarEstadoComision() {
        ...
        RowSetIterator iq = voComisiones.getViewObject()
                    ...
                    if ((subEstadoComision.compareTo(
                        ...
                        if ((estadoComision.compareTo(
                            ...
                            if ((estadoComision.compareTo(
                                    ...
                                    (estadoComision.compareTo(
                                        FenixConstants.ESTADO_COMISION_MANDATORIO) == 0)) {
                                LOGGER.warning("No actualiza su Estado: " +
                                    codigoComison);
                            }
                        } else {
                            if (estadoComision.compareTo(
                                ...
                                retorno = Boolean.FALSE;
                                JSFUtils.addFacesErrorMessage(
                                    "Estado comision incorrecto para poder solicitar mas informacion");
                            }
                        }
                    }
                    else {
    ...
    public Boolean servicioComisiones() {
        ...
        try {
            for (Row row : comisionesActuales) {
                ...
                if (((contratoCodigo != null) &&
                    ...
                    if ((contratoCodigo != null) &&
                            (contratoCodigo.trim().length() > 0)) {
                        LOGGER.warning("Codigo comision: " + codigoComison +
                            " " + descripcion + " ya se encuentra en FLEXCUBE");
                    }
                    ...
                    LOGGER.warning("Codigo comision: " + codigoComison + " " +
                        descripcion + " se intenta mandar a Flexcube");
            ...
            LOGGER.log(ADFLogger.WARNING,
                "Error en crearComision " + e.getClass() + ":" +
                e.getMessage());
            retorno = Boolean.FALSE;
        }
    ...
    public Boolean guardaEquipos() {
        PreparacionBean preparacion = (PreparacionBean) JSFUtils.resolveExpression(
                "#{pageFlowScope.preparacionBean}");
        DCIteratorBinding voEquipoConformado = ADFUtils.getDCBindingContainer().findIteratorBinding("ConformarEquiposVOIterator");
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_ENVIAR_OPERACION_ANALISIS)) {
            Row[] equipoRowsActivadosValidaDocumento = eq.getFilteredRows("Documento",
                    true);
    ...
    public void equiposPayload(String tarea, Boolean participan) {
        ...
        if (tarea.equalsIgnoreCase(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "requiereAnalisisAdquisiciones");
            attributeBinding.setInputValue(participan);
            LOGGER.warning("Adquisiciones: " +
        ...
        if (tarea.equalsIgnoreCase(
                    FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "requiereElaborarDocumentoTecnicoOPD");
            attributeBinding.setInputValue(participan);
            LOGGER.warning("Elaborar Documento Tecnico OPD: " +
                attributeBinding.getInputValue());
        }

        if (tarea.equalsIgnoreCase(
                    FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "requiereGenerarPlanMonitoreoOPD");
            attributeBinding.setInputValue(participan);
            LOGGER.warning("Generar Plan Monitoreo y Evaluacion OPD: " +
                attributeBinding.getInputValue());
        }

        if (tarea.equalsIgnoreCase(
                    FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "requiereRevisarEvidenciaCumplimientoOPD");
            attributeBinding.setInputValue(participan);
            LOGGER.warning("Revisar Evidencia Cumplimiento OPD: " +
                attributeBinding.getInputValue());
        }
    }
        ...
        if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_REVISAR_COMISIONES)) {
            ...
            if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC)) {
                ...
                if (preparacion.getCodigoTarea().equalsIgnoreCase(FenixConstants.TAREA_CONFORMAR_EQUIPO_DE_PREPARACION) ||
                    ...
                    if ((preparacion.getContadorComisiones() > 0) && (preparacion.getContadorComisionesActuales() > preparacion.getContadorComisiones())) {
    ...
    private Long getIdOperacion() {
        PreparacionBean preparacionBean = (PreparacionBean) JSFUtils.resolveExpression(
                "#{pageFlowScope.preparacionBean}");

        if ((null != preparacionBean.getIdOperacion()) &&
                (preparacionBean.getIdOperacion().trim().length() > 0)) {
    ...
    private Integer getCodigoTarea() {
        ...
        if ((null != preparacionBean.getCodigoTarea()) &&
                (preparacionBean.getCodigoTarea().trim().length() > 0)) {
    ...
    public String invokeMasInformacionAction() {
        ...
        validaCometnario = validateComentario(getIdOperacion(),
                getCodigoTarea().toString(), getInstanciaTarea());
        ...
        if (validaCometnario) {
            ...
            if (preparacion.getCodigoTarea()
                               .equalsIgnoreCase(FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_REVISAR_COMISIONES) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES) ||
                    ...
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_ENVIAR_OPERACION_ANALISIS) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD) ||
                    preparacion.getCodigoTarea()
                                   .equalsIgnoreCase(FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD)) {
                JSFUtils.addFacesErrorMessage(getStringFromBundle(
                        "CONFIRMA_COMENTARIO"));
            } else {
                LOGGER.warning(
                    "Inicia popup de confirmacion de mas informacion preparacion");
                masInformacionPopup();
            }
        }
    ...
    public void guardarSolicitar(String tareaCodigo, Boolean requerido) {
        ...
        if (tareaCodigo.equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoDeterminarRAROC");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("RAROC: " + attributeBinding.getInputValue());
        }

        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoHojaTerminosPCT");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("PCT: " + attributeBinding.getInputValue());
        }

        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_EMITIR_OPINION_TECNICA)) {
            LOGGER.warning("Activa la bandera de emitir opinion tecnica");
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoOpinionTecnica");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("Opinion solicita mas informacion?  " +
                attributeBinding.getInputValue());
        }

        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_ANALIZAR_ADQUISICIONES)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoAnalisisAdquisiciones");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("Adquisiciones: " +
                attributeBinding.getInputValue());
        }
        ...
        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoElaborarDocumentoTecnicoOPD");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("Elaborar Documento Tecnico OPD: " +
                attributeBinding.getInputValue());
        }
        ...
        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoGenerarPlanMonitoreoOPD");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("Generar Plan Monitoreo Evaluacion OPD: " +
                attributeBinding.getInputValue());
        }
        ...
        if (tareaCodigo.equalsIgnoreCase(
                    FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD)) {
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoRevisarEvidenciaCumplimientoOPD");
            attributeBinding.setInputValue(requerido);
            LOGGER.warning("Revisar Evidencia Cumplimiento OPD: " +
                attributeBinding.getInputValue());
        }
    ...
    public void guadarMasInformacion() {
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_DETERMINAR_RAROC)) {
            guardarSolicitar(FenixConstants.TAREA_DETERMINAR_RAROC,
                solicitaValidado);
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT)) {
            guardarSolicitar(FenixConstants.TAREA_HOJA_DE_TERMINOS_PCT,
                solicitaValidado);
        }
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_EMITIR_OPINION_TECNICA)) {
            LOGGER.warning(
                "Inicia activar banderas de mas informacion en opinion tecnica");
            guardarSolicitar(FenixConstants.TAREA_EMITIR_OPINION_TECNICA,
                solicitaValidado);
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES)) {
            guardarSolicitar(FenixConstants.TAREA_ANALIZAR_ADQUISICIONES,
                solicitaValidado);
        }
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD)) {
            guardarSolicitar(FenixConstants.TAREA_ELABORAR_DOCUMENTO_TECNICO_OPD,
                solicitaValidado);
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD)) {
            guardarSolicitar(FenixConstants.TAREA_GENERAR_PLAN_MONITOREO_EVALUACION_OPD,
                solicitaValidado);
        }

        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD)) {
            guardarSolicitar(FenixConstants.TAREA_REVISAR_EVIDENCIA_CUMPLIMIENTO_OPD,
                solicitaValidado);
        }
        ...
        if (preparacion.getCodigoTarea()
            ...
            AttributeBinding attributeBinding;
            attributeBinding = ADFUtils.findControlBinding(
                    "solicitaMasInfoEnviarOperAnalisis");
            attributeBinding.setInputValue(Boolean.TRUE);
            LOGGER.warning("Revisar comisiones: " +
                attributeBinding.getInputValue());

            DCIteratorBinding voEquipoConformado = ADFUtils.getDCBindingContainer()
                                                           .findIteratorBinding("ConformarEquiposVOIterator");
            ViewObject eq = voEquipoConformado.getViewObject();
            Row[] equipoRowsActivados = eq.getAllRowsInRange();
    ...
    public String invokeMasInformacion() {
        ...
        LOGGER.warning(
            "Inicia metodo de confirmar mas informacion en preparacion");
        ...
        if (siGuarda) {
            ...
            LOGGER.warning(
                "Inicia el invoke action de solicitar mas informacion");

            //IF OK
            InvokeActionBean invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue(
                    "invokeActionBean");
    ...
    private Boolean comisionValida() {
        ...
        if ((FenixConstants.ESTADO_COMISION_DISPENSADO.compareTo(
                    preparacion.getEstadoComisionValidar()) == 0) ||
                (FenixConstants.ESTADO_COMISION_PORDISPENSAR.compareTo(
                    preparacion.getEstadoComisionValidar()) == 0)) {
            if ((null != preparacion.getObservaciones()) &&
                    (preparacion.getObservaciones().length() > 0)) {
        ...
        if (tasa) {
            if ((null == valorPorcentaje) ||
                    ((valorPorcentaje.doubleValue() <= 0) ||
                    (valorPorcentaje.doubleValue() >= 100))) {
                preparacion.setErrorPorcentaje(Boolean.TRUE);
                valida = false;
            }

            if ((null == idMontoBase) ||
                    ((null == idMontoBase.getInputValue()) ||
                    idMontoBase.getInputValue().toString().equals(""))) {
        ...
        if ((null == idTipoMoneda) ||
                ((null == idTipoMoneda.getInputValue()) ||
                idTipoMoneda.getInputValue().toString().equals(""))) {
            preparacion.setErrorTipomoneda(Boolean.TRUE);
            valida = false;
        }

        if ((null == tipoFrecuencia) ||
                ((null == tipoFrecuencia.getInputValue()) ||
                tipoFrecuencia.getInputValue().toString().equals(""))) {
            preparacion.setErrorTipoFrecuncia(Boolean.TRUE);
            valida = false;
        } else {
            if (!TIPO_VENCIMIENTO.equals(Integer.valueOf(
                            tipoFrecuencia.getInputValue().toString())) &&
                    ((null == frecuenciaPago) ||
                    (null == frecuenciaPago.getInputValue()) ||
                    (Integer.valueOf(frecuenciaPago.getInputValue().toString()) == 0))) {
                preparacion.setErrorFrecuenciaPago(Boolean.TRUE);
                valida = false;
            } else {
                if (TIPO_VENCIMIENTO.compareTo(Integer.valueOf(
                                tipoFrecuencia.getInputValue().toString())) == 0) {
        ...
        if ((null == montoComision) || (null == montoComision.getInputValue()) ||
                (Double.valueOf(montoComision.getInputValue().toString()) <= 0) ||
                montoComision.getInputValue().toString().equals("")) {
    ...
    public void guardarComision(ActionEvent actionEvent) {
        if (comisionValida()) {
            ...
            fecjaInicioComision = ADFUtils.findControlBinding(
                    "FechaInicioComision");

            AttributeBinding fechaInicioCapital;
            fechaInicioCapital = ADFUtils.findControlBinding(
                    "FechaInicioCapital");
            ...
            DCBindingContainer dcBindings = (DCBindingContainer) BindingContext.getCurrent()
                                                                               .getCurrentBindingsEntry();
            DCIteratorBinding cargosIterator = dcBindings.findIteratorBinding(
                    "CargoVOIterator");
    ...
    public String eliminarComision() {
        ...
        DCBindingContainer dcBindings = (DCBindingContainer) BindingContext.getCurrent()
                                                                           .getCurrentBindingsEntry();
        DCIteratorBinding comentariosIterator = dcBindings.findIteratorBinding(
                "CargoVOIterator");
        comentariosIterator.executeQuery();
        popupEliminar.hide();
        JSFUtils.addFacesInformationMessage(getStringFromBundle(
                "MSG_COMISION_ELIMINADA"));
    ...
    public String guardarDispensar() {
        ...
        if (preparacion.getObservaciones() instanceof String &&
                (preparacion.getObservaciones().length() > 0)) {
    ...
    public void guardarCommit() {
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_CONFORMAR_EQUIPO_DE_PREPARACION)) {
            tablaEquipos();
        }
    }
    ...
    public void cambioPorcentaje(ValueChangeEvent vce) {
        ...
        porcentajeMontoBase = ADFUtils.findControlBinding(
    ...
    public void cambioValorTasa(ValueChangeEvent vce) {
        ...
        if ((null != vce) && (null != vce.getNewValue())) {
            ...
            AttributeBinding montoComision = ADFUtils.findControlBinding(
                    "MontoComision");

            if ((nuevoValor.doubleValue() <= 0) ||
                    (nuevoValor.doubleValue() > 100)) {
                JSFUtils.addFacesErrorMessage(getStringFromBundle(
                        "MSG_PORCENTAJE"));
            ...
            if ((preparacion.getPorcentajeBase().doubleValue() > 0) &&
                ...
                if ((null != montoBase) && (null != montoBase.getInputValue())) {
                    ...
                    if (monto.doubleValue() > 0.00) {
                        LOGGER.warning("porcentaje:" +
                            preparacion.getPorcentajeBase());
    ...
    public void cambioMontoBase(ValueChangeEvent vce) {
        ...
        case 1:
            ...
            if ((valorMax <= 0) && (valorMin >= 0)) {
                ...
                LOGGER.warning(
                    "despues de calcular comision cuando se elige el monto base");
                rowComisionActiva();
            } else {
        ...
        default:
            ...
            montoComisionPay.setInputValue(calculaMontoComision());
            LOGGER.warning(
                "despues de calcular monto comision despues de elegir una opcion incorrecta en el monto base");
            rowComisionActiva();
    ...
    public void detalleCargo(ActionEvent actionEvent) {
        ...
        LOGGER.warning("PorcentajeSobreMontoBase " +
            (BigDecimal) porcentajeBase.getInputValue());
    ...
    public void modificarCargo(ActionEvent actionEvent) {
        ...
        if (preparacion.getCodigoTarea()
                           .equalsIgnoreCase(FenixConstants.TAREA_RECOPILAR_INFORMACION)) {
        ...
        if ((((contratoCodigo != null) && (contratoCodigo.trim().length() > 0)) ||
            ...
            if (estadoActual.compareTo(
                ...
                AttributeBinding fechaValor = ADFUtils.findControlBinding(
                        "FechaValor");
                fechaValor.setInputValue(preparacion.getFechaValorFlexcube());

                AttributeBinding comisionCompartida = ADFUtils.findControlBinding(
                        "ComisionCompartida");
            ...
            AttributeBinding porcentajeBase;
            porcentajeBase = ADFUtils.findControlBinding(
                    "PorcentajeSobreMontoBase");
            LOGGER.warning("PorcentajeSobreMontoBase " +
                (BigDecimal) porcentajeBase.getInputValue());
            ...
            OperationBinding opt = binding.getOperationBinding(
    ...
    public void eliminarCargo(ActionEvent actionEvent) {
        ...
        if (((null != contratoCodigo) && (contratoCodigo.trim().length() > 0)) ||
            ...
            if (preparacion.getEliminar() != 1) {
                AttributeBinding attributeBinding;
                attributeBinding = ADFUtils.findControlBinding("BanEstatus");
                LOGGER.warning("Estatus: " +
                    (Integer) attributeBinding.getInputValue());
    ...
    public void dispensarCargo(ActionEvent actionEvent) {
        ...
        if ((((contratoCodigo != null) && (contratoCodigo.trim().length() > 0)) ||
            ...
            if (FenixConstants.ESTADO_COMISION_PORDISPENSAR.compareTo(
                        preparacion.getEstadoComisionValidar()) != 0) {
                AttributeBinding subEstadoComision;
                subEstadoComision = ADFUtils.findControlBinding(
                        "IdTcaSubEstadoTcc");
    ...
    public Boolean validarComisiones() {
        BindingContainer bindings = getBindings();
        OperationBinding operationBinding = bindings.getOperationBinding(
                "validarComisionesDispensadasPagadas");

        operationBinding.getParamsMap()
                        .put("estadoPagado",
            FenixConstants.ESTADO_COMISION_PAGADO);
        operationBinding.getParamsMap()
                        .put("estadoDispensado",
            FenixConstants.ESTADO_COMISION_DISPENSADO);
    ...
    public Boolean actualizarComisionTCC(Long estado, Long claveComision,
        Long subEstado) {
        ...
        try {
            ...
            operationBinding.getParamsMap()
                            .put("tipoTCC", FenixConstants.COMISION);
            ...
            LOGGER.log(ADFLogger.ERROR,
                "Error en actualizarEstadoComision " + e.getClass() + ":" +
                e.getMessage());
        }
    ...
    public void fondoPorDefecto() {
        ...
        DCIteratorBinding cargoVOIteratorDC = ADFUtils.findIterator(
                "CargoVOIterator");
        ...
        if (null != currentRow) {
            if (null != currentRow.getAttribute("IdTcaComision")) {
                idTcaComision = (Integer) currentRow.getAttribute(
                        "IdTcaComision");
            } else {
            ...
            if (null != currentRow.getAttribute("IdTcaEstadoTcc")) {
                idTcaEstadoTcc = (Integer) currentRow.getAttribute(
                        "IdTcaEstadoTcc");
            } else {
            ...
            if (null != currentRow.getAttribute("IdTcaSubEstadoTcc")) {
                idTcaSubEstadoTcc = (Integer) currentRow.getAttribute(
                        "IdTcaSubEstadoTcc");
            } else {
            ...
            if ((null != idTcaComision) && (null != idTcaEstadoTcc)) {
                ...
                if (idTcaComision.equals(new Integer("9")) &&
                        idTcaEstadoTcc.equals(new Integer("8")) &&
                ...
                LOGGER.warning(
                    "no se puede realizar la validacion para fondo por defecto");
            }
        } else {
