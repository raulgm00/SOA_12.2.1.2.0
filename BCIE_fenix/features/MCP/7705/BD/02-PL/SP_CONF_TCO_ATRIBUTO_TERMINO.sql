-- ##--------------------------------------------------------------------------------------------##
-- Descripción: Script que permite agregar, modificar, desactivar/activar la configuración de los atributos del  término, así mismo, permite listar la información de dicha tabla.
/*
...
6 - Listar los términos de acuerdo al tipo de término (ID_TCA_TIPO_TERMINO)
...
CREATE OR REPLACE PROCEDURE SP_CONF_TCO_ATRIBUTO_TERMINO (
	...
	P_COLUMNA          IN VARCHAR2 := NULL,		-- Nombre de la columna, de la tabla de término (TERMINO)
	...
	P_ID_TCA_TERMINO   IN NUMBER   := NULL,		-- Id. del catálogo de término (TCA_TERMINO)
	-- P_ID_TCA_CONDICION IN NUMBER   := NULL,
	-- P_ID_TCA_COMISION  IN NUMBER   := NULL,
	P_TIPO_VALOR       IN VARCHAR2 := NULL,		-- Id. del catálogo de término (TCA_TERMINO)
	P_ID_TCO_ATRIBUTO_TCC IN NUMBER   := NULL,	-- Id. del  registro de la entidad (TCO_ATRIBUTO_TCC)
	P_ID_TCA_TIPO_TERMINO IN NUMBER   := NULL,	-- Id. del registro del tipo de término. (TCA_TERMINO)
...
BEGIN
	...
	THEN
		...
		IF  (P_ID_TCA_TERMINO > 0) THEN
			...
			END IF;			            
		ELSE
			v_Error_msg := 'No se ha indicado el Id. del registro del catálogo de Término (TCA_TERMINO).';
		END IF;
