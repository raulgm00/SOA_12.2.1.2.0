...
declare function local:funcConsultaatributosusuarioto_to_enviarcorreoelectronico($outInvoke_ConsultarAtributosUsuarioTO.response as element() (:: schema-element(usu:ConsultarAtributosUsuarioResponse) ::)) as element() (:: schema-element(cor:EnviarCorreoElectronicoBPELRequest) ::) {
    <cor:EnviarCorreoElectronicoBPELRequest>
        <cor:correoElectronico>
          <cor1:to>
          {
          for $mail in distinct-values($outInvoke_ConsultarAtributosUsuarioTO.response/usu:listaUsuarios/usu1:usuario/usu1:mail)
          return
            if ($mail != "")
            then
...
local:funcConsultaatributosusuarioto_to_enviarcorreoelectronico($outInvoke_ConsultarAtributosUsuarioTO.response)
