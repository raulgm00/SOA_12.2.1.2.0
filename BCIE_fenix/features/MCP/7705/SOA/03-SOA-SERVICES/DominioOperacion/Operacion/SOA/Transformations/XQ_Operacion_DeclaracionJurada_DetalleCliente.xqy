...
declare variable $outConsultarDeclaracionJuradaByIdOperacion.response as element() (:: schema-element(dec:ConsultarDeclaracionJuradaResponse) ::) external;

declare function local:funcXq_operacion_declaracionjurada_detallecliente($InvokeConsultarOperacion_ConsultarOperacionById_OutputVariable.response as element() (:: schema-element(ope:ConsultarOperacionResponse) ::), 
                                                                         $outConsultarClienteById.response as element() (:: schema-element(cli:ConsultarClienteResponse) ::), 
                                                                         $outConsultarDeclaracionJuradaByIdOperacion.response as element() (:: schema-element(dec:ConsultarDeclaracionJuradaResponse) ::)) 
    ...
    <ope:ConsultarOperacionResponse>
        <ope:Operacion>
            ...
            {
                if ($InvokeConsultarOperacion_ConsultarOperacionById_OutputVariable.response/ope:Operacion/ope1:oficina)
                then <ope1:oficina>{fn:data($InvokeConsultarOperacion_ConsultarOperacionById_OutputVariable.response/ope:Operacion/ope1:oficina)}</ope1:oficina>
                else ()
            }
            {
            ...
            <ope1:cliente>
                ...
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:idFacturador)
                    then <cli1:idFacturador>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:idFacturador)}</cli1:idFacturador>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:razonSocial)
                    then <cli1:razonSocial>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:razonSocial)}</cli1:razonSocial>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:abreviatura)
                    then <cli1:abreviatura>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:abreviatura)}</cli1:abreviatura>
                    else ()
                }
                <cli1:tipoPersona>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoPersona/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:tipoPersona>
                <cli1:tipoCliente>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoCliente/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:tipoCliente>
                <cli1:sector>
                ...
                <cli1:tipoInstitucion>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoInstitucion/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:tipoInstitucion>
                <cli1:pais>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:pais/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:pais>
                <cli1:grupoEconomico>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEconomico/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:grupoEconomico>
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:tipoIdentificacion)
                    then <cli1:tipoIdentificacion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:tipoIdentificacion)}</cli1:tipoIdentificacion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:numeroIdentificacion)
                    then <cli1:numeroIdentificacion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:numeroIdentificacion)}</cli1:numeroIdentificacion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:direccion)
                    then <cli1:direccion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:direccion)}</cli1:direccion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:telefono)
                    then <cli1:telefono>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:telefono)}</cli1:telefono>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:fax)
                    then <cli1:fax>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:fax)}</cli1:fax>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:email)
                    then <cli1:email>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:email)}</cli1:email>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:ciudad)
                    then <cli1:ciudad>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:ciudad)}</cli1:ciudad>
                    else ()
                }
                <cli1:oficina>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:oficina/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:oficina>
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:grupoEmpresarial)
                    then <cli1:grupoEmpresarial>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:grupoEmpresarial)}</cli1:grupoEmpresarial>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:fechaRegistro)
                    then <cli1:fechaRegistro>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:fechaRegistro)}</cli1:fechaRegistro>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:fechaAprobacion)
                    then <cli1:fechaAprobacion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:fechaAprobacion)}</cli1:fechaAprobacion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:ejecutivo)
                    then <cli1:ejecutivo>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:ejecutivo)}</cli1:ejecutivo>
                    else ()
                }
                {for $contactos in $outConsultarClienteById.response/cli:Cliente/cli1:Contactos/cli1:Contacto
                return
                  if(fn:string-length($contactos/cli1:idContacto/text()) > 0)then
                <cli1:Contactos>
                    <cli1:Contacto>
                        <cli1:idContacto>{fn:data($contactos/cli1:idContacto)}</cli1:idContacto>
                        <cli1:idCliente>{fn:data($contactos/cli1:idCliente)}</cli1:idCliente>
                        <cli1:nombre>{fn:data($contactos/cli1:nombre)}</cli1:nombre>
                        <cli1:telefono>{fn:data($contactos/cli1:telefono)}</cli1:telefono>
                        <cli1:correoElectronico>{fn:data($contactos/cli1:correoElectronico)}</cli1:correoElectronico>
                        <cli1:cargo>{fn:data($contactos/cli1:cargo)}</cli1:cargo>
                        <cli1:tipo>{fn:data($contactos/cli1:tipo)}</cli1:tipo>
                        <cli1:fechaRegistro>{fn:data($contactos/cli1:fechaRegistro)}</cli1:fechaRegistro>
                        <cli1:idContactosClientes>{fn:data($contactos/cli1:idContactosClientes)}</cli1:idContactosClientes>
                    </cli1:Contacto>
                </cli1:Contactos>
                else()
                }
                <cli1:InformacionCorrecta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:InformacionCorrecta)}</cli1:InformacionCorrecta>
                <cli1:ActualizacionPermitida>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:ActualizacionPermitida)}</cli1:ActualizacionPermitida>
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:comentarioAprobacion)
                    then <cli1:comentarioAprobacion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:comentarioAprobacion)}</cli1:comentarioAprobacion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:revisadoAprobacion)
                    then <cli1:revisadoAprobacion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:revisadoAprobacion)}</cli1:revisadoAprobacion>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:status)
                    then <cli1:status>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:status)}</cli1:status>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:fechaBaja)
                    then <cli1:fechaBaja>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:fechaBaja)}</cli1:fechaBaja>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:diaPago)
                    then <cli1:diaPago>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:diaPago)}</cli1:diaPago>
                    else ()
                }
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR)
                    then 
                        <cli1:SCR>
                            {
                                if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:Id)
                                then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:Id)}</cat:Id>
                                else ()
                            }
                            {
                                if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:Descripcion)
                                then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:Descripcion)}</cat:Descripcion>
                                else ()
                            }
                            {
                                if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:DescripcionCorta)
                                then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:DescripcionCorta)}</cat:DescripcionCorta>
                                else ()
                            }
                            {
                                if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:estatus)
                                then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:estatus)}</cat:estatus>
                                else ()
                            }
                            {
                                if ($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:codigoExterno)
                                then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:SCR/cat:codigoExterno)}</cat:codigoExterno>
                                else ()
                            }
                        </cli1:SCR>
                    else ()
                ...
                <cli1:perspectiva>
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:Id)
                        then <cat:Id>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:Id)}</cat:Id>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:Descripcion)
                        then <cat:Descripcion>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:Descripcion)}</cat:Descripcion>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:DescripcionCorta)
                        then <cat:DescripcionCorta>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:DescripcionCorta)}</cat:DescripcionCorta>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:estatus)
                        then <cat:estatus>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:estatus)}</cat:estatus>
                        else ()
                    }
                    {
                        if ($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:codigoExterno)
                        then <cat:codigoExterno>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:perspectiva/cat:codigoExterno)}</cat:codigoExterno>
                        else ()
                    }
                </cli1:perspectiva>
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:enMora)
                    then <cli1:enMora>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:enMora)}</cli1:enMora>
                    else ()
                }
                <cli1:mora>
                    <cli1:dias>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:mora/cli1:dias)}</cli1:dias>
                    <cli1:monto>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:mora/cli1:monto)}</cli1:monto>
                </cli1:mora>
                {
                    if ($outConsultarClienteById.response/cli:Cliente/cli1:deteriorado)
                    then <cli1:deteriorado>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:deteriorado)}</cli1:deteriorado>
                    else ()
                }
                <cli1:reserva>
                    <cli1:importeReserva>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:reserva/cli1:importeReserva)}</cli1:importeReserva>
                    <cli1:tipoReserva>{fn:data($outConsultarClienteById.response/cli:Cliente/cli1:reserva/cli1:tipoReserva)}</cli1:tipoReserva>
                </cli1:reserva>
            </ope1:cliente>
            <ope1:producto>
            ...
            <ope1:declaracionJurada>
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:idDeclaracion)
                    then <dec1:idDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:idDeclaracion)}</dec1:idDeclaracion>
                    else ()
                }
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:codigoExternoDeclaracion)
                    then <dec1:codigoExternoDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:codigoExternoDeclaracion)}</dec1:codigoExternoDeclaracion>
                    else ()
                }
                <dec1:estadoDeclaracion>
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:codigoEstadoDeclaracion)
                        then <dec1:codigoEstadoDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:codigoEstadoDeclaracion)}</dec1:codigoEstadoDeclaracion>
                        else ()
                    }
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:nombreEstadoDeclaracion)
                        then <dec1:nombreEstadoDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:nombreEstadoDeclaracion)}</dec1:nombreEstadoDeclaracion>
                        else ()
                    }
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:EstadoNoObjecion)
                        then <dec1:EstadoNoObjecion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:EstadoNoObjecion)}</dec1:EstadoNoObjecion>
                        else ()
                    }
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:nombreEstadoNoObjecion)
                        then <dec1:nombreEstadoNoObjecion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoDeclaracion/dec1:nombreEstadoNoObjecion)}</dec1:nombreEstadoNoObjecion>
                        else ()
                    }
                </dec1:estadoDeclaracion>
                <dec1:estadoBusqueda>
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoBusqueda/dec1:codigoEstadoBusqueda)
                        then <dec1:codigoEstadoBusqueda>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoBusqueda/dec1:codigoEstadoBusqueda)}</dec1:codigoEstadoBusqueda>
                        else ()
                    }
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoBusqueda/dec1:nombreEstadoBusqueda)
                        then <dec1:nombreEstadoBusqueda>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:estadoBusqueda/dec1:nombreEstadoBusqueda)}</dec1:nombreEstadoBusqueda>
                        else ()
                    }
                </dec1:estadoBusqueda>
                <dec1:calificacionDeRiesgo>
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:calificacionDeRiesgo/dec1:codigoCalificacionDeRiesgo)
                        then <dec1:codigoCalificacionDeRiesgo>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:calificacionDeRiesgo/dec1:codigoCalificacionDeRiesgo)}</dec1:codigoCalificacionDeRiesgo>
                        else ()
                    }
                    {
                        if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:calificacionDeRiesgo/dec1:nombreCalificacionDeRiesgo)
                        then <dec1:nombreCalificacionDeRiesgo>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:calificacionDeRiesgo/dec1:nombreCalificacionDeRiesgo)}</dec1:nombreCalificacionDeRiesgo>
                        else ()
                    }
                </dec1:calificacionDeRiesgo>
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaRegistro)
                    then <dec1:fechaRegistro>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaRegistro)}</dec1:fechaRegistro>
                    else ()
                }
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaFirmaDeclaracion)
                    then <dec1:fechaFirmaDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaFirmaDeclaracion)}</dec1:fechaFirmaDeclaracion>
                    else ()
                }
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaVencimiento)
                    then <dec1:fechaVencimiento>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:fechaVencimiento)}</dec1:fechaVencimiento>
                    else ()
                }
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:comentarioDeclaracion)
                    then <dec1:comentarioDeclaracion>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:comentarioDeclaracion)}</dec1:comentarioDeclaracion>
                    else ()
                }
                {
                    if ($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:comentarioBusqueda)
                    then <dec1:comentarioBusqueda>{fn:data($outConsultarDeclaracionJuradaByIdOperacion.response/dec:DeclaracionJurada/dec1:comentarioBusqueda)}</dec1:comentarioBusqueda>
                    else ()
                }
            </ope1:declaracionJurada>
            {
            ...
            {
                if ($InvokeConsultarOperacion_ConsultarOperacionById_OutputVariable.response/ope:Operacion/ope1:idCatPais)
...
local:funcXq_operacion_declaracionjurada_detallecliente($InvokeConsultarOperacion_ConsultarOperacionById_OutputVariable.response, $outConsultarClienteById.response, $outConsultarDeclaracionJuradaByIdOperacion.response)
