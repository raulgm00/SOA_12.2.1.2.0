...
declare variable $idFlujoNegocio as xs:string external;
...
declare function local:func($ListaDocumentos as element() (:: schema-element(ns1:OutputParameters) ::),$ConsultarDocumentosRequest as element() (:: schema-element(ns2:ConsultaDocumentosRequest) ::), $idFlujoNegocio as xs:string ) as element() (:: schema-element(ns2:ConsultaDocumentosResponse) ::) {
              ...
              return
                 <doc:tipoDocumento>
                    <doc:nombreTipoDocumento>{data($tipoDocumento)}</doc:nombreTipoDocumento>
                    {
                     ...
                     and      ( string($idFlujoNegocio)=string ($documento/ns1:IDFLUJONEGOCIO ) or string ($documento/ns1:IDFLUJONEGOCIO )='')
                      return
                        <doc:documento>
                            ...
                            <doc:PuedeBorrar>{ data($documento/ns1:PUEDE_BORRAR)}</doc:PuedeBorrar>
                             <doc:usuarioAgrega>{fn:data($documento/ns1:NOMBRE_USUARIO_CREA)}</doc:usuarioAgrega>
                              ...
                              <doc:idFlujoNegocio>{fn:data($documento/ns1:IDFLUJONEGOCIO)}</doc:idFlujoNegocio>
...
local:func($ListaDocumentos, $ConsultarDocumentosRequest, $idFlujoNegocio)