...
public class SolicitudDesembolsoActionBean extends FenixActionBean implements Serializable {
    @SuppressWarnings("compatibility:-5110705207148245694")
    ...
    public void crearContratoDesembolso(ActionEvent actionEvent) {
        logger.warning("*Inf, Inicia el metodo crearContratoDesembolso ");
        ...
        if (!operBinding.getErrors().isEmpty()) {
            logger.warning("***Error, OperationBinding crearContratoDesembolso devuelve errores");
            ...
            if(operBinding.getResult() !=null){
             ...
             logger.warning("*Inf, ID del contato de desembolso recuperado->" + idContratoDesembolso);
        ...
        logger.warning("**** Termina el metodo crearContratoDesembolso ");
    }
...
//                    }
                ...
                if (montoDesembolsarOperacion == null) {                 
                    ...
                    if(null != idTipoMonedaSolicitud){
                        montoTotalSolicitudUSD = montoSolicitado;
    ...
    public void cerrarPopUpNotificacionContratoDesembolsoCreado(ActionEvent actionEvent) {
        ...
        public void iniciarCumplimientoCondiciones(ActionEvent actionEvent) {
            logger.warning("Inicia metodo iniciarCumplimientoCondiciones");
            Map<String, Object> params = new HashMap<String, Object>();
            SolicitudDesembolsosBean solicitudBean =
                (SolicitudDesembolsosBean) JSFUtils.resolveExpression("#{pageFlowScope.SolicitudDesembolsoBean}");
            ...
            if (null != solicitudBean) {
                ...
                try{
                    ...
                    logger.warning("No se pudo obtener el idSOlicitud Long");
                }
                
                params.put("idSolicitud", idSolicitud);
                params.put("idOperacion", idOperacion);                
                try {
                    logger.warning("Ejecutando OperationBinding para cambiar el estado de los contratos de la solicitud.");
                ...
                if (resultado) {
                    ...
                    params.put("idSolicitud", idSolicitud);
                    try {
                        logger.warning("Ejecutando OperationBinding para iniciar validacion asignacion de recursos de la solicitud.");
                    ...
                    if(resultado){
                        logger.warning("Se ha iniciado el proceso de validacion de asignacion de recursos y el estado de los contrados fue actualizado correctamente.");
                    }else{
                        logger.warning("No se pudo actualizar el estado de los contratos de desembolso de la solicitud.");
                    }
                    ...
                    logger.warning("No se pudo iniciar el proceso de Validacion de asignacion de recursos.");
                    JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_ERROR_INICIO_VALIDACION"));
                }
            }else{
                logger.warning("Instancia de solicitudBean es NULL");
                 JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_ERROR_INICIO_VALIDACION"));
              }
            
            
            logger.warning("*Inf, recuperando estado del contrato");
            
            Long idDesembolso = solicitudBean.getIdContratoSeleccionado();            
            Integer idTcaEstadoDesembolso = recperarEstadoContratoDesembolso(idDesembolso);
            ...
            }else if(idTcaEstadoDesembolso.compareTo(12) == 0){
                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("estadoContratoSeleccionado", "Cumplimiento de condiciones");
            }else if(idTcaEstadoDesembolso.compareTo(13) == 0){
                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("estadoContratoSeleccionado", "Preparado");
            }else{
        ...
        SolicitudDesembolsosBean solicitudBean =
            (SolicitudDesembolsosBean) JSFUtils.resolveExpression("#{pageFlowScope.SolicitudDesembolsoBean}");
        Long idOperacion =  solicitudBean.getIdOperacion();
        Long idContrato =  solicitudBean.getIdContratoSeleccionado();
        String idSolicitud = solicitudBean.getIdSolicitud().toString(); 
         
        Long idContratoDesembolso; 
        Long idSolicitudLong = null;
        Boolean resultado = Boolean.FALSE;
        String loginUsuario = ADFContext.getCurrent().getSecurityContext().getUserName();
        Boolean pasoValidacionDatosContratos = Boolean.FALSE;
        
        // jeanmorado 05/08/2021
        logger.warning("Entrando metodo AdvetenciaFechasSolicitudmayorFechaActual"); 
        AdvetenciaFechasSolicitudmayorFechaActual();
        logger.warning("Saliendo metodo AdvetenciaFechasSolicitudmayorFechaActual"); 
        ...
        if (null != solicitudBean) {  
            ...
            logger.warning("idSolicitud: " + idSolicitud);
            logger.warning("idContrato_x: " + idContrato);
            logger.warning("idContrato_x2: " + idContratoDesemboldoSeleccionado);
            ...
            if (validarMontoSolicitudMenorAMontoOperacion()) {
                logger.warning("Inf, ok paso la validacion de montos");
                params.put("idSolicitud", idSolicitudLong);
                //se valida que a todos los contratos de desembolso de la solicitud cuenten con los datos obligatorios
                try {
                    logger.warning("validando campos obligatorios de los contratos de la solicitud...");
                    pasoValidacionDatosContratos = execute(params, "validarCamopsObligatoriosContratos");

                } catch (Exception e) {
                    logger.warning("ERROR al validar camops obligatoriosContratos.", e);
                }
                ...
                if (pasoValidacionDatosContratos) {
                    try {
                        ...
                        logger.warning("ERROR al validar montos de contratos vs asignacion.", e);
                    }
                    ...
                    if (resultado) {
                        ...
                        if(null != resultado){
                            if(resultado){
                                ...
                                try {
                                    if (null != idOperacion) { 
                                        operationBinding = bindings.getOperationBinding("condicionDesembolsoPorValidar"); 
                                    ...
                                    if (null != operationBinding.getResult()) {
                                        ...
                                        logger.log(ADFLogger.WARNING, "Se han cargado datos de obtenerCondicionPorValidarSolicitud.");
                                        validar = true;
                                    } else {
                                    ...
                                    {
                                        JSFUtils.addFacesErrorMessage("No se abrira el getPopupIniciarCumplimientoCondiciones.");          
                                    }
                                } catch (Exception e) {
                                    logger.log(ADFLogger.ERROR,
                                               "Error en abrirPopupCumplimientoCondiciones." + e.getClass() + "." + e.getMessage());
                                }  
                            }else{
                                logger.warning("La validacion de Fuentes sin Id Linea Pasiva no se cumplio.");
                                JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_ERROR_FUENTES_PENDIENTES"));
                            }
                        }else{
                            logger.warning("La validacion de Fuentes sin Id linea pasiva no se realizo correctamente.");
                            JSFUtils.addFacesErrorMessage("La validacion de Fuentes sin Id linea pasiva no se realizo correctamente.");
                        }                        
                    } else {
                        logger.warning("No se pudo validar montos de contratos vs asignacion.");
                        JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_ERROR_MONTO_ASIGNACION"));
                    }

                } else { //  si no paso la  validacion de Datos de los Contratos
                    logger.warning("***Error, no todos los contratos tienen los datos obligatorios");
                    JSFUtils.addFacesErrorMessage(getStringFromBundle("CAPTURAR_DATOS_OBLIGATORIOS"));
                }

            } else { //  el monto de la solicitud es mayor al de la operacion
