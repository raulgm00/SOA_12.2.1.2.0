...
public class DatosGeneralesBean implements Serializable {
    ...
    public void cargarEstado() {
        ...
        try {
            idLineaCredito = (Long.parseLong(JSFUtils.resolveExpression("#{pageFlowScope.pIdLineaCredito}").toString()));
            logger.warning("idLineaCredito " + idLineaCredito);
        } catch (Exception ex) {
            logger.warning("idLineaCredito no obtenido");
        }
        try{
            cargarInformacion = new Boolean(JSFUtils.resolveExpression("#{pageFlowScope.pCargarInformacion}").toString());
        }catch(Exception e){
    ...
    public void cargarTipoTasa() {
            logger.warning("*Inf, Inicia metodo cargarTipoTasa ");
              ...
              Long idOperacion = (null == JSFUtils.resolveExpression("#{pageFlowScope.pIdOperacion}"))? null
                               : (Long)JSFUtils.resolveExpression("#{pageFlowScope.pIdOperacion}");
              
             Long idDesembolso = (null == JSFUtils.resolveExpression("#{pageFlowScope.pIcContratoDe}"))? null
