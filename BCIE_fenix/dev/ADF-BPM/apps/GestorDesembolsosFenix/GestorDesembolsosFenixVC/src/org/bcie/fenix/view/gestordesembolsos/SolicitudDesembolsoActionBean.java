...
public class SolicitudDesembolsoActionBean extends FenixActionBean implements Serializable {
    @SuppressWarnings("compatibility:-6604285193627391415")
...
//                    }
   ...
   /**********---------------- Accesors  -------------------------***************/
    ...
    public void iniciarCumplimientoCondiciones(ActionEvent actionEvent) {
        ...
        try{
            ...
            if(solicitudBean.getListaCondiciones().size()<=0 || solicitudBean.getSoloCondicionesCalendario()){
                ...
                try {
                    logger.warning("Ejecutando OperationBinding para cambiar el estado de los contratos de la solicitud 2. "+solicitudBean.getIdOperacion());
                    ...
                    logger.warning("ERROR al ejecutar el inicio de validacion de asignacion de recursos 2.", e);
                    resultado = Boolean.FALSE;
                }
                ...
                if (resultado) {
                    ...
                    try {
                        logger.warning("Ejecutando OperationBinding para iniciar validacion asignacion de recursos de la solicitud 2. "+solicitudBean.getIdOperacion());
                    ...
                    if(resultado){
                        logger.warning("Se ha iniciado el proceso de validacion de asignacion de recursos y el estado de los contrados fue actualizado correctamente 2. "+solicitudBean.getIdOperacion());
                    }else{
                        logger.warning("No se pudo actualizar el estado de los contratos de desembolso de la solicitud 2. "+solicitudBean.getIdOperacion());
                    }
    ...
    public void desestimarContrato(ActionEvent actionEvent){
        final SolicitudDesembolsosBean solicitudBean = (SolicitudDesembolsosBean) JSFUtils.resolveExpression("#{pageFlowScope.SolicitudDesembolsoBean}");
        logger.warning("Inicio metodo desestimarContrato: "+solicitudBean.getIdOperacion());
        try{
            final Long idContrato = solicitudBean.getIdContrato();
            final BindingContainer bindings = getBindings();
            final String loginUsuario = ADFContext.getCurrent().getSecurityContext().getUserName();
            ...
            params.put("idOperacion", solicitudBean.getIdOperacion());
            params.put("idContrato",idContrato);
            params.put("responsable",loginUsuario);
            final Boolean exito = execute(params, "iniciarCancelarInstanciaDesembolso");
            logger.warning("Invocar iniciarCancelarInstanciaDesembolso - "+exito+" - "+solicitudBean.getIdOperacion());
            if(!exito){
                logger.warning("Error al iniciarCancelarInstanciaDesembolso. "+solicitudBean.getIdOperacion());
                JSFUtils.addFacesErrorMessage("Error al iniciarCancelarInstanciaDesembolso. ");
                return;
            }
        
            final OperationBinding operationBinding =  bindings.getOperationBinding("cambiarContratoABanstatusCero");
            operationBinding.getParamsMap().put("idContrato", idContrato);
            seDesestimoContrato = (Boolean) operationBinding.execute();
            logger.warning("Luego de cambiarContratoABanstatusCero: "+seDesestimoContrato+" - "+idContrato+" - "+solicitudBean.getIdOperacion());
               
            if(seDesestimoContrato){
                logger.warning("seDesestimoContrato: "+idContrato+" - "+solicitudBean.getIdOperacion());
                final OperationBinding operationBinding2 =  bindings.getOperationBinding("comitDatosContrato");                
                seRealizoCommit = (Boolean) operationBinding2.execute();   
                logger.warning("Luego de comitDatosContrato: "+seRealizoCommit+" - "+idContrato+" - "+solicitudBean.getIdOperacion());
            }else{
                logger.warning("***Error, No se pudo cambiar el BanStatus del contrato,  : "+idContrato+" - "+solicitudBean.getIdOperacion());
            }
              
            if(seDesestimoContrato && seRealizoCommit){
                logger.warning("Entrando al if: - "+solicitudBean.getIdOperacion());
                
                logger.warning("Inf, ok se desestimo el contrato idContrato : "+idContrato+" refrescando tree.."+" - "+solicitudBean.getIdOperacion());
                solicitudBean.refrescarTree();                    
                ...
                logger.warning("*** Error, No se pudo desestimar el contrato,  : "+idContrato+" - "+solicitudBean.getIdOperacion());
            }
              
        }catch(Exception e){
            e.printStackTrace();
            JSFUtils.addFacesErrorMessage("DesestimarContrato: " + e.getMessage());
            logger.warning("***Error al Desestimar Contrato" + e.getClass() + ":" + e.getMessage());              
        } 
          
        popUpDesestimarDesembolso.hide();
        logger.warning("Fin metodo desestimarContrato "+solicitudBean.getIdOperacion());
    }
}
