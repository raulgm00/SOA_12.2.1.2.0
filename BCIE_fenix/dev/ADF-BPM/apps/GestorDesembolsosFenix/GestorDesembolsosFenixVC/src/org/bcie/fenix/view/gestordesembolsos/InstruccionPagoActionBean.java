...
public class InstruccionPagoActionBean {
    ...
    public void selccionarTransferencia(SelectionEvent selectionEvent) {
        ...
        if (null != JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.IdTransferencia}")) {
            Long idTransferencia = (Long) JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.IdTransferencia}");
            logger.warning(" Valor recuperado idTransferencia ->" + idTransferencia);
            instruccionBean.setIdTransferencia(idTransferencia);
            if (null != JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.MontoTransferencia}")) {

                String MontoTransferencia = JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.MontoTransferencia}").toString();
                instruccionBean.setMontoTransfrenciaSeleccionada(new BigDecimal(MontoTransferencia));
                logger.warning("Monto de la transferencia seleccionada ->"+instruccionBean.getMontoTransfrenciaSeleccionada());

            } else {
                logger.warning("El monto de la transferencia seleccionada es resuelta a null");
            }

            try {
                BindingContainer binding = ADFUtils.getBindingContainer();
                OperationBinding operBinding = binding.getOperationBinding("selectAndInsertRowRTransferenciasForm");
                operBinding.getParamsMap().put("idTransferencia", idTransferencia);
                operBinding.execute();

                if (!operBinding.getErrors().isEmpty()) {
                    logger.warning("OperationBinding selectAndInsertRowRTransferenciasForm devuelve errores"
                                                                                +operBinding.getErrors().toString());
                    JSFUtils.addFacesErrorMessage("Error, operationBinding devuelve errores");
                }

            } catch (Exception e) {
                logger.log(ADFLogger.ERROR, "Error en establecer transferenciao ", e);
            }

            InstruccionPagoBean instruccionesBean = (InstruccionPagoBean) JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
            ...
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorCamposTransferencia());
            AdfFacesContext.getCurrentInstance().addPartialTarget(getFormulariosInstruccionesPago());
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorAccionesAInstrucciones());

        } else {
            JSFUtils.addFacesErrorMessage("Ocurrio un error al seleccionar la transferencia");
        } 
    }
    ...
    public List onSuggest(String string) {
        ...
        JUCtrlListBinding list = (JUCtrlListBinding) bindings.get("NumeroCtaBenef2");
    ...
    public void recuperarDatosCuenta(String numCuentaBeneficiario) {
        logger.warning("*Inf, Inicia metodo recuperarDatosCuenta para noCuenta: " + numCuentaBeneficiario);
        BindingContainer binding = ADFUtils.getBindingContainer();
        OperationBinding operBinding = null;

        try {
            operBinding = binding.getOperationBinding("recuperarEstablecerInstruccionPago");
        ...
        if (!operBinding.getErrors().isEmpty()) {
            logger.warning("OperationBinding recuperarEstablecerInstruccionPago devuelve errores ->"
                                                                      +operBinding.getErrors().toString());
        } else {
            ...
            if (operBinding.getResult() != null) {
                ...
                if ((Boolean) operBinding.getResult()) {
                    logger.warning("ok instruccion de pago recuperada refrescando formulario..");
                    AdfFacesContext.getCurrentInstance().addPartialTarget(getFormulariosInstruccionesPago());
                } else {
                    logger.warning("*Inf, Importante! no se pudo recuerar la instruccion de pago");
                }

            } else {
                logger.warning("*Error, operBinding.getResult() resuelto a null");
            }
    ...
    public void recuperarDatosCuenta2(String numCuentaBeneficiario){    
         logger.warning("*Inf, Inicia metodo recuperarDatosCuenta");
            ...
            InstruccionPagoBean instruccionesBean 
                                    = (InstruccionPagoBean)JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
            ...
            try{
             ...
             operBinding.execute();
               ...
               if(!operBinding.getErrors().isEmpty()){
                   logger.warning("OperationBinding buscarRecuperarNumCuentaBnf devuelve errores ->"
                ...
                if(operBinding.getResult() != null){
                        rowInstruccionPago = (Row)operBinding.getResult();
                             ...
                             idInstruccion = (rowInstruccionPago.getAttribute("Id") == null)? null
                                           : (Long)rowInstruccionPago.getAttribute("Id");                                
                    }
        ...
        logger.warning("*Inf, es nueva Transferencia : " + instruccionesBean.getEsNuevaTransferencia());

        AdfFacesContext.getCurrentInstance().addPartialTarget(getFormulariosInstruccionesPago());
        logger.warning("*Inf, termina metodo recuperarDatosCuenta");
    }

    public void recuperarDatosCuentaSeleccionada(ValueChangeEvent valueChangeEvent) {
    ...
    public Boolean existenTransferenciaByIdContrato(Long IdContrato) {
        ...
        try {
            ...
            OperationBinding operBinding = bindings.getOperationBinding("existenTransferenciasRegistradasByIdContrato");
   ...
   */
    ...
    public void agregarTransferencia(ActionEvent actionEvent) {
        ...
        Long idDesembolso =
            (null == JSFUtils.resolveExpression("#{pageFlowScope.idContratoDesembolso}")) ? null :
            new Long(JSFUtils.resolveExpression("#{pageFlowScope.idContratoDesembolso}").toString());

        Integer idTcaTipoMoneda =
            (null == JSFUtils.resolveExpression("#{pageFlowScope.idTcaTipoMoneda}") ? null :
        ...
        BigDecimal disponibleTransferir =
            (null == instruccionesBean.getDisponibleTransferir()) ? BigDecimal.ZERO :
            instruccionesBean.getDisponibleTransferir();

        BigDecimal totalTransferir =
            (null == instruccionesBean.getTotalTransferir()) ? BigDecimal.ZERO : instruccionesBean.getTotalTransferir();
        ...
        if (idOperacion == null || idDesembolso == null) {
            JSFUtils.addFacesErrorMessage("Error al recuperar idOperacion, IdContratoDesembolso");
            ...
            try {
                operBinding = bindings.getOperationBinding("nuevoRegistroTransferencia");
                operBinding.getParamsMap().put("idContrato", idDesembolso);
                operBinding.getParamsMap().put("idOperacion", idOperacion);
                operBinding.getParamsMap().put("IdTcaTipoMoneda", idTcaTipoMoneda);
                operBinding.getParamsMap().put("montoTransferencia", montoTransferencia);
                operBinding.execute();
            } catch (Exception e) {
                logger.warning("***Error, al ejecutar el operationBinding establecerInstruccionPago : ", e);
            }
            if (!operBinding.getErrors().isEmpty()) {
                ...
                instruccionesBean.setBtnGuardarVisible(Boolean.TRUE);
                instruccionesBean.setBtnCancelarVisible(Boolean.TRUE);
                ADFUtils.setBoundAttributeValue("TipoOpcionBancoBeneficiario", "OPCION_DEFAULT");
                ADFUtils.setBoundAttributeValue("TipoOpcionBancoIntermediario", "OPCION_DEFAULT");
                AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorAccionesAInstrucciones());
                AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorCamposTransferencia());
                AdfFacesContext.getCurrentInstance().addPartialTarget(getFormulariosInstruccionesPago());
            }
    ...
    public void agregarTransferencia2(ActionEvent actionEvent) {
        ...
        InstruccionPagoBean instruccionesBean = (InstruccionPagoBean)JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
        instruccionesBean.setMontoTransfrenciaSeleccionada(BigDecimal.ZERO);
        if(instruccionesBean.getTotalDesembolso() == null || instruccionesBean.getTotalDesembolso().compareTo( BigDecimal.ZERO) == 0 ){                                
            logger.warning("*** Error monto a desembolsar del contrato ->"+instruccionesBean.getTotalDesembolso());
            JSFUtils.addFacesErrorMessage("No se cuenta con monto disponible para la transferencia");             
        }else{ 

            Boolean respuesta = null;
            Long newIdTransaccion = null;
                InstruccionPagoBean transferenciasBean = (InstruccionPagoBean)JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
            ...
            try {
                BindingContainer bindings = ADFUtils.getBindingContainer();
                OperationBinding operBinding = bindings.getOperationBinding("nuevaTransaccion");
                operBinding.getParamsMap().put("idOperacion", idOperacion);
            ...
            if (newIdTransaccion == null)
                JSFUtils.addFacesErrorMessage("Error, no se pudo crear la nueva tranferenca");
            ...
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorAccionesAInstrucciones());
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorAccionesTransaccionesPago());
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorCamposTransferencia());
            AdfFacesContext.getCurrentInstance().addPartialTarget(getFormulariosInstruccionesPago());

                logger.warning("*** Termina metodo agregarInstruccionPago  -:"+instruccionesBean.getEsNuevaTransferencia());

        }
    }

    private boolean validacionBloqueInstrucciones(
        String bloque,
        String TipoOpcion,
        String NumeroCuenta,
        ...
        logger.warning("Inicio validacionBloqueInstrucciones bloque: " + bloque);
        ...
        {  
            if (Identificador == null){ 
                msgTemp = "El identificador del " + bloque + " no puede estar vacio."; 
            ...
            else
            {
                ...
                logger.warning("longitudIdentificador.compareTo(longitudMinimaIdentificador): " + longitudIdentificador.compareTo(longitudMinimaIdentificador));
                if(longitudIdentificador.compareTo(longitudMinimaIdentificador) == -1)
                {
                    msgTemp = "El identificador del " + bloque + " no puede tener menos de " + longitudMinimaIdentificador + " carácteres actualmente tiene " + longitudIdentificador + " carácteres."; 
                    guardarErrorInstrucciones("Identificador"+bloque,msgTemp);
                    camposObligatorios = Boolean.FALSE;
                }
                
                logger.warning("longitudIdentificador.compareTo(longitudMaximaIdentificador): " + longitudIdentificador.compareTo(longitudMaximaIdentificador));
                if(longitudIdentificador.compareTo(longitudMaximaIdentificador) == 1)
                {
                    msgTemp = "El identificador del " + bloque + " no puede tener más de " + longitudMaximaIdentificador + " carácteres actualmente tiene " + longitudIdentificador + " carácteres."; 
                    guardarErrorInstrucciones("Identificador"+bloque,msgTemp);
                    camposObligatorios = Boolean.FALSE;
                }
            }
        ...
        {
            logger.warning("Nombre y Direccion " + bloque + ": " + NombreDireccion);
            if (NombreDireccion == null) {
                msgTemp = "El Nombre y Dirección del " + bloque + " no puede estar vacio.";
                ...
                for (int i = 0; i < partsDireccion.length; i++) {
                    ...
                    logger.warning("partsDireccion[" + (i + 1) + "] = " + linea);
                    int longitudLinea = linea.length();

                    if (longitudLinea > longitudLineaDireccion) {
                        msgTemp =
                            "La longitud de la línea " + (i + 1) + " para el Nombre y Dirección del " + bloque + " es de " +
                            longitudLinea + ". El limite permitido es de " + longitudLineaDireccion + ".";
                        guardarErrorInstrucciones("longitudLinea"+bloque+"("+(i + 1)+")",msgTemp);
                        camposObligatorios = Boolean.FALSE;
                    } else {
                        msgTemp = "La longitud de la línea " + (i + 1) + " es de " + longitudLinea + ".";
                        logger.warning(msgTemp);
                    }
                    if (linea.charAt(0) == guion) {
                        msgTemp =
                            "La línea " + (i + 1) + " para el Nombre y Dirección del " + bloque + " no puede comenzar con un guión.";
                        guardarErrorInstrucciones("guion"+bloque+"("+(i + 1)+")",msgTemp);
                        camposObligatorios = Boolean.FALSE;
                    }
                }
                if (partsDireccion.length > cantidadLineasDireccion) {
                    msgTemp =
                        "La cantidad de líneas es " + partsDireccion.length +
                        " para el Nombre y Dirección del " + bloque + " no puede ser mayor a " + cantidadLineasDireccion + ".";
                    guardarErrorInstrucciones("cantidadLineasDireccion"+bloque,msgTemp);
                    camposObligatorios = Boolean.FALSE;
                }
            }               
        }
        else{
            msgTemp = "No es obligatorio seleccionar algún tipo de opcion para el " + bloque + "." ;
            //guardarErrorInstrucciones("TipoOpcion"+bloque,msgTemp);
            //camposObligatorios = Boolean.FALSE;
        }
        logger.warning("Fin validacionBloqueInstrucciones bloqu: " + bloque + " retorna " + camposObligatorios);
    ...
    private Map<String, String> mapTable = new HashMap<String, String>();
    private Object[] mapKeysErroresInstrucciones;
    
    public Object[] getMapKeysErroresInstrucciones()
    {  
        logger.warning("Inicio getMapKeysErroresInstrucciones");
        Object[] x; 
        try{
            logger.warning("getHashMapTableErroresInstrucciones size " + getHashMapTableErroresInstrucciones().size());
             x = getHashMapTableErroresInstrucciones().keySet().toArray();
        }
        catch(Exception exp){
            logger.warning("Error " + exp);
            x = new Object[]{"1","2","3"};
        }
        return x;
    }
    
    public Map<String, String> getHashMapTableErroresInstrucciones()
    {  
        return mapTable;
    }
    ...
    public void borrarCamposNoUtilizadosTransferencias(){
        ...
        String TipoOpcionBeneficiario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBeneficiario"); 
        String TipoOpcionBancoBeneficiario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBancoBeneficiario");   
        String TipoOpcionBancoIntermediario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBancoIntermediario");
        ...
        {
            ADFUtils.setBoundAttributeValue("IdentificadorBancoBeneficiario", null);
            ADFUtils.setBoundAttributeValue("DireccionBancoBeneficiario", null);    
        } 
        if(TipoOpcionBancoBeneficiario.equals("OPCION_DEFAULT"))
        {
            ...
            ADFUtils.setBoundAttributeValue("IdentificadorBancoBeneficiario", null);
            ADFUtils.setBoundAttributeValue("DireccionBancoBeneficiario", null);
            ADFUtils.setBoundAttributeValue("NumeroCuentaBancoBeneficiario", null);           
        }
        //********************************************************************************
        ...
        {
            ADFUtils.setBoundAttributeValue("IdentificadorBancoIntermediario", null);
            ADFUtils.setBoundAttributeValue("DireccionBancoIntermediario", null);         
        } 
        if(TipoOpcionBancoIntermediario.equals("OPCION_DEFAULT"))
        {
            ...
            ADFUtils.setBoundAttributeValue("IdentificadorBancoIntermediario", null);
            ADFUtils.setBoundAttributeValue("DireccionBancoIntermediario", null);
            ADFUtils.setBoundAttributeValue("NumeroCuentaBancoIntermediario", null);            
        }
        //********************************************************************************
    }
    ...
    public void guardarTransferencia(ActionEvent actionEvent) { 
        ...
        String TipoOpcionBeneficiario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBeneficiario");
        String NumeroCtaBenef = (String) ADFUtils.getBoundAttributeValue("NumeroCuenteBeneficiario");
        String IdentificadorBeneficiario = (String) ADFUtils.getBoundAttributeValue("IdentificadorBeneficiario");
        String NombreDireccionBeneficiario = (String) ADFUtils.getBoundAttributeValue("NombreBeneficiario");
        
        String TipoOpcionBancoBeneficiario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBancoBeneficiario");        
        String NumeroCuentaBancoBeneficiario = (String) ADFUtils.getBoundAttributeValue("NumeroCuentaBancoBeneficiario");
        String IdentificadorBancoBeneficiario = (String) ADFUtils.getBoundAttributeValue("IdentificadorBancoBeneficiario");
        String NombreDireccionBancoBeneficiario = (String) ADFUtils.getBoundAttributeValue("NombreBancoBeneficiario");
        
        String TipoOpcionBancoIntermediario = (String) ADFUtils.getBoundAttributeValue("TipoOpcionBancoIntermediario");
        String NumeroCuentaBancoIntermediario = (String) ADFUtils.getBoundAttributeValue("NumeroCuentaBancoIntermediario");
        String IdentificadorBancoIntermediario = (String) ADFUtils.getBoundAttributeValue("IdentificadorBancoIntermediario");
        String NombreDireccionBancoIntermediario = (String) ADFUtils.getBoundAttributeValue("NombreBancoIntermediario");
        ...
        logger.warning("IdentificadorBeneficiario: " + IdentificadorBeneficiario);
        logger.warning("NombreDireccionBeneficiario: " + NombreDireccionBeneficiario);
        
        logger.warning("TipoOpcionBancoBeneficiario: " + TipoOpcionBancoBeneficiario);
        logger.warning("NumeroCuentaBancoBeneficiario: " + NumeroCuentaBancoBeneficiario);
        logger.warning("IdentificadorBancoBeneficiario: " + IdentificadorBancoBeneficiario);
        logger.warning("NombreDireccionBancoBeneficiario: " + NombreDireccionBancoBeneficiario);
        
        logger.warning("TipoOpcionBancoIntermediario: " + TipoOpcionBancoIntermediario);
        logger.warning("NumeroCuentaBancoIntermediario: " + NumeroCuentaBancoIntermediario);
        logger.warning("IdentificadorBancoIntermediario: " + IdentificadorBancoIntermediario);
        logger.warning("NombreDireccionBancoIntermediario: " + NombreDireccionBancoIntermediario);
         
        boolean vb_InstBeneficiarioValidas = validacionBloqueInstrucciones(
            "Beneficiario",
        ...
        boolean vb_InstBancoBeneficiarioValidas = validacionBloqueInstrucciones(
            "Banco Beneficiario",
        ...
        boolean vb_InstBancoIntermediarioValidas = validacionBloqueInstrucciones(
            "Banco Intermediario",
        ...
        logger.warning("vb_InstBeneficiarioValidas: " + vb_InstBeneficiarioValidas);        
        logger.warning("vb_InstBancoBeneficiarioValidas: " + vb_InstBancoBeneficiarioValidas);        
        logger.warning("vb_InstBancoIntermediarioValidas: " + vb_InstBancoIntermediarioValidas);
        
        camposObligatorios = vb_InstBeneficiarioValidas && vb_InstBancoBeneficiarioValidas && vb_InstBancoIntermediarioValidas;

        BigDecimal montoTransferencia = (BigDecimal) ADFUtils.getBoundAttributeValue("MontoTransferencia1");
        InstruccionPagoBean instruccionesBean = (InstruccionPagoBean) JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
         
        if (montoTransferencia == null || montoTransferencia == BigDecimal.ZERO) {
    ...
    public Boolean guardarInformacionTransferencia() {
        ...
        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("valiarCamposCuentaInstruccionPago");
            operBinding.execute();

            if (!operBinding.getErrors().isEmpty()) {
                 logger.warning("*** OperationBinding valiarCamposCuentaInstruccionPago devuelve errores-> "
                                                                             +operBinding.getErrors().toString());
            }
        } catch (Exception e) {
            logger.warning("***Error al ejecutar el operationBinding valiarCamposCuentaInstruccionPago ->", e);
        }
    ...
    public void validarCamposNumeroCuenta() {
        ...
        String numeroCuentaBancoBenef = (null == ADFUtils.getBoundAttributeValue("NumeroCuentaBancoBeneficiario") ) ? null
                                      : ADFUtils.getBoundAttributeValue("NumeroCuentaBancoBeneficiario").toString();

        if (null == numeroCuentaBancoBenef || numeroCuentaBancoBenef.equals("")) {
            logger.warning("*Inf, el campo NumeroCuentaBancoBeneficiario es null no se validara");
        } else {

            if (numeroCuentaBancoBenef.substring(0, 1).equals("/")) {
                logger.warning("*Inf,ok NumeroCuentaBancoBeneficiario ya cuenta con el caracter /");
            } else {
                logger.warning("*Inf, agregando el caracter / al numero de cuenta de NumeroCtaBanBenef");
                numeroCuentaBancoBenef = "/" + numeroCuentaBancoBenef;
                ADFUtils.setBoundAttributeValue("NumeroCuentaBancoBeneficiario", numeroCuentaBancoBenef);
            }
        ...
        String numeroCuentaBancoInter = (null == ADFUtils.getBoundAttributeValue("NumeroCuentaBancoIntermediario")) ? null
                                      : ADFUtils.getBoundAttributeValue("NumeroCuentaBancoIntermediario").toString();

        if (null == numeroCuentaBancoInter || numeroCuentaBancoInter.equals("")) {
            logger.warning("*Inf, el campo NumeroCuentaBancoIntermediario es null no se validara");
        } else {

            if (numeroCuentaBancoInter.substring(0, 1).equals("/")) {
                logger.warning("*Inf,ok NumeroCuentaBancoIntermediario ya cuenta con el caracter /");
            } else {
                logger.warning("*Inf, agregando el caracter / al numero de cuenta de NumeroCuentaBancoIntermediario");
                numeroCuentaBancoInter = "/" + numeroCuentaBancoInter;
                ADFUtils.setBoundAttributeValue("NumeroCuentaBancoIntermediario", numeroCuentaBancoInter);
            }
    ...
    public void comitTransferencia() {
        ...
        OperationBinding operBinding = null;
        try {
            operBinding = binding.getOperationBinding("guardarInformacionTransferencia");
            operBinding.execute();
        } catch (Exception e) {
            logger.warning("**Error al ejecutar el operationBinding comitTransferencia: ", e);
        }


        if (!operBinding.getErrors().isEmpty()) {
                 logger.warning("*Inf, OperationBinding comitTransferencia devuelve errores ->"
                                                                +operBinding.getErrors().toString());
        } else {
            ...
            if (idDesembolso != null) {
                ...
                logger.warning("*Error, No se pudo recargar la tabla de transferencias idDesembolso resuelto a NUll");
                JSFUtils.addFacesErrorMessage("Error, No se pudo recuperar el id del contrato de desembolso");
            }
            ...
            instruccionesBean.setAccionAgregar(Boolean.FALSE);
            AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorPrincipal());
    ...
    public void comitTransferencia2() {
        ...
        try {
            ...
            logger.warning("***Error, no se pudo recuperar el id de la operacion");
        }


        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("comitTransferencia");
            operBinding.execute();

            if (!operBinding.getErrors().isEmpty()) {
                logger.warning("*Inf, OperationBinding comitTransferencia devuelve errores");
                logger.warning("***Error -> " + operBinding.getErrors().toString());
            }
        } catch (Exception e) {
            logger.warning("**Error al ejecutar el operationBinding comitTransferencia: ", e);
        }
    ...
    public void actualiarInstruccionPago(ActionEvent actionEvent) {
        ...
        InstruccionPagoBean transferenciasBean = (InstruccionPagoBean) JSFUtils.resolveExpression("#{pageFlowScope.TransferenciasBean}");
        Long IdOperacion = transferenciasBean.getIdOperacion();

        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("actualizarInstruccionPago");
            operBinding.getParamsMap().put("idOperacion", IdOperacion);
            operBinding.execute();
    ...
    public void editarDatosTransferencia(ActionEvent actionEvent) {
        Boolean vbEditarDeshabilitado = (Boolean)JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean.deshabilitarBtnEdicionTran}");
    ...
    public void aceptarEliminarInstruccion(ActionEvent actionEvent) {
        ...
        ADFUtils.setBoundAttributeValue("TipoOpcionBancoBeneficiario", "OPCION_A");
        ...
        ADFUtils.setBoundAttributeValue("TipoOpcionBancoIntermediario", "OPCION_A");
    ...
    public void eliminarTransferencia(Long idContrato, Long idTransferencia) {
        ...
        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("transferenciaABanEstatusCero");
            operBinding.getParamsMap().put("idTransferencia", idTransferencia);
            operBinding.execute();

            if (!operBinding.getErrors().isEmpty()) {
                logger.warning("*Inf, OperationBinding transferenciaABanEstatusCero devuelve errores"
                                                                      +operBinding.getErrors().toString());
            }else{
                seEliminoTransferencia = (null == operBinding.getResult())? Boolean.FALSE
                                        : (Boolean)operBinding.getResult();
            }

        } catch (Exception e) {
            logger.warning("***Error al ejecutar el operationBinding transferenciaABanEstatusCero: ", e);
        }
        ...
        if (seEliminoTransferencia) {
            ...
            if (quedanRegistrosTrans) {
                logger.warning("*Inf, aun hay transferencias estado del formulario visible");
            } else {
    ...
    public Boolean validarMontoTransferencia(BigDecimal montoTransferencia) {
        logger.warning(" Inicia metodo validarMontoTransferencia valor recuperado-->" + montoTransferencia);

        Boolean respuesta = Boolean.TRUE;

        if (montoTransferencia == null || montoTransferencia == BigDecimal.ZERO) {
            JSFUtils.addFacesErrorMessage("Error, se requiere un valor para el monto de la transferencia.");
        }
        ...
        if (instruccionesBean.getAccionAgregar()) {
            ...
            if (res == 1) {
                ...
                JSFUtils.addFacesErrorMessage("No se cuenta con el monto disponible para la transferencia..");
            }
        } else {
            ...
            if (instruccionesBean.getMontoTransfrenciaSeleccionada() == BigDecimal.ZERO) {
                if (null !=
                    JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.MontoTransferencia}")) {
                    String montoTransferenciaSelect =
                        JSFUtils.resolveExpression("#{bindings.ConsultaTransferenciasBanByDesembolso.MontoTransferencia}").toString();
                    instruccionesBean.setMontoTransfrenciaSeleccionada(new BigDecimal(montoTransferenciaSelect));
                } else {
                    logger.warning("El monto de la transferencia seleccionada es a Null");
                }
            }
            logger.warning("montoTransSelect : " + instruccionesBean.getMontoTransfrenciaSeleccionada());
    ...
    public Boolean validarMontoTransferencia2(BigDecimal montoTransferencia) {
        ...
        logger.warning("disponible: " + disponible);
        logger.warning(" MontoTransfrenciaSeleccionada : " + instruccionesBean.getMontoTransfrenciaSeleccionada());
        BigDecimal disponibleAtransferir = disponible.add(instruccionesBean.getMontoTransfrenciaSeleccionada());
    ...
    public BigDecimal obtenerMontoTransferenciaActual() {
        ...
        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("obtenerTransferenciaActual");
            Row fila = (Row) operBinding.execute();

            if (fila != null) {
                logger.warning("*Inf, monto actual de la Transferencia recuperado : " +fila.getAttribute("MontoTransferencia"));
                if (fila.getAttribute("MontoTransferencia") != null) {
                    montoTransferencia = (BigDecimal) fila.getAttribute("MontoTransferencia");
                }
            } else {
                logger.warning("*Inf, Current en transferencia null, monto de la transferencia null");
            }

            if (!operBinding.getErrors().isEmpty()) {
                logger.warning("*** OperationBinding obtenerMontoTransferenciaActual devuelve errores");
            }
        } catch (Exception e) {
            logger.warning("***Error, al ejecutar el operationBinding: ", e);
        }
    ...
    public void aceptarCancelarGuardado2(ActionEvent actionEvent) {
        ...
        if (instruccionesBean.getEsNuevaTransferencia()) {
            ...
            try {
                BindingContainer binding = ADFUtils.getBindingContainer();
                OperationBinding operBinding = binding.getOperationBinding("cambiarTransferenciaABanEstatusCero");
                Boolean res = (Boolean) operBinding.execute();

                if (!operBinding.getErrors().isEmpty()) {
                    logger.warning("*** OperationBinding cambiarTransferenciaABanEstatusCero devuelve errores");
                }
            } catch (Exception e) {
                logger.warning(" :( Error al ejecutar el operationBinding cambiarTransferenciaABanEstatusCero : ", e);
            }
    ...
    public void aplicarTransferencia(ActionEvent actionEvent) {
        ...
        if (eventoCLick == 1) {
            ...
            logger.warning("Id recuperado para aplicar la transferencia ->" + idTransferenciaAplicar);
            InstruccionPagoBean instruccionesBean =
                (InstruccionPagoBean) JSFUtils.resolveExpression("#{pageFlowScope.InstruccionPagoBean}");
            ...
            try {
                BindingContainer binding = ADFUtils.getBindingContainer();
                OperationBinding operBinding = binding.getOperationBinding("aplicarTransferencia");
                operBinding.getParamsMap().put("idTransferencia", idTransferenciaAplicar);
                operBinding.getParamsMap().put("usuario", instruccionesBean.getUsuario());
                operBinding.getParamsMap().put("instanciaProceso", instruccionesBean.getInstanciaDProceso());

                operBinding.execute();

                if (!operBinding.getErrors().isEmpty()) {
                    logger.warning("*** OperationBinding aplicarTransferencia devuelve errores->" +operBinding.getErrors());
                    JSFUtils.addFacesErrorMessage("Ha ocurrido un error durante la aplicacion de la transferencia. Por favor intentelo de nuevo.");
                } else {
                    bhqTransferencia = (String) operBinding.getResult();
                    if (bhqTransferencia != null) {
                        logger.warning("ok BHQ de transferencia recuperado ->" + bhqTransferencia);
                        Long idContrato = instruccionesBean.getIdContratoDesembolso();
                        instruccionesBean.cargarTablaTransferencias(idContrato);
                        instruccionesBean.cargarTablaTransferencias(idContrato);
                        AdfFacesContext.getCurrentInstance().addPartialTarget(getContenedorPrincipal());
                        ...
                        logger.warning("*** Error, No se pudo recuperar el BHQ de la transferencia");
                    }
                }

            } catch (Exception e) {
                logger.warning("***Error al ejecutar el operationBinding aplicarTransferencia ", e);
                JSFUtils.addFacesErrorMessage("Error durante la aplicacion de la transferencia ->" +e.getMessage().toString());
            }

            logger.warning("Termina metodo aplicarTransferencia");
            eventoCLick = 0;
        } else {
            logger.warning("Boton aplicar transferencia presionado:" + eventoCLick +" aun se esta ejecutando la primera accion espere");
        }
    }
    ...
    public Row recuperarInstruccionPagoCurrent() {
        ...
        try {
            BindingContainer binding = ADFUtils.getBindingContainer();
            OperationBinding operBinding = binding.getOperationBinding("getInstruccionPagoCurrent");
            operBinding.execute();

            if (!operBinding.getErrors().isEmpty()) {
                            logger.warning("***Error, OperationBinding getInstruccionPagoCurrent devuelve errores"
                                           +operBinding.getErrors());
            } else {
    ...
    public void restaurarCamposByInstruccion() {
        ...
        if (instruccionCurrent != null) {
            try {
                rowAtributetipoOpcionBenef = instruccionCurrent.getAttribute("TipoOpcionBenef").toString();
                logger.warning("*Inf, tipo de opcion Beneficiario: " + rowAtributetipoOpcionBenef);
            } catch (Exception e) {
                logger.warning("*inf, ocurrio un error al castear TipoOpcionBenef de instruccion de pago");
            }
            try {
                rowAtributetipoOpcionBanInter = instruccionCurrent.getAttribute("TipoOpcionBanInter").toString();
                logger.warning("*Inf, tipo de ocpion Banco intermediario: " + rowAtributetipoOpcionBanInter);
            } catch (Exception e) {
                logger.warning("*inf, ocurrio un error al castear TipoOpcionBanInter de instruccion de pago");
            }
            try {
                rowAtributetipoOpcionBanBenef = instruccionCurrent.getAttribute("TipoOpcionBanBenef").toString();
                logger.warning("*Inf, tipo de ocpion Banco Beneficiario: " + rowAtributetipoOpcionBanBenef);
            } catch (Exception e) {
                logger.warning("*inf, ocurrio un error al castear TipoOpcionBanBenef de instruccion de pago");
            }
        ...
        logger.warning("*Inf, Termina metodo restaurarCamposByInstruccion con una duracion de: " + tiempo +
                       " segundos");
    }
    ...
    public void validarCampoNumCuentaTrans(ClientEvent ce) {
        ...
        String numeroCuenta = (String) ADFUtils.getBoundAttributeValue("NumeroCtaBenef");
        ...
        if (null == numeroCuenta || numeroCuenta.equals("")) {
            ...
            if (numeroCuenta.substring(0, 1).equals("/")) {
                ...
                logger.warning("*Inf, agregando el caracter / al numero de cuenta");
    ...
    public void validarCampoNumCuentaBanBeneficiario(ClientEvent ce) {
        logger.warning("*Inf, Inicia metodo validarCampoNumCuentaBanBeneficiario");

        String numeroCuenta = (String) ADFUtils.getBoundAttributeValue("NumeroCuentaBancoBeneficiario");
        numeroCuenta = (null == numeroCuenta) ? null : numeroCuenta.trim();
        ...
        if (numeroCuenta == null || numeroCuenta.equals("")) {
            ...
            if (numeroCuenta.substring(0, 1).equals("/")) {
                ...
                logger.warning("*Inf, agregando el caracter / al numero de cuenta");
                numeroCuenta = "/" + numeroCuenta;
                ADFUtils.setBoundAttributeValue("NumeroCuentaBancoBeneficiario", numeroCuenta);
                AdfFacesContext.getCurrentInstance().addPartialTarget(getForm2());
            }

        }

        logger.warning("*Inf, termina metodo validarCampoNumCuentaBanBeneficiario");
    }


    public void validarCampoNumCuentaBanIntermediario(ClientEvent ce) {
        logger.warning("*Inf, Inicia metodo validarCampoNumCuentaBanIntermediario");

        String numeroCuenta = (String) ADFUtils.getBoundAttributeValue("NumeroCuentaBancoIntermediario");
        numeroCuenta = (null == numeroCuenta) ? null : numeroCuenta.trim();
        ...
        if (numeroCuenta == null || numeroCuenta.equals("")) {
            ...
            if (numeroCuenta.substring(0, 1).equals("/")) {
                ...
                logger.warning("*Inf, agregando el caracter / al numero de cuenta");
                numeroCuenta = "/" + numeroCuenta;
                ADFUtils.setBoundAttributeValue("NumeroCuentaBancoIntermediario", numeroCuenta);
                AdfFacesContext.getCurrentInstance().addPartialTarget(getForm3());
            }

        }

        logger.warning("*Inf, termina metodo validarCampoNumCuentaBanIntermediario");
    }

    /********      Accesors       *****/
    public void setFormulariosInstruccionesPago(RichPanelGroupLayout formulariosInstruccionesPago) {
    ...
    public void cambioTipoMensaje(ValueChangeEvent valueChangeEvent) {
        ...
        if (null != valueChangeEvent) {
            try {
                ...
                logger.warning("ERROR al recuperar tipoMensaje de ValueChangeEvent.", e);
            }
        }
    ...
    public void setContenedorCamposTransferencia(RichPanelGroupLayout contenedorCamposTransferencia) {
    ...
    public void setContenedorAccionesTransaccionesPago(RichPanelGridLayout contenedorAccionesTransaccionesPago) {
    ...
    public void setContenedorAccionesTransferencia(RichPanelGroupLayout contenedorAccionesTransferencia) {
    ...
    public void setContenedorAccionesAInstrucciones(RichPanelGroupLayout contenedorAccionesAInstrucciones) {
    ...
    public void setPopUpErrorAplicarTransferencia(RichPopup popUpErrorAplicarTransferencia) {
