...
public class DetalleOperacionBean implements Serializable {
    ...
    private static final Integer PROCESO_SUPERVISION_AMBIENTAL_SOCIAL = 39;
    ...
    private RichPopup popupInicioCondicionPreviaFormalizar;
    private RichPopup popupConfirmInicioCondicionPreviaFormalizar;
    ...
    private static final String HEADER_ACTION_FORMENMIENDA_FENIX = "solicitarFormalizacionEnmiendaFenix";
    ...
    public void aceptarConfirmarActionListener(ActionEvent actionEvent) {
        ...
        if ((headerAction != null) && (headerAction.trim().length() > 0)) {
            ...
            if (headerAction.equalsIgnoreCase("solicitarSupervisionAmbientalSocial")) {
                metodo = solicitarSupervisionAmbientalSocial();
                if (!metodo) {
                    cerrar = Boolean.FALSE;
                }
            ...
            if (headerAction.equalsIgnoreCase(HEADER_ACTION_FORMENMIENDA_FENIX)){   
                solicitarFormalizacionEnmienda(FenixModelConstants.ORIGEN_FORMALIZACION_ENMIENDA_FENIX_VALUE);
            }
    ...
    private Boolean solicitarSupervisionAmbientalSocial() {
        Integer ContadorErrores = 0;
        logger.log(ADFLogger.TRACE, "Inside solicitarSupervisionAmbientalSocial.");
        BindingContainer bindings = ADFUtils.getBindingContainer();
        OperationBinding operationBinding = null;
        try {

            operationBinding = bindings.getOperationBinding("inicioSupervisionAmbientalSocial");
            String idOperacion = Long.toString(gestorOperacionesBean.getIdOperacion());
            operationBinding.getParamsMap().put("idOperacion", idOperacion);
            operationBinding.getParamsMap().put("loginUsuario", ADFContext.getCurrent().getSecurityContext().getUserName());
            operationBinding.getParamsMap().put("descripcionOperacion", gestorOperacionesBean.getOperacion());
    ...
    public void solicitarSupervisionAmbientalSocialActionListener(ActionEvent actionEvent) {
        logger.log(ADFLogger.TRACE, "Inside solicitarSupervisionAmbientalSocialActionListener.");
        BindingContainer bindings = ADFUtils.getBindingContainer();
        OperationBinding operationBinding = null;
        operationBinding = bindings.getOperationBinding("validaProcesos");
        operationBinding.getParamsMap().put("operacion", Long.toString(gestorOperacionesBean.getIdOperacion()));
        operationBinding.getParamsMap().put("proceso", PROCESO_SUPERVISION_AMBIENTAL_SOCIAL);
        Boolean result = (Boolean) operationBinding.execute();
        if (result) {
            JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_SUPERVISION_AMBIENTAL_SOCIAL"));
        } else {
            BindingContainer bindingsEstado = ADFUtils.getBindingContainer();
            OperationBinding operationBindingEstado = null;
            operationBindingEstado = bindingsEstado.getOperationBinding("validarEstadoOperacion");
            ...
            if (resultEstado == ESTADO_SUSPENDIDO) {
                ...
                if (resultEstado == ESTADO_FINALIZADO) {
                    ...
                    if (resultEstado == ESTADO_CANCELADO) {
                        ...
                        logger.log(ADFLogger.TRACE,
                                   "Inside solicitarSupervisionActionListener: " + actionEvent.getComponent().getId());
                        setMensajeConfirmacion("¿Confirma iniciar el ‘Proceso de Supervisión A&S’ de la operación " +
                                               gestorOperacionesBean.getIdOperacion() + "?"); // Asignamos mensaje
                        showPopupConfirmarHeaderAction(); // Mostramos popup
    ...
    public void solicitarPrepagoActionListener(ActionEvent actionEvent) {
        ...
        if (contratosOperacion) {
            ...
            JSFUtils.addFacesErrorMessage("Debe verificar el saldo no vencido de la operación " + gestorOperacionesBean.getIdOperacion() + ".");
            //JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_PREPAGO"));
        }
    }
    ...
    /**
    ???? * Se crea metodo iniciaCumplimientoCondicionPreviaFormalizarActionListener() para validar que
    ???? * hay condiciones con el estado por validar
    ???? * @param actionEvent
    ???? * @since 07/09/2021
    ???? */
    public void iniciaCumplimientoCondicionPreviaFormalizarActionListener(ActionEvent actionEvent) {
        logger.log(ADFLogger.WARNING, "INTO iniciaCumplimientoCondicionPreviaFormalizarActionListener");
        ...
        try {
            if (null != gestorOperacionesBean.getIdOperacion()) {
                ...
                logger.log(ADFLogger.WARNING, "getOperationBinding obtenerCondicionPreviaFormalizarPorValidar con idOperacion: " + idOperacion);
                operationBinding = bindings.getOperationBinding("obtenerCondicionPreviaFormalizarPorValidar");
                operationBinding.getParamsMap().put("idOperacion", idOperacion);
                logger.log(ADFLogger.WARNING, "Execute obtenerCondicionPreviaFormaliarPorValidar con idOperacion" + idOperacion);
                operationBinding.execute();
            ...
            if (null != operationBinding.getResult()) {
                ...
                logger.log(ADFLogger.WARNING, "El valor de retorno de obtenerCondicionPreviaFormalizarPorValidar es nulo.");
            }
            if (!validar) {
                JSFUtils.addFacesInformationMessage(getStringFromBundle("MSG_38_DETALLE_OPERACION"));
            } else {
                RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
                getPopupInicioCondicionPreviaFormalizar().show(popupHints);
            }
        } catch (Exception e) {
            logger.log(ADFLogger.ERROR,
                       "Error en iniciaCumplimientoCondicionPreviaFormalizarActionListener." + e.getClass() + "." + e.getMessage());
        }
    }
    ...
    public void setPopupConfirmInicioCondicionPreviaFormalizar(RichPopup popupConfirmInicioCondicionPreviaFormalizar) {
        this.popupConfirmInicioCondicionPreviaFormalizar = popupConfirmInicioCondicionPreviaFormalizar;
    }

    public RichPopup getPopupConfirmInicioCondicionPreviaFormalizar() {
        return popupConfirmInicioCondicionPreviaFormalizar;
    }
    ...
    /**
    ???? * Se crea metodo cumplimientoCondicionPreviaFormalizarActionListener() para validar que el proceso de Aprobacion ha finalizado
    ???? * y que el usuario tiene rol de responsable de la operacion
    ???? * @param actionEvent
    ???? * @since 07/09/2021
    ???? */
    public void cumplimientoCondicionPreviaFormalizarActionListener(ActionEvent actionEvent) {
        logger.log(ADFLogger.WARNING, "Inside cumplimientoCondicionPreviaFormalizarActionListener.");
        ...
        try {
            ...
            if (null != iniciaProceso) {
                if (iniciaProceso) {
                    getPopupInicioCondicionPreviaFormalizar().hide();
                    getPopupConfirmInicioCondicionPreviaFormalizar().hide();
                } else {
                    getPopupInicioCondicionPreviaFormalizar().hide();
                    JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_04_DETALLE_OPERACION"));
                }
            ...
            logger.log(ADFLogger.ERROR,
                       "Error en cumplimientoCondicionPreviaFormalizarActionListener." + e.getClass() + "." + e.getMessage());
        }
    }
    ...
    public void aceptarCumplimientoCondicionPreviaFormalizarActionListener(ActionEvent actionEvent) {
        if (!validarSeleccionCondicion()){
            JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_SELECCION_CONDICION"));
        }else{
        RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
        getPopupConfirmInicioCondicionPreviaFormalizar().show(popupHints);}
    }
    ...
    public void cancelarCumplimientoCondicionPreviaFormalizarActionListener(ActionEvent actionEvent) {
        popupConfirmInicioCondicionPreviaFormalizar.hide();
    }
...
//            }
     ...
     * 1. Que el usuario actual sea el responsable de la operación.
     * 2. Que la operacion se encuentre en estado EN PROCESO.
     * 3. Si es desde fenix, que se haya marcado alguna de las banderas que solicitan la formalización de una enmienda.
     * 
     * Si es una formalización desde fenix, levanta popup de confirmación. De lo contrario levante popup para formalizaciones genéricas.
     * 
     * @param actionEvent
     */
    public void solicitarFormalizacionEnmiendaActionListener(ActionEvent actionEvent) {
        ...
        logger.info("Iniciando solicitarFormalizacionEnmiendaActionListener accion = " + accion);
        logger.log(ADFLogger.TRACE,
        ...
        //Para formalizaciones desde fenix se debe solicitar confirmacion
        if (HEADER_ACTION_FORMENMIENDA_FENIX.equals(accion)) {
            // Asignamos mensaje al popup de confirmacion
            String msg = getStringFromBundle("FORMALIZACION_ENMIENDAS_PREGUNTA_TXT");
    ...
    private void solicitarFormalizacionEnmienda(int origen) {
        logger.log(ADFLogger.TRACE, "Inside solicitarFormalizacionEnmienda.");
        ...
        BindingContainer bindings = ADFUtils.getBindingContainer();
        //Para las formalizaciones desde fenix debemos obtener todas las enmiendas que requieran formalización y enviarlas una por una
        List<Long> enmiendas = new ArrayList<>();
        if (origen == FenixModelConstants.ORIGEN_FORMALIZACION_ENMIENDA_FENIX_VALUE) {
            OperationBinding operationEnmiendas = bindings.getOperationBinding("obtenerEnmiendasXFormalizar");
            operationEnmiendas.getParamsMap().put("idOperacion", gestorOperacionesBean.getIdOperacion());
            enmiendas = (List<Long>) operationEnmiendas.execute();
            if (!operationEnmiendas.getErrors().isEmpty()) {
                //Permitimos que el framework despliegue los mensajes de error por su cuenta
                return;
            }
            //si no hay enmiendas por formalizar, entonces verificar líneas de crédito
            if (enmiendas == null || enmiendas.isEmpty()) {
                logger.info("No se encontraron enmiendas desde fenix para formalizar. Verificando líneas de crédito.");

                //mostrar error porque no hubo enmiendas. No continuar.
                JSFUtils.addFacesErrorMessage(getStringFromBundle("FORMALIZACION_ENMIENDAS_NO_ENMIENDAS_ERROR"));
                return;
            }
        } else {
            ...
            //las formalizaciones genéricas no requieren un idEnmienda, pero necesitamos almenos un elemento nulo en la lista
            enmiendas.add(null);
        }

        //por cada enmienda necesitamos inicializar el proceso de formalización        
        OperationBinding operationBinding = bindings.getOperationBinding("inicioFormalizacionEnmiendas");
        boolean exito = true;
        if (enmiendas != null) {
            for (Long idEnmienda : enmiendas) {
                //modifica el idEnmienda antes de invocar la operación nuevamente
                params.put(FenixModelConstants.AM_PARAM_KEY_IDENMIENDA, idEnmienda);
    ...
    public void setPopupInicioCondicionPreviaFormalizar(RichPopup popupInicioCondicionPreviaFormalizar) {
        this.popupInicioCondicionPreviaFormalizar = popupInicioCondicionPreviaFormalizar;
    }

    public RichPopup getPopupInicioCondicionPreviaFormalizar() {
        return popupInicioCondicionPreviaFormalizar;
    }
    ...
    public void cancelarConfirmacionEnmiendaGenericaActionListener(ActionEvent actionEvent) {
        logger.log(ADFLogger.TRACE,
                   "Inside cancelarConfirmacionEnmiendaGenericaActionListener: " + actionEvent.getComponent().getId());
        getPopupConfirmacionEnmiendaGenerica().hide();
    }
