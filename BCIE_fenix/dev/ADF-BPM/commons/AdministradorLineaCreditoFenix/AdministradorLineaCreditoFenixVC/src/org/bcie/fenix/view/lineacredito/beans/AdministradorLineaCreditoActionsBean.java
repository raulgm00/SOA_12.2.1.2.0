...
public class AdministradorLineaCreditoActionsBean {
        ...
        if (validarDatosLineaCredito()) {
            ...
            if (operationBinding.getErrors().size() < 1) {
                ...
                if (desobligacion) {
                    ...
                    Map mapDesobligacionFin = (Map) ADFContext.getCurrent().getPageFlowScope().get("desobligacionFin");
                    if (mapMontosDesobligar != null && idLineaCredito != null) {
                        ...
                        {
                        mapDesobligacionFin.put("DESOBLIGACION_FIN", this.desobligacionFin.toString());
                    }
                        else
                        {
                    ...
                    logger.log(ADFLogger.WARNING, "Se agrega launchDesobligacionFinEvent");
    ...
    public void aceptarAgregarLineaCreditoActionListener(ActionEvent actionEvent) {
        ...
        if (null != sMontoLinea) {
            try {
                logger.warning("+++++++++++++ sMontoLineaCredito: " + sMontoLinea);
    ...
    public void setBtnActualizarInformacionUIC(RichButton btnActualizarInformacionUIC) {
    ...
    public Boolean getDesobligacionFin() {
        ...
        try{
        logger.warning("Leyendo Desobliga desde mapa en memoria." + this.desobligacionFin);
        if (this.desobligacionFin == null) {
            ...
            if (mapDesobligacionFin != null) {
                ...
                String vDesobligacionFin = mapDesobligacionFin.get("DESOBLIGACION_FIN").toString();
    ...
    private void launchDesobligacionFinEvent(ActionEvent evt) {
        ...
        JUEventBinding eventBinding = (JUEventBinding) ADFUtils.getBindingContainer().get("desobligacionFinEventBinding");
        if (eventBinding == null) {
            logger.warning("ContextEvent not found!!!");
        } else {
            logger.warning("Lanzando evento");
    ...
    public void confirmarPropagarLineasCredito(ActionEvent evt) {
        ...
        if (desobligacion) {
            ...
            if (mapMontosDesobligar == null || mapMontosDesobligar.size() == 0) {
                logger.warning("No se encontraron líneas para desobligar. Mapa de montos a desobligar {0}", mapMontosDesobligar); 
                JSFUtils.addFacesErrorMessage("No se encontraron líneas de crédito con montos por desobligar.");
                return;
            }
            logger.warning("{0} Líneas de crédito para desobligar encontradas", mapMontosDesobligar.size());
            ...
            for (Map.Entry<Long, BigDecimal> entry : mapMontosDesobligar.entrySet()) {
                ...
                logger.warning("Línea {0} => MontoDesobligar {1}", new Object[] { idLinea, montoDesobligarLC });
                ...
                if (!validarDesobligacion()) {
                    ...
                    logger.warning("Falló la validación de la desobligación para la línea {0}", idLinea);
                    rollbackPropagarLineas();
                    break;
                }

                logger.warning("Validación de la desobligación exitosa. Calculando y actualizando montos");
                ...
                if (!invocarActualizarLinea()) {
                    propagacionExitosa = false;
                    logger.warning("Falló actualización de línea de crédito. Solicitando rollback");
                ...
                if (!invocarPropagarLinea(idLinea)) {
                    propagacionExitosa = false;
                    logger.warning("Falló la propagación de línea de crédito. Solicitando rollback");
                    rollbackPropagarLineas();
                    break;
                } 
            }
    ...
    public void AceptarConfirmarPropagarLineasCredito(ActionEvent evt) {
            ...
            AdfFacesContext.getCurrentInstance().addPartialTarget(getBtnActualizarInformacionUIC());
    ...
    private BigDecimal calcularNuevoMontoLinea(final BigDecimal montoLinea, final BigDecimal desobligar){
        ...
        logger.warning("MontoLinea=>{0}, Desobligar=>{1}, NuevoMontoLinea=>{2}", new Object[]{montoLinea, desobligar, nuevoMontoLinea});        
    ...
    private void putLineaInUndoMontosMap(final Long idLinea, final BigDecimal montoLineaOriginal){
        ...
        logger.warning("Almacenando línea {0} al undoMap. Monto original => {1}", new Object[]{idLinea, montoLineaOriginal});
    ...
    private boolean invocarPropagarLinea(final Long idLinea) {
        ...
        logger.warning("idLinea=>{0}, fechaValor=>{1}", new Object[]{idLinea, fechaValor});
    ...
    private boolean validarDesobligacion() {
        ...
        Boolean desobligacion = JSFUtils.resolveExpressionAsBoolean("#{pageFlowScope.desobligacion}");
        if (!desobligacion) {
            logger.warning("No se trata de una desobligación. No se continúa la validación");
            return exito;
        }
    ...
    private void rollbackPropagarLineas(){
        ...
        if(mapUndoMontosLineas != null){
            logger.warning("Iniciando rollback de {0} líneas", mapUndoMontosLineas.size());
            
            for (Map.Entry<Long, BigDecimal> entry : mapUndoMontosLineas.entrySet()) {
                Long idLinea = entry.getKey();
                BigDecimal montoLineaOriginal = entry.getValue();
                logger.warning("Línea=>{0}, MontoLineaOriginal=>{1}", new Object[] { idLinea, montoLineaOriginal });
