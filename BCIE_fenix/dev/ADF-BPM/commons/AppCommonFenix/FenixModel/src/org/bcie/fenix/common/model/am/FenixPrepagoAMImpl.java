...
import com.bcie.xmlns.prepagoservice.Prepago12BndQSService;
...
import org.bcie.fenix.common.model.vo.TcaTipoMonedaDesembolsoVOImpl;
...
import org.bcie.prepagomo.CalculoInteresPrepagoRequestType;
import org.bcie.prepagomo.CalculoInteresPrepagoResponseType;
import org.bcie.prepagomo.DetalleContratoDesembolsoRequestType;
import org.bcie.prepagomo.DetalleContratoDesembolsoResponseType;
...
import org.bcie.prepagomo.DetallePenalidadPrepagoListaPenalidadType;
import org.bcie.prepagomo.DetallePenalidadPrepagoPenalidadType;
import org.bcie.prepagomo.DetallePenalidadPrepagoRequestType;
import org.bcie.prepagomo.DetallePenalidadPrepagoResponseType;
import org.bcie.prepagomo.TDESEMBOLSOPREPAGO;
...
public class FenixPrepagoAMImpl extends ApplicationModuleImpl implements FenixPrepagoAM {
    ...
    public void insertarContratoDesembolso(Long idPrepago){
        logger.log(ADFLogger.WARNING,"Inicio insertarContratoDesembolso");
        ...
        logger.log(ADFLogger.WARNING,"row count in  contratoDesembolsoPrepagoVO is : " + contratoDesembolsoPrepagoVOImpl.getRowCount());
        ...
        for (Row row : rowContratoDesembolsoPrepago) {
            if(new Integer("0").equals(row.getAttribute("IdHijo"))){
                ...
                {
                    logger.log(ADFLogger.WARNING,"No se insertoID value of the table 'TRE_PREPAGO_CONTRATO' is comparateBigDecimal: "+comparateBigDecimal);
                }
            }
        }
        logger.log(ADFLogger.WARNING,"Fin insertarContratoDesembolso");
    ...
    public void llenarDetallePenalidad2(Long idPrepago,Integer idProducto,Integer idOperacion, Timestamp fechaCalculoDefinitivo){
    ...
    public void llenarDetallePenalidad(Long idPrepago,Integer idProducto,Integer idOperacion, Timestamp fechaCalculoDefinitivo){
        try{
            logger.log(ADFLogger.WARNING, "Inicio llenarDetallePenalidad");
        ...
        //            calculoDetallePenalidadVOImpl.executeQuery();
        //            calculoDetallePenalidadVOImpl.setRangeSize(-1);
            logger.log(ADFLogger.WARNING, "Inicio DatosPenalidad dentro de llenarDetallePenalidad6"); 
            Map  map = this.TablaPenalidad(idPrepago,idOperacion);
            List<Map<String,String>> rows = (List<Map<String,String>>)map.get("TBLPENALIDAD");
            logger.log(ADFLogger.WARNING, "Fin DatosPenalidad dentro de llenarDetallePenalidad3");
            logger.log(ADFLogger.WARNING, "rows: " + rows);
            formularioDetallePenalidadVOImpl.llenarCalculoPenalidades(
                rows
            );
        }
        catch(Exception exp){
            logger.log(ADFLogger.WARNING, "Error llenarDetallePenalidadPrepagoTabla exp: " + exp);
        } 
    } 

    public Map<String,Object> llenarDetallePenalidadPrepago(Long idPrepago,Integer idOperacion){
        logger.log(ADFLogger.WARNING, "Inicio llenarDatosPenalidadPrepago2");
        logger.log(ADFLogger.WARNING, "idPrepago: "+idPrepago);
        logger.log(ADFLogger.WARNING, "idOperacion: "+idOperacion);  
        Map<String,Object>  map = this.DatosPenalidad(idPrepago,idOperacion);  
        logger.log(ADFLogger.WARNING, "this.mapDatosPenalidad: " + map);     
        Map<String,Object> rowsCampos = (Map<String,Object>)map.get("LCAMPOS");
        logger.log(ADFLogger.WARNING, "Fin DatosPenalidad dentro de llenarDatosPenalidadPrepago");
        logger.log(ADFLogger.WARNING, "rows: " + rowsCampos); 
        return rowsCampos;
    }
    
    public Map<String,Object> DatosPenalidad(Long idPrepago,Integer idOperacion){
        logger.log(ADFLogger.WARNING, "Inicio DatosPenalidadv2");
        logger.log(ADFLogger.WARNING, "idPrepago: " + idPrepago);
        logger.log(ADFLogger.WARNING, "idOperacion: " + idOperacion);
        Map<String,Object> map = new HashMap <String,Object>(); 
        
        String wsdl = getWsdl(IWsdlLocation.REPORTE_PREPAGO);
        PrepagoPT12BndQSService prepagoPT12BndQSService = IWsdlLocation.Service.getInstance(PrepagoPT12BndQSService.class, wsdl);
        PrepagoPT prepagoPT = prepagoPT12BndQSService.getPrepagoPT12BndQSPort();

        DetallePenalidadPrepagoRequestType request = new DetallePenalidadPrepagoRequestType();  
        BigDecimal IdPrepago =  new BigDecimal(idPrepago);
        BigDecimal IdOperacion =  new BigDecimal(idOperacion);
        
        request.setPIDPREPAGO(IdPrepago);     
        request.setPIDOPERACION(IdOperacion);

        DetallePenalidadPrepagoResponseType response = prepagoPT.detallePenalidadPrepago(request); 
        DetallePenalidadPrepagoListaPenalidadType tbl = response.getPTBLPENALIDAD();           
    
        Timestamp vFechaCalculoDefinitivo = response.getPFECHACALCULODEFINITIVO() != null ? new Timestamp(response.getPFECHACALCULODEFINITIVO().toGregorianCalendar().getTimeInMillis()) : null;
        Timestamp vFechaReferencia = response.getPFECHAREFERENCIA() != null ? new Timestamp(response.getPFECHAREFERENCIA().toGregorianCalendar().getTimeInMillis()) : null;
        
        logger.log(ADFLogger.WARNING,"vFechaCalculoDefinitivo> " + vFechaCalculoDefinitivo);        
        logger.log(ADFLogger.WARNING,"vFechaReferencia> " + vFechaReferencia);
           
        Map<String,Object> tblRowCampos = new HashMap<String,Object>();
        
        tblRowCampos.put("PCARGOTRAMITEDESCRIPCION", response.getPCARGOTRAMITEDESCRIPCION());            
        tblRowCampos.put("PCARGOTRAMITEVALOR", response.getPCARGOTRAMITEVALOR());            
        tblRowCampos.put("PCODIGO", response.getPCODIGO());            
        tblRowCampos.put("PESTADO", response.getPESTADO());            
        tblRowCampos.put("PFECHACALCULODEFINITIVO", vFechaCalculoDefinitivo); 
        tblRowCampos.put("PFECHAREFERENCIA", vFechaReferencia);            
        tblRowCampos.put("PMENSAJE", response.getPMENSAJE());            
        tblRowCampos.put("PTASALIBOR", response.getPTASALIBOR());         
        tblRowCampos.put("PTASAPRIME", response.getPTASAPRIME());         
        tblRowCampos.put("PIDTCATIPOMONEDATRAMITE", response.getPIDTCATIPOMONEDATRAMITE());
        
        logger.log(ADFLogger.WARNING,"PCARGOTRAMITEDESCRIPCION> " + response.getPCARGOTRAMITEDESCRIPCION());     
        logger.log(ADFLogger.WARNING,"PCARGOTRAMITEVALOR> " + response.getPCARGOTRAMITEVALOR());     
        logger.log(ADFLogger.WARNING,"PCODIGO> " + response.getPCODIGO());     
        logger.log(ADFLogger.WARNING,"PESTADO> " + response.getPESTADO());     
        logger.log(ADFLogger.WARNING,"PFECHAREFERENCIA> " + response.getPFECHAREFERENCIA()); 
        logger.log(ADFLogger.WARNING,"PFECHACALCULODEFINITIVO> " + response.getPFECHACALCULODEFINITIVO());     
        logger.log(ADFLogger.WARNING,"PMENSAJE> " + response.getPMENSAJE());     
        logger.log(ADFLogger.WARNING,"PIDTCATIPOMONEDATRAMITE> " + response.getPIDTCATIPOMONEDATRAMITE()); 
        logger.log(ADFLogger.WARNING,"PTASALIBOR> " + response.getPTASALIBOR());   
        logger.log(ADFLogger.WARNING,"PTASAPRIME> " + response.getPTASAPRIME());
             
        map.put("LCAMPOS", tblRowCampos); 
        logger.log(ADFLogger.WARNING, "Finalizo DatosPenalidad");
        
        return map;
    }
    
    public Map<String,Object> TablaPenalidad(Long idPrepago,Integer idOperacion){
        logger.log(ADFLogger.WARNING, "Inicio TablaPenalidad");
        logger.log(ADFLogger.WARNING, "idPrepago: " + idPrepago);
        logger.log(ADFLogger.WARNING, "idOperacion: " + idOperacion);
        Map<String,Object> map = new HashMap <String,Object>(); 
        ...
        if(tbl == null){ 
            logger.log(ADFLogger.WARNING, "tbl es null en TablaPenalidad" ); 
            logger.log(ADFLogger.WARNING, "tbl: " + tbl); 
        }
        else{
            int countPenalidad = tbl.getPTBLPENALIDADITEM().size();            
            List<Map<String,String>> tblRow = new ArrayList<Map<String,String>>();
            
            logger.log(ADFLogger.WARNING, "v2 FormularioDetallePenalidadVO ejecutado con id prepago: " + idPrepago + " y con " + countPenalidad +" filas.");
            for (int i = 0; i < tbl.getPTBLPENALIDADITEM().size(); i++) {
                DetallePenalidadPrepagoPenalidadType item = tbl.getPTBLPENALIDADITEM().get(i);
                Timestamp fechaInicio = item.getFECHAINICIO() != null ? new Timestamp(item.getFECHAINICIO().toGregorianCalendar().getTimeInMillis()) : null;
                Timestamp fechaFin =  item.getFECHAINICIO() != null ? new Timestamp(item.getFECHAFIN().toGregorianCalendar().getTimeInMillis()) : null;
                BigDecimal plazo = item.getPLAZO();
                BigDecimal spread = item.getSPREAD();
                String fraccionLibor = item.getFRACCIONLIBOR();
                BigDecimal tasaPrepago = item.getTASAPREPAGO();
                BigDecimal montoPenalidad = item.getMONTOPENALIDAD();
                BigDecimal intereses = item.getMONTOPENALIDAD();
                Long idTrePrepagoContrato = item.getFECHAINICIO() != null ? item.getIDTREPREPAGOCONTRATO().longValue() : null;
                String contratoDesembolso = item.getCONTRATODESEMBOLSO();
                String moneda = item.getMONEDA();
                String resolucion = item.getRESOLUCION();
                BigDecimal montoPrepago = item.getMONTOPREPAGO();
                Timestamp proximoPago = item.getFECHAINICIO() != null ? new Timestamp(item.getPROXIMOPAGO().toGregorianCalendar().getTimeInMillis()) : null;
                ...
                row.put("fechaInicio",fechaInicio.toString());            
                row.put("fechaFin",fechaFin.toString());            
                row.put("plazo",plazo.toString());            
                row.put("spread",spread.toString());            
                row.put("fraccionLibor",fraccionLibor.toString());            
                row.put("tasaPrepago",tasaPrepago.toString());            
                row.put("montoPenalidad",montoPenalidad.toString());            
                row.put("intereses",intereses.toString());            
                row.put("idTrePrepagoContrato",idTrePrepagoContrato.toString());            
                row.put("contratoDesembolso",contratoDesembolso.toString());            
                row.put("moneda",moneda.toString());            
                row.put("resolucion",resolucion.toString()); 
                row.put("montoPrepago",montoPrepago.toString()); 
                row.put("proximoPago",proximoPago.toString());
                
                tblRow.add(row);
            }
            map.put("TBLPENALIDAD", tblRow); 
            int estimado = tblRow.size();
            logger.log(ADFLogger.WARNING, "estimado tblRow: " + estimado);
            logger.log(ADFLogger.WARNING, "Finalizo TablaPenalidad");
    ...
    public Boolean validarCambiosContratoDesembolsoPrepago(Long idPrepago){
        ...
        try {
            ...
            for(Row row : trePrepagoContratoImpl.getAllRowsInRange()){
                logger.warning("trePrepagoContratoImpl> ");
                String contratoFlexcube = null;
                BigDecimal montoPrepago = null;
                ...
                for(Row rowContratoDesembolso : contratoDesembolsoPrepagoVOImpl.getAllRowsInRange()){
                    ...
                    for(Row row : trePrepagoContratoImpl.getAllRowsInRange()){
                        ...
                        logger.warning("contratoFlexcube> " + contratoFlexcube);
                        logger.warning("montoPrepago> " + montoPrepago);
                logger.warning("contratoDesembolsoPrepagoVOImpl> ");
                    logger.warning("IdHijo> " + rowContratoDesembolso.getAttribute("IdHijo"));
                    if(new Integer("0").equals(rowContratoDesembolso.getAttribute("IdHijo"))){
                        ...
                        logger.warning("contratoFlexcubeComparar> " + contratoFlexcubeComparar);
                        logger.warning("montoPrepagoComparar> " + montoPrepagoComparar);
                        
                        if(contratoFlexcube.equals(contratoFlexcubeComparar) && !montoPrepago.equals(montoPrepagoComparar)){
                            logger.warning("contratoFlexcube.equals(contratoFlexcubeComparar) && !montoPrepago.equals(montoPrepagoComparar)> true");
                            return Boolean.TRUE;
                        }
                    }
                }
        ...
        logger.warning("contratoFlexcube.equals(contratoFlexcubeComparar) && !montoPrepago.equals(montoPrepagoComparar)> false");
...
//                                                             totalCargoPrepagoLocal,tipoMonedaCargoLocal,idTcaTipoResolucion);
    ...
    private String getWsdl(String iWsdlLocation) {
        logger.warning("Inicio getWsdl");
        FenixAMImpl fenixAM = null;
        String wsdl = null;
        try{
            fenixAM = (FenixAMImpl)this.getRootApplicationModule();
            wsdl = fenixAM.getWsdl(iWsdlLocation);
        }
        catch(Exception e){
            wsdl = "http://osblnx1-vip.dev.bcie.org:7021/Entidad/PrepagoService_PS/V1.0?WSDL";
            logger.warning("Error getWsdl.", e);
        }
        logger.warning("Fin getWsdl: " + wsdl);
        return wsdl;
    }
    ...
    /**
     * Container's getter for TcaTipoMonedaDesembolsoVO1.
     * @return TcaTipoMonedaDesembolsoVO1
     */
    public TcaTipoMonedaDesembolsoVOImpl getTcaTipoMonedaDesembolsoVO() {
        return (TcaTipoMonedaDesembolsoVOImpl) findViewObject("TcaTipoMonedaDesembolsoVO");
    }

    public List<Map<String, Object>> ObtenerInteresesPrepago(BigDecimal idPrepago)
    {
        logger.log(ADFLogger.WARNING, "Inicio ObtenerInteresesPrepago");

        String wsdl = getWsdl(IWsdlLocation.REPORTE_PREPAGO);
        PrepagoPT12BndQSService prepagoPT12BndQSService = IWsdlLocation.Service.getInstance(PrepagoPT12BndQSService.class, wsdl);
        PrepagoPT prepagoPT = prepagoPT12BndQSService.getPrepagoPT12BndQSPort();
        
        logger.log(ADFLogger.WARNING, "ObtenerInteresesPrepago prepagoPT12BndQSService");
        
        CalculoInteresPrepagoRequestType request = new CalculoInteresPrepagoRequestType();
        request.setPPREPAGO(idPrepago);
         
        CalculoInteresPrepagoResponseType response = prepagoPT.calculoInteresPrepago(request);
         
        List<CalculoInteresPrepagoInteresType> ptblinteresesitem = response.getPTBLINTERESES().getPTBLINTERESESITEM();
        
        List<Map<String, Object>> lista = new ArrayList<Map<String, Object>>();
        
        for(CalculoInteresPrepagoInteresType  item : ptblinteresesitem)
        {
            Map<String, Object> map = new HashMap<String, Object>();
            System.out.println("----------------------------------------------------------");
            String Vcontractdesembolso = item.getCONTRACTDESEMBOLSO(); 
            String Vbase = item.getBASE(); 
            BigDecimal Vtasa = item.getTASA(); 
            BigDecimal Vmontoprepagar = item.getMONTOPREPAGAR(); 
            XMLGregorianCalendar Vfechadesde = item.getFECHADESDE(); 
            XMLGregorianCalendar Vfechahasta = item.getFECHAHASTA(); 
            BigDecimal Vdiascalculados = item.getDIASCALCULADOS(); 
            String Vmoneda = item.getMONEDA(); 
            BigDecimal Vintereses = item.getINTERESES(); 
            BigDecimal Vidtreprecont = item.getIDTREPRECONT(); 
            BigDecimal Vespagototal = item.getESPAGOTOTAL(); 
            XMLGregorianCalendar Vfechaproximopago = item.getFECHAPROXIMOPAGO(); 
            
            map.put("CONTRACT_DESEMBOLSO", Vcontractdesembolso);
            map.put("BASE", Vbase);
            map.put("TASA", Vtasa);
            map.put("MONTO_PREPAGAR", Vmontoprepagar);
            map.put("FECHA_DESDE", Vfechadesde);
            map.put("FECHA_HASTA", Vfechahasta);
            map.put("DIAS_CALCULADOS", Vdiascalculados);
            map.put("MONEDA", Vmoneda);
            map.put("INTERESES", Vintereses);
            map.put("ID_TRE_PRE_CONT", Vidtreprecont);
            map.put("ES_PAGO_TOTAL", Vespagototal);
            map.put("FECHA_PROXIMO_PAGO", Vfechaproximopago);
            
            lista.add(map);
        }

        logger.log(ADFLogger.WARNING, "Fin ObtenerInteresesPrepago"); 
        return lista;

    }


    public List<Map<String, Object>> ObtenerDetalleDesembolsosPrepago(
        Long idOperacion,
        ...
        logger.log(ADFLogger.WARNING, "Inicio ObtenerDetalleDesembolsosPrepago");

        String wsdl = getWsdl(IWsdlLocation.REPORTE_PREPAGO);
        PrepagoPT12BndQSService prepagoPT12BndQSService = IWsdlLocation.Service.getInstance(PrepagoPT12BndQSService.class, wsdl);
        PrepagoPT prepagoPT = prepagoPT12BndQSService.getPrepagoPT12BndQSPort();
        
        logger.log(ADFLogger.WARNING, "ObtenerDetalleDesembolsosPrepago prepagoPT12BndQSService");
         
        DetalleContratoDesembolsoRequestType request = new DetalleContratoDesembolsoRequestType();
        
        
        BigDecimal VidOperacion = new BigDecimal(idOperacion.toString());        
        BigDecimal VidTcaTipoResolucion = new BigDecimal(idTcaTipoResolucion.toString());        
        BigDecimal VidTcaTipoMoneda = new BigDecimal(idTcaTipoMoneda.toString());        
        BigDecimal VidPrepago = new BigDecimal(idPrepago.toString());        
        
        GregorianCalendar cfechaRenovacion = new GregorianCalendar();
        XMLGregorianCalendar VfechaRenovacion = null;
        if(fechaRenovacion != null){
            cfechaRenovacion.setTime(fechaRenovacion);
            VfechaRenovacion = DatatypeFactory.newInstance().newXMLGregorianCalendar(cfechaRenovacion);
        }
        
        GregorianCalendar cfechaPrepago = new GregorianCalendar();
        cfechaPrepago.setTime(fechaPrepago);
        XMLGregorianCalendar VfechaPrepago = DatatypeFactory.newInstance().newXMLGregorianCalendar(cfechaPrepago);
        
        logger.warning("VidOperacion: " + VidOperacion);        
        logger.warning("VidTcaTipoResolucion: " + VidTcaTipoResolucion);        
        logger.warning("VidTcaTipoMoneda: " + VidTcaTipoMoneda);        
        logger.warning("VidPrepago: " + VidPrepago);        
        logger.warning("VfechaRenovacion: " + VfechaRenovacion);        
        logger.warning("VfechaPrepago: " + VfechaPrepago);       
        logger.warning("busquedaSinTrePrepagoContrato: " + busquedaSinTrePrepagoContrato);
        
        request.setPIDOPERACION(VidOperacion);
        request.setPIDRESOLUCION(VidTcaTipoResolucion);
        request.setPIDMONEDA(VidTcaTipoMoneda);
        request.setPIDPREPAGO(VidPrepago);
        request.setPFRENOVACION(VfechaRenovacion);
        request.setPFPREPAGO(VfechaPrepago);
        request.setPSINTREPREPAGOCONTRATO(busquedaSinTrePrepagoContrato);
        
         
        DetalleContratoDesembolsoResponseType response = prepagoPT.detalleContratoDesembolso(request);
         
        List<TDESEMBOLSOPREPAGO> ptbldesembolsositem = response.getPTBLDESEMBOLSOS().getPTBLDESEMBOLSOSITEM();
        
        List<Map<String, Object>> lista = new ArrayList<Map<String, Object>>();
        
        for(TDESEMBOLSOPREPAGO  item : ptbldesembolsositem)
        {
            Map<String, Object> map = new HashMap<String, Object>();
            System.out.println("----------------------------------------------------------");
            BigDecimal Vidlineacredito = item.getIDLINEACREDITO();
            BigDecimal Vidcontrato = item.getIDCONTRATO();
            String Vnumerolineacredito = item.getNUMEROLINEACREDITO();
            String Vdescripcionlinea = item.getDESCRIPCIONLINEA();
            String Vidflexcube = item.getIDFLEXCUBE();
            BigDecimal Vmontolinea = item.getMONTOLINEA();
            BigDecimal Vidcontratodesembolso = item.getIDCONTRATODESEMBOLSO();
            String Vcontratodesembolso = item.getCONTRATODESEMBOLSO();
            BigDecimal Vidresolucion = item.getIDRESOLUCION();
            String Vresolucion = item.getRESOLUCION();
            XMLGregorianCalendar Vfechaefectiva = item.getFECHAEFECTIVA();
            XMLGregorianCalendar Vvencimiento = item.getVENCIMIENTO();
            XMLGregorianCalendar Vproximopago = item.getPROXIMOPAGO();
            String Vmoneda = item.getMONEDA();
            BigDecimal Vmontoprepago = item.getMONTOPREPAGO();
            BigDecimal Vespagototal = item.getESPAGOTOTAL();
            String Vfondocontable = item.getFONDOCONTABLE();
            XMLGregorianCalendar Vescrituracion = item.getESCRITURACION();
            BigDecimal Vcapitalnvfechaprepa = item.getCAPITALNVFECHAPREPA();
            String Vuserrefno = item.getUSERREFNO();
            
            logger.warning("ptbldesembolsositem Vidflexcube: " + Vidflexcube);
            
            map.put("IDLINEACREDITO", Vidlineacredito);
            map.put("IDCONTRATO", Vidcontrato);
            map.put("NUMEROLINEACREDITO", Vnumerolineacredito);
            map.put("DESCRIPCIONLINEA", Vdescripcionlinea);
            map.put("IDFLEXCUBE", Vidflexcube);
            map.put("MONTOLINEA", Vmontolinea);
            map.put("IDCONTRATODESEMBOLSO", Vidcontratodesembolso);
            map.put("CONTRATODESEMBOLSO", Vcontratodesembolso);
            map.put("IDRESOLUCION", Vidresolucion);
            map.put("RESOLUCION", Vresolucion);
            map.put("FECHAEFECTIVA", Vfechaefectiva);
            map.put("VENCIMIENTO", Vvencimiento);
            map.put("PROXIMOPAGO", Vproximopago);
            map.put("MONEDA", Vmoneda);
            map.put("MONTOPREPAGO", Vmontoprepago);
            map.put("ESPAGOTOTAL", Vespagototal);
            map.put("FONDOCONTABLE", Vfondocontable);
            map.put("ESCRITURACION", Vescrituracion);
            map.put("CAPITALNVFECHAPREPA", Vcapitalnvfechaprepa);
            map.put("USERREFNO", Vuserrefno);
            
            lista.add(map);
        }

        logger.log(ADFLogger.WARNING, "Fin ObtenerDetalleDesembolsosPrepago"); 
        return lista;
