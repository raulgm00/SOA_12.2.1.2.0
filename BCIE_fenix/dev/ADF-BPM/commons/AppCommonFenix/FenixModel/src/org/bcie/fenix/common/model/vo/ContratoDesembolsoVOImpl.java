...
import org.bcie.desembolsomo.InicioCancelarInstanciaRequestType;
import org.bcie.desembolsomo.InicioCancelarInstanciaResponseType;
...
//        request.setInfoDetalleCliente(Boolean.TRUE);
    ...
    public Map obtenerDatosConsultarDesembolso(Long idContrato) {
        ...
        try{
            logger.warning("wsdl desembolso: " + wsdl);
            desembolsoService =  IWsdlLocation.Service.getInstance(DesembolsoPTSOAP12BindingQSService.class, wsdl);
            //logger.warning("Location desembolsoService: " + desembolsoService.getClass().getProtectionDomain().getCodeSource().getLocation());
            if (desembolsoService != null) {
                if (desembolsoService.getClass() != null) {
                    if (desembolsoService.getClass().getProtectionDomain() != null) {
                        if (desembolsoService.getClass().getProtectionDomain().getCodeSource() != null) {
                            if (desembolsoService.getClass().getProtectionDomain().getCodeSource().getLocation() != null) {
                                logger.warning("desembolsoService Locaation:" +desembolsoService.getClass().getProtectionDomain().getCodeSource().getLocation());
                            } else {
                                logger.warning("desembolsoService.getClass().getProtectionDomain().getCodeSource().getLocation() is null:");
                            }    
                        } else {
                            logger.warning("desembolsoService.getClass().getProtectionDomain().getCodeSource() is null:");
                        }      
                    } else {
                        logger.warning("desembolsoService.getClass().getProtectionDomain() is null:");
                    }      
                } else {
                    logger.warning("desembolsoService.getClass() is null:");
                }   
            } else {
                logger.warning("desembolsoService is null:");
            }
            ...
            logger.warning("*Error, la definicion del WSDL_DESEMBOLSO no esta disponible ->",e.getMessage());
    ...
    */
    ...
    public Boolean cambiarEstadoContratoValidacion(Long idContrato, Boolean requiereValidacionAsignacion){
        logger.warning("Inicia metodo cambiarEstadoContratoValidacion "+idContrato);
        Row row = null;
        Boolean resultado = Boolean.FALSE;
        ...
        if(getEstimatedRowCount() > 0){
            ...
            if(null != row){
                ...
                try{
                    ...
                    logger.warning("No se pudo obtener el idEstado del contrato "+idContrato);
                }
                logger.warning("Estado del contrato: " + idEstado);
                    
                if(null != idEstado){
                    logger.warning("Evaluando el estado del contrato "+idContrato);
                    if(!idEstado.equals(FenixModelConstants.ID_ESTADO_CONTRATO_DESEMBOLSO_DESESTIMADO)){
                        if(null != requiereValidacionAsignacion){
                            if(requiereValidacionAsignacion){
                                logger.warning("RequiereValidacionAsignacion es TRUE, cambiando estado a Validacion de asignacion. "+idContrato);
                                row.setAttribute("IdTcaEstado", FenixModelConstants.ID_ESTADO_CONTRATO_DESEMBOLSO_VALIDACION_RECURSOS);
                            }else{
                                logger.warning("RequiereValidacionAsignacion es FALSE, cambiando estado a Cumplimiento de condiciones. "+idContrato);
                                row.setAttribute("IdTcaEstado", FenixModelConstants.ID_ESTADO_CONTRATO_DESEMBOLSO_CUMPLIMIENTO_CONDICIONES);
                            }
                        }else{
                            logger.warning("RequiereValidacionAsignacion es NULL, cambiando estado a Cumplimiento de condiciones. "+idContrato);
                            row.setAttribute("IdTcaEstado", FenixModelConstants.ID_ESTADO_CONTRATO_DESEMBOLSO_CUMPLIMIENTO_CONDICIONES);
                        }
                        resultado = Boolean.TRUE;
                    }else{
                        logger.warning("El contrato esta desestimado. "+idContrato);
                    }
                }else{
                    logger.warning("El estado del contrato es NULL "+idContrato);
                }
            }else{
                logger.warning("No existe registro de contrato con id: " + idContrato);
            }
        ...
        logger.warning("Termina metodo cambiarEstadoContratoValidacion "+idContrato);
        return resultado;
    }
...
//        String idFondoLinea = null;
    ...
    public Boolean iniciarCancelarInstanciaDesembolso(final Long idOperacion,final Long idSolicitud,final Long idContrato,final String responsable){
        logger.warning("Inicia iniciarCancelarInstanciaDesembolso. "+idOperacion);
        ...
        try {
            ...
            final InicioCancelarInstanciaRequestType rq= new InicioCancelarInstanciaRequestType();
            final Header header = new Header();
            final org.bcie.xmlns.objetoproceso.comun.operacion.v1.Operacion operacion = new org.bcie.xmlns.objetoproceso.comun.operacion.v1.Operacion();
            ParameterType parameterType = null;
            if(idOperacion!=null){
                operacion.setCodigoOperacion(idOperacion.intValue());
                logger.warning("Se envia operacion: - "+idOperacion);
            }
            if(responsable!=null){
                operacion.setResponsableOperacion(responsable);
                logger.warning("Se envia loginUsuario:"+responsable+" - "+idOperacion);
            }
            if(idSolicitud!=null){
                parameterType = new ParameterType();
                parameterType.setParameterName(ContratoDesembolsoVOImpl.PARAMETER_TYPE_SOLICITUD);
            ...
            if(idContrato!=null){
                parameterType = new ParameterType();
                parameterType.setParameterName(ContratoDesembolsoVOImpl.PARAMETER_TYPE_DESEMBOLSO);
            ...
            rq.setHeader(header);
            xmlEntrada = fenixAmImpl.writeXMLRequest(rq, rq.getClass());
            logger.log(ADFLogger.WARNING, ">HNWS" + xmlEntrada.toString()); 
            final InicioCancelarInstanciaResponseType rs=port.inicioCancelarInstancia(rq);
            xmlSalida = fenixAmImpl.writeXMLRequest(rs, rs.getClass());
            logger.log(ADFLogger.WARNING, ">HNWS" + xmlSalida.toString());
            logger.warning("Luego de llamar inicioCancelarInstancia. "+rs+" - "+idOperacion);
            if (null != rs.getResultado().getError() || (rs.getResultado().getResult() != null && rs.getResultado().getResult().value() == "ERROR")) {
                String mensajeError = "";
                if (null != rs.getResultado().getMessage() && rs.getResultado().getMessage().trim().length() > 0) {
                    mensajeError = rs.getResultado().getMessage();
                }
                existeError = Boolean.TRUE;
                msgError = "Servicio inicioCancelarInstancia devuelve ERROR. " + "Mensaje: " + mensajeError;
                exito = Boolean.FALSE;
            }
            ...
            exito = Boolean.FALSE;
            existeError = Boolean.TRUE;
            logger.log(ADFLogger.ERROR, "Error en iniciarCancelarInstanciaDesembolso ", e);

            JboException ex = new JboException(e);
            ex.addToExceptions(new Exception("Error en iniciarCancelarInstanciaDesembolso " + e.getClass() + ":" + e.getMessage()));
            throw ex;
        ...
        logger.warning("Termina iniciarCancelarInstanciaDesembolso. "+idOperacion);
        return exito;  
    }
}
