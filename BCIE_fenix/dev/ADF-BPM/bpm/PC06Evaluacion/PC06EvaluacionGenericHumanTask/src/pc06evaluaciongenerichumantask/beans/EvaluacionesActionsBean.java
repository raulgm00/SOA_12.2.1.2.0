        ...
        if (validacionesCA()) {
            ...
            try {
                ...
                case FenixConstants.PC06_GENERARINFORMES:
                    if (evaluacionesBean.getTipoEvaluacion() != null && evaluacionesBean.getTipoEvaluacion().trim().length() > 0) {
                        if (evaluacionesBean.getTipoEvaluacion().equalsIgnoreCase("IBCIE")) {
                            esValidaFinalizarTarea = validateDocumento(getIdOperacion(),FenixConstants.DOCUMENTO_I_BCIE,idTarea);
