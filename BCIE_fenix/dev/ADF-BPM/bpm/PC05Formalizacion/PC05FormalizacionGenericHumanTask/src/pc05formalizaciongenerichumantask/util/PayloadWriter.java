...
public final class PayloadWriter {
    ...
    private static final ADFLogger LOGGER = ADFLogger.createADFLogger(PayloadWriter.class);
    private Task task;
    ...
    private void updateParameterType(final Node paramTypeNode, final String value) {
        ...
        Node paramTypeValue = findParameterTypeChildNode(paramTypeNode, PAYLOAD_PARAMETERTYPE_VALUE_NODE);
    ...
    private void createParameterType(final String name, final String value, final XMLElement taskPayload) {
        ...
        Element newParamTypeNameNode = doc.createElement(PAYLOAD_PARAMETERTYPE_NAME_NODE);
        newParamTypeNameNode.setTextContent(name);

        Element newParamTypeValueNode = doc.createElement(PAYLOAD_PARAMETERTYPE_VALUE_NODE);
        newParamTypeValueNode.setTextContent(value);

        Element newParamTypeNode = doc.createElement(PAYLOAD_PARAMETERTYPE_NODE);
    ...
    private Node findParameterType(final String name, final XMLElement taskPayload) {
        ...
        NodeList nl = taskPayload.getElementsByTagName(PAYLOAD_PARAMETERTYPE_TAGNAME);
        if (nl == null) {
            ...
            if (nl.getLength() == 0) {
                ...
                for (int i = 0; i < nl.getLength(); i++) {
                    ...
                    Node paramNameNode = findParameterTypeChildNode(paramTypeNode, PAYLOAD_PARAMETERTYPE_NAME_NODE);
