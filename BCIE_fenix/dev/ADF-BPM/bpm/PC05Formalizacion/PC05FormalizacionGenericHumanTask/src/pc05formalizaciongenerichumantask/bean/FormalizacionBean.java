...
public class FormalizacionBean extends FenixPanelBean implements Serializable {
    ...
    public static final String ACTUALIZO_LINEA_CREDITO = "actualizoLineaCredito";
    /**
    ...
    private Long idLote; //FNXII-6103 solo aplica para cuando incia proceso de Foramlizaci?n
    private Boolean esRetorno;
    private Boolean actualizoLineaCredito;

    //Variables custodiar contrato
    private String numeroCustodia;

    // Renders de p?gina Definir recursos externos y condiciones
    private Integer cuentaConRecExternos; // Radio "Se cuenta con recursos externos para esta operaci?n"
    
    // Renders de p?gina Adjuntar contrato firmado
    private Boolean esFechaVigenciaIgualFechaFirma = Boolean.FALSE; // Checkbox Fecha de vigencia es igual a fecha de firma.
    
    // Renders de p?gina Asignaci?n de recursos
    ...
    private Boolean mostrarDerivadosImplicitos = Boolean.FALSE;
    // Feature 20: Formulario de Derivado Implicitos
    private Cuestionario objCuestionario;
    
    /** 
     * Mapa que contiene las l?neas de cr?dito que se almacenan en el payload. 
     * Key=ID L?nea cr?dito, Value = Estado L?nea
     */
    private Map<String, String> mapEstadoLineas;

    /** Prefijo usado en el Payload del proceso para almacenar los valores de estados de l?neas de cr?dito.*/
    public static final String PALYOAD_PREFIXLINEAS = "$LINEAS$";
    ...
    public void getPayloadInformation() throws WorkflowException, IOException { 
       ...
       // instanciaProceso ="4277246";
		...
		if (getIdProducto().equals(FenixConstants.PRESTAMO_TRADICIONAL)) {
                    setEstaConfiguradoProducto(1);
                } else {
                    setEstaConfiguradoProducto(0);
                }
            }
        }
        // US :2012
        ...
        nl = xmlPayload.getElementsByTagName( PAYLOAD_PARAMETERTYPE_TAGNAME);
        if (nl != null) {

            if (nl.getLength() > 0) {
                this.mapEstadoLineas = this.readParameterTypeLineas(nl, PALYOAD_PREFIXLINEAS);
            } else {
                this.mapEstadoLineas = Collections.emptyMap();
            }
        } else {
            this.mapEstadoLineas = Collections.emptyMap();
        }  
        ...
        nl = xmlPayload.getElementsByTagName(ACTUALIZO_LINEA_CREDITO);
            if (nl != null) {

                if (nl.getLength() > 0) {
                    setActualizoLineaCredito(Boolean.parseBoolean(nl.item(0).getTextContent()));
                } else {
                    setActualizoLineaCredito(Boolean.FALSE);
                }
            } else {
                setActualizoLineaCredito(Boolean.FALSE);
            }
        ...
        logger.log(ADFLogger.WARNING, "actualizoLineaCredito: " + getActualizoLineaCredito());
        logger.log(ADFLogger.WARNING, "mapEstadoLineas: " + getMapEstadoLineas());
        
        
   
            final DCBindingContainer bindingsVO = (DCBindingContainer) BindingContext.getCurrent().getCurrentBindingsEntry();
               final DCIteratorBinding dcIterator = bindingsVO.findIteratorBinding("CatProductoVOIterator");
               final RowSetIterator it = dcIterator.getViewObject().createRowSetIterator(null);
               if(it!=null){
                   it.reset();
                   logger.warning("Antes de recorrer el iterador "+it.getRowCount());
                   while(it.hasNext()){
                       final Row item=it.next();
                       final Integer id=(Integer)item.getAttribute("Id");
                       final Integer RequiereDerivadosImplicitos=(Integer)item.getAttribute("RequiereDerivadosImplicitos");
                       logger.warning("Id:"+id+"-RequiereDerivadosImplicitos:"+RequiereDerivadosImplicitos);
                       
                       if(RequiereDerivadosImplicitos!=null){
                           if(getIdProducto().equals(id)&&RequiereDerivadosImplicitos.equals(1)){
                                   setMostrarDerivadosImplicitos(Boolean.TRUE);
                                   break;
                           }
                       }
                   }
        ...
        logger.log(ADFLogger.WARNING, "mostrarDerivadosImplicitos: " + getMostrarDerivadosImplicitos());
    ...
    public void obtenerNumeroResolucion() {
        ...
        /**
     * Busca la parametrizaci?n de los botones definidos para la tarea (Dinamicamente)
    ...
    public void obtenerDatosTablaDinamica() {
        ...
        //Validar siempre y cuando sea retorno sobre la configuraci?n de los productos a dinamicos a conciderar
        ...
        try {
            ...
            logger.warning("Valor de ban: " + ban );
                ...
                /*logger.log(ADFLogger.WARNING, "La ejecuci?n del m?todo BINDINGS ? fue ejecutado con exito?: " + mapaObjetos);*/
                        ...
                        if (null != mapaObjetos) {
                            ...
                            if (mapaDinamicoLocal.size() > 1) {
                               ...
                               setConfigurtacionVisibilidad(mapaDinamicoLocal, ban);
                                ...
                                /*Seteo de visibilidad normal de b?tones*/
                    ...
                    setConfigurtacionVisibilidad(null, Boolean.FALSE);
                    logger.log(ADFLogger.ERROR, "Ocurrio un error al ejecutar la operaci?n obtenerDatos directamente de los bindings: " + e);
            ...
            /*Seteo de visibilidad normal de b?tones*/
            setConfigurtacionVisibilidad(null, Boolean.FALSE);
            logger.log(ADFLogger.ERROR, "Error en ejecutar el m?todo obtenerDatosTablaDinamica : " + e);
       }
        }else{
            logger.warning("Valor de la operacion : " + Long.valueOf(getIdOperacion()));
            logger.warning("Valor de IdTarea: " + this.getIdTarea());
            logger.log(ADFLogger.WARNING, "Se aplicar? la configuraci?n por default de botones");
            setConfigurtacionVisibilidad(null, Boolean.FALSE);
        }
             ...
             for(Map.Entry <String,TablaDinamicaVOBean> entry : m.entrySet()){
                ...
                String a = entry.getValue().getProductosConfigurados();
                 ...
                 for(String producto: arrayTiposProducto){
                     ...
                     if( this.getIdProducto() ==  Integer.parseInt(producto) ){
                         enviar = Boolean.TRUE;
                         this.setEstaConfiguradoProducto(1);
                         logger.log(ADFLogger.WARNING, "Producto encontrado :"  + Integer.parseInt(producto) + " Accion: " + entry.getValue().getAccion());
                         break;
                     }else{
                 ...
                 if(esActivo){
                     ...
                     //Configuraci?n por default de botosnes
    ...
    public void asignacionVisibilidadPorProducto(Map.Entry <String, TablaDinamicaVOBean> entry){
        ...
        logger.log(ADFLogger.WARNING, "Comienza configuraci?n de botones U2012");
        ...
        try{        
                ...
                switch (accionBtn) {
                // Renderiza conforme a al configuraci?n de la base de datos.
    ...
    ???? * hay condiciones con el estado por validar previo a formalizacion
    ???? * @param actionEvent
    ???? * Feature 6053
    ???? * @since 06/09/2021
    ???? */
    public void iniciaCumplimientoCondicionPreviaFormalizar() {
        logger.log(ADFLogger.WARNING, "INTO iniciaCumplimientoCondicionPreviaFormalizar");

        Boolean validar = Boolean.FALSE;
        Long idOperacion = null;
        idOperacion = Long.parseLong(getIdOperacion());
        ...
        try {
            ...
            logger.log(ADFLogger.ERROR,
                       "Error en iniciaCumplimientoCondicionPreviaFormalizar." + e.getClass() + "." + e.getMessage());
        }
    }     
    ...
    public void setMostrarDerivadosImplicitos(Boolean mostrarDerivadosImplicitos) {
        this.mostrarDerivadosImplicitos = mostrarDerivadosImplicitos;
    }

    public Boolean getMostrarDerivadosImplicitos() {
        return mostrarDerivadosImplicitos;
    }
    
    public Map<String, String> getMapEstadoLineas() {
        return mapEstadoLineas;
    }

    public void setMapEstadoLineas(Map<String, String> mapEstadoLineas) {
        this.mapEstadoLineas = mapEstadoLineas;
    }
    
    private Map<String, String> readParameterTypeLineas(NodeList nl, String prefix){
        logger.warning("Dentro de readParameterTypeDesobligar");
        ...
        if (nl != null && nl.getLength() > 0) { 
            ...
            for (int i = 0; i < nl.getLength(); i++) {
                ...
                for (int j = 0; j < paramChildNodes.getLength(); j++) {
                    ...
                    if (nodeName != null) {
                        if (nodeName.equalsIgnoreCase(PAYLOAD_PARAMETERTYPE_NAME_NODE)) {
                            paramName = paramChildNodes.item(j);
                        } else if (nodeName.equalsIgnoreCase(PAYLOAD_PARAMETERTYPE_VALUE_NODE)) {
                    ...
                    if (paramName != null && paramName.getTextContent() != null &&
                        ...
                        String montoDesobligar = null;
                        if (paramValue != null && paramValue.getTextContent() != null) {
                            try {
                                montoDesobligar =  paramValue.getTextContent();
                            } catch (Exception ex) {  
                            }
                        }
        ...
        logger.warning("===Mapa de montos a desobligar leido del payload===");
        logger.warning("{0}", map);
        logger.warning("===================================================");
    ...
    public void setActualizoLineaCredito(Boolean actualizoLineaCredito) {
        this.actualizoLineaCredito = actualizoLineaCredito;
    }

    public Boolean getActualizoLineaCredito() {
        return actualizoLineaCredito;
    }
