...
public class FormalizacionActionBean extends FenixActionBean implements Serializable {
    ...
    //variables para los popupï¿½s
    private transient RichPopup popupConfirmarFinalizarTarea;
    private transient RichPopup popupConfirmarFinalizarTareaSC;
    ...
    private transient RichPopup popupAgregarModificarFuente; // Usado en pantalla Asignaciï¿½n de recursos
    ...
    private transient RichButton verDocumentoButtonUIC;
   ...
   private List<String> listaAdvertencias = new ArrayList<String>(); 
    ...
    public void setPopupConfirmarFinalizarTareaSC(RichPopup popupConfirmarFinalizarTareaSC) {
        this.popupConfirmarFinalizarTareaSC = popupConfirmarFinalizarTareaSC;
    }

    public RichPopup getPopupConfirmarFinalizarTareaSC() {
        return popupConfirmarFinalizarTareaSC;
    }

    /*** START Pantalla solicitar elaboraciï¿½n de documentaciï¿½n contractual ***/
    ...
    public String invokeFinalizarTareaCCPF() {
        if (!validarSeleccionCondicion()){          
            //mostrar Popup de confirmacion para finalizar la tarea sin condiciones 
            RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
            getPopupConfirmarFinalizarTareaSC().show(popupHints);            
        }else{
            //mostrar Popup de confirmacion para finalizar la tarea
            RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
            getPopupConfirmarFinalizarTarea().show(popupHints);
    ...
    public Boolean validarSeleccionCondicion() {
        logger.log(ADFLogger.WARNING, "INTO validarSeleccionCondicion.");
        ...
        if (null != formalizacionBean.getIdOperacion()) {
            idOperacion = Long.parseLong(formalizacionBean.getIdOperacion());
        ...
        try {
            ...
            if (null != condicionIncumplimientoResultadoVO) {
                ...
                if (SeleccionaRows.length > 0 && null != idOperacion) {
            ...
            resultado = Boolean.FALSE;
            logger.log(ADFLogger.ERROR, "Error en validarSeleccionCondicion." + e.getClass() + "." + e.getMessage());
        }
        return resultado;
    }
    ...
    public String aceptarFinalizarTarea() {
        // Invocar mï¿½todo para cargar documentos en onBase
    ...
    public String aceptarFinalizarTareaCCF() {
        ...
        if (idOperacion != null) {
            if (opValidarJustificacion != null) {
                ...
                errorMessage = "Error: Al validar la justificaciï¿½n.";
            }
        } else {
            errorMessage = "Error: Operaciï¿½n Invalida.";
        }
        ...
        if (errorMessage == null) {
            ...
            if (null != justification && justification > 0) {
                ...
                JSFUtils.addFacesErrorMessage("Debe ingresar una justificaciï¿½n para finalizar.");
            }
    ...
    /**
     * Aceptar finalizar tarea SIN Condiciones Previas a Formalizar.
     * @return action
     */
    public String aceptarFinalizarTareaSCPF() {
        logger.log(ADFLogger.WARNING, "Inside aceptarFinalizarTareaSCPF.");
        InvokeActionBean invokeActionBean = null;       
        String returnAction = null;
        invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue("invokeActionBean");
        returnAction = invokeActionBean.invokeOperation();
    ...
    /**
     * Aceptar finalizar tarea Cumplmiento de Condiciones Previas a Formalizar.
     * @return action
     */
    public String aceptarFinalizarTareaCCPF() {
            logger.log(ADFLogger.WARNING, "Inside aceptarFinalizarTareaCCPF.");
        ...
        try {
            ...
            logger.log(ADFLogger.ERROR,
                       "Error en aceptarFinalizarTareaCCPF." + e.getClass() + "." + e.getMessage());
        }
    ...
    public Boolean agregarCondicionById() {
        ...
        try {
            OperationBinding operationBinding = bindings.getOperationBinding("agregarCondiciones1");
            DCIteratorBinding condicionIncumplimientoVOIterator = null;
            ViewObject condicionIncumplimientoResultadoVO = null;
    ...
    /*** END Pantalla solicitar elaboraciï¿½n de documentaciï¿½n contractual ***/
    ...
    public String aceptarFinalizarTareaRecExternosAction() {
        ...
        case SI:
            //RN01 En caso de que el Analista DAECI indique que la operaciï¿½n requiere de recursos externos se debe de
    ...
    public void recursosExternosValueChangeListener(ValueChangeEvent valueChangeEvent) {
        ...
        // Si eligiï¿½ la opciï¿½n de NO y existen fuentes agregadas, advertimos al usuario que serï¿½n eliminadas
    ...
    public void confirmEliminarFuentesAgregadasActionListener(ActionEvent actionEvent) {
        ...
        // Eliminamos de BD todas las fuentes agregadas (borrado lï¿½gico)
    ...
    public String aceptarFinalizarTareaBorradorContratoAction() {
        ...
        /**@Overview: US: 2012; Se agrega La validaciï¿½n de comantario para poder finalizar la tarea solo para cuando esRetorno
         * @Premisas: Validar si es retonor de la oepraciï¿½n (*COFI) por lo tanto debe contener un comentario de justificaciï¿½n hacia COFI 
         * @Nota: se deja las lineas cpmentadas de la secciï¿½n (1) con el fin de contar con ellas si se desea validar algun docmuento en la tarea
         * @Develper: RUGM
         * @Date: 01/06/2021
         * */
    ...
    /**@Overview: US: 2012; Se agrega mï¿½todo de validaciï¿½n d elogica para la ejecuciï¿½n de finalzar la tarea en retorpectiva de la acciï¿½n
    ...
    public String logicaFinalizarTareaBorradorContratoAction(Boolean esRetorno, String accion, Long idOperacion, String idTcaTareaBpm, String IdInstanciaTarea){
        ...
        // Enviar notificaciï¿½n de comentario cuando se requeira ir a Enviar aCOFI y sea retorno
        if(esRetorno && ( accion.equalsIgnoreCase("EnviarCofi")) ){
            ...
            /* Overview: Complemento de codigo para evitar la validaciï¿½n del comentario para cuandso no es Retorno de proceso
             * Nota: se deja las lineas copmentadas de la secciï¿½n (1) con el fin de contar con ellas si se desea validar algun docmuento en la tarea
             * Develper: RUGM
             * Date: 01/06/2021
             * */
            // RN: El sistema verifica que se cuente con el documento de tipo "Borrador de contrato".
    ...
    public Boolean validarComisionesDispensadasPagadas() {
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            JSFUtils.addFacesErrorMessage("Algunas de las comisiones de la operaciï¿½n deben de tener el estado pagada o dispensada.");
    ...
    private Boolean validarTccElaborarBorradorContrato(Long idOperacion) {
        ...
        // Ejecutamos mï¿½todo que verifica que la combinaciï¿½n de Estados con Sub-estados coincida con el diagrama de estados.
        // Lo anterior, debido a que esta es la ï¿½ltima pantalla donde se pueden editar los TCC.
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            esEjecucionExitosa = Boolean.TRUE; // sub-estados vï¿½lidos.
        else
            JSFUtils.addFacesErrorMessage("Uno o mï¿½s sub-estados de TCC son invï¿½lidos, favor de revisar."); // getResult() es null o false
    ...
    /*** START Pantalla Negociar documentaciï¿½n contractual con el cliente ***/
    public String aceptarFinalizarTareaNegociarDocumentacionAction() {
        ...
        // Consultamos la DJ para la operaciï¿½n, con el fin de obtener el estado de la "No objeciï¿½n"
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            // RN1: Para poder finalizar la tarea es necesario que exista una "No objeciï¿½n" vigente.
            // Nota: la regla anterior sï¿½lo aplica cuando el NombreEstadoNoObjecion sea diferente a "No aplica"
            if ((noObjecion != null) && (noObjecion.trim().equalsIgnoreCase("SI")) ||
                ...
                if (validarCondicionesFormalizacion != null) {
                    ...
                    if (!esCondicionesFormalizar)
                        JSFUtils.addFacesErrorMessage("Para finalizar la tarea, las condiciones a formalizar de la operaciï¿½n deben cumplirse.");

                    // RN3: El sistema valida que el plazo de formalizaciï¿½n se ha vencido.
                ...
                JSFUtils.addFacesErrorMessage("Para finalizar la tarea la operaciï¿½n debe de contar con una ï¿½No Objeciï¿½nï¿½ vigente.");
            }
        }
    ...
    private Boolean validarTccFinalizarNegociarDocumentacion(Long idOperacion) {
        ...
        // Ejecutamos mï¿½todo que realiza RN05: Los elementos de la Matriz TCC que se encuentren en el estado
    ...
    /*** END Pantalla Negociar documentaciï¿½n contractual con el cliente ***/
    ...
    public String aceptarFinalizarTareaElaboraCoreTyCAction() {
        ...
        // RN: Se deberï¿½ haber asociado a la operaciï¿½n el documento de tï¿½rminos y condiciones digital.
        ...
        //}
        //else {
        //    JSFUtils.addFacesErrorMessage("Debe adjuntar el documento de tï¿½rminos y condiciones.");
    ...
    /*** START Pantalla Analizar documentaciï¿½n contractual ***/
    ...
    /*** END Pantalla Analizar documentaciï¿½n contractual ***/
    ...
    public Boolean actualizarComisionesMontoFormalizado() {
        ...
        operationBinding.getParamsMap().put("tipoMonto",
                                            FenixConstants.MONTO_FORMALIZADO); // Monto Aprobado Id = 3, segï¿½n tabla TCA_TIPO_MONTO_OPERACION
        operationBinding.execute();
        if (operationBinding.getErrors().isEmpty()) {
            // Asignaciï¿½n para Monto aprobado
    ...
    public String aceptarFinalizarTareaContratoFirmadoAction() {
        ...
        Long vIdOperacion = formalizacionBean.getOperacionId() != null ? formalizacionBean.getOperacionId().longValue() : null;
        ...
        if (condicionesValidas) {
            if (formalizacionBean.getSolicitudContratacion()) {
                ...
                // RN2: El monto escriturado no deberï¿½ exceder la diferencia del monto aprobado y el monto total de las formalizaciones ya ejecutadas.
            ...
            if (condicionesValidas) {
                // RN1: La fecha de la firma no debe ser posterior al plazo mï¿½ximo para formalizar, incluido en los tï¿½rminos y condiciones de la operaciï¿½n.
                ...
                if (blnValidacion != null && blnValidacion) {
                    blnValidacion =
                        actualizarFechaInicioTerminoTipoPlazo(vIdOperacion);
                    if (blnValidacion != null && blnValidacion) {
                  
                        blnValidacion =
                            validarTccAdjuntarContratoFirmado(vIdOperacion);
                        if (blnValidacion != null && blnValidacion) {
                ...
                if (blnContinuar) {
                    ...
                    guardarDatosEnContrato(vIdOperacion,
                                           formalizacionBean.getInstanciaProceso());
                    if (actualizarComisionesMontoFormalizado()) {

                        // Invocar mï¿½todo para cargar documentos en onBase
                        cargarDocumentosOnBase();
                        // Si eligiï¿½ que la Fecha de vigencia es igual a fecha de firma, actualizamos flag en payload
                        ...
                        operationBinding.getParamsMap().put("idOperacion",vIdOperacion);
                        operationBinding.execute();
            ...
            logger.warning("Error al finalizar tarea, condiciones no validas");
        ...
        if (keyError != null) {
            logger.warning("Error al finalizar tarea");
    ...
    private Boolean validarContratoFirmado() {
        ...
        // RN3: El actor accede a la secciï¿½n "Documentos" y adjunta el archivo del contrato digitalizado.
        ...
        // El monto escriturado deberï¿½ ser menor o igual al monto aprobado.
    ...
    private Boolean validarTccAdjuntarContratoFirmado(Long idOperacion) {
        ...
        // Ejecutamos mï¿½todo que realiza: Todos los elementos TCC que se encuentren en un estado diferente a "Por validar"
    ...
    public String aceptarFinalizarTareaCustodiarContrato() {
        ...
        if (numeroCustodia != null) {
            ...
            //el sistema valida que la informaciï¿½n estï¿½ incompleta.  MSG01
            JSFUtils.addFacesErrorMessage("Debe completar la informaciï¿½n  requerida.");
            //cerramos popup de Confirmar Finalizar Tarea
            getPopupConfirmarFinalizarTarea().hide();
        }
        return returnAction;
    ...
    public String aceptarFinalizarTareaFechaVigenciaAction() {
        ...
        // RN1: La fecha de Vigencia deberï¿½ ser mayor ï¿½ igual a la fecha de firma del contrato.
        // Nota: Fecha de firma nunca deberï¿½a llegar como null y la Fecha de vigencia es requerida en pantalla
        if ((fechaFirma != null) && (fechaVigencia != null) && (fechaVigencia.compareTo(fechaFirma) >= 0)) {
            ...
            JSFUtils.addFacesErrorMessage("La fecha de Vigencia deberï¿½ ser mayor ï¿½ igual a la fecha de firma del contrato.");
    ...
    private Boolean actualizarFechasContrato(Long idOperacion, String instanciaProceso, Timestamp fechaFirma,
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            esEjecucionExitosa = Boolean.TRUE; // Actualizaciï¿½n exitosa en BD
    ...
    private Boolean validarRegistrarLineaCredito() {
        ...
        /*  Gabriel Niï¿½o Rosales 29/02/2016
         *      FNXII-2967 Pruebas-PC05-Formalizaciï¿½n-Estructuraciï¿½n de prestamo
         *      Si el producto es Prï¿½stamo Estructurado, se validara al finalizar que el montoSindicado no sea mayor
         *      al montoEscriturado
         *      Prï¿½stamo Estructurado == 2
        */
        ...
        if (lineasGuardadas == "N") {
            esDatosCorrectos = Boolean.FALSE;
            JSFUtils.addFacesErrorMessage("Debe guardar la lï¿½nea de crï¿½dito para identificar las validaciones pendientes.");
        }
        
        //Feature 20: Formulario de Derivados de Implicitos
        
        if(formalizacionBean.getObjCuestionario().getExisteResultado() && !formalizacionBean.getObjCuestionario().cuestionarioCompletado()){
            esDatosCorrectos = Boolean.FALSE;
            JSFUtils.addFacesErrorMessage(ADFUtils.getStringFromBundle("MENSAJE_VALIDACION_FORMULARIO_DERIVADOS_IMPLICITOS", BUNDLE));
        }
        if(formalizacionBean.getObjCuestionario().getPresionarGuardar()){
            JSFUtils.setExpressionValue("#{bindings.requiereValidarFormulario.inputValue}", Boolean.TRUE);
        }
        
        logger.warning("Finaliza validarRegistrarLineaCredito con esDatosCorrectos en: " + esDatosCorrectos);
        return esDatosCorrectos;
    }
    /*** END Pantalla Registrar datos de la lï¿½nea de crï¿½dito ***/


    /*** START Pantalla Validar datos de lï¿½nea de crï¿½dito ***/
    public String aceptarFinalizarTareaValidarLineaCreditoAction() {

        logger.warning("Inicia aceptarFinalizarTareaValidarLineaCreditoAction. 42472");
        ...
        if (idProducto == FenixConstants.ID_AMPLIACION_LINEA_GLOBAL_CREDITO_IFI) {
            ...
            logger.warning("entra if producto " + idProducto);
            

            operationBinding = bindings.getOperationBinding("getFechaValorContratoByOperacionInstanciaProceso");
            operationBinding.execute();
            Timestamp fechaValorCon = operationBinding.getResult() != null ? (Timestamp) operationBinding.getResult() : null;                        
            logger.warning("- la fechaValorCon: " + fechaValorCon );

            if(fechaValorCon != null) {
                logger.warning("- fechaValor a asignar en propagar: " + fechaValorCon);
                operationBinding = bindings.getOperationBinding("propagarLineaCreditoFechaValor");
                operationBinding.getParamsMap().put("fechaValor", fechaValorCon);
            }else{
                logger.warning("- Sin fechaValorCon -");
                operationBinding = bindings.getOperationBinding("propagarLineaCreditoInstancia");
            }
        ...
        logger.log(ADFLogger.WARNING, "propagarLineaCreditoInstancia(FechaValor) resultado: " + resultado);
    ...
    public String aceptarSolicitarAjustesValidarLineaCreditoAction() {
        ...
        if (validateComentario(Long.valueOf(formalizacionBean.getIdOperacion()), formalizacionBean.getIdTarea(),
            ...
            //EX01
            //MSG01
            JSFUtils.addFacesErrorMessage("Debe ingresar la informaciï¿½n requerida.");
    ...
    /*** END Pantalla Validar datos de lï¿½nea de crï¿½dito ***/


    /*** START Pantalla Asignaciï¿½n de recursos ***/
    public String aceptarFinalizarTareaAsignacionRecursosAction() {
        ...
        // Debe de existir al menos una lï¿½nea de crï¿½dito
        if (validarNumeroLineasCredito()) {

            // Valida que las fuentes de cada lï¿½nea de crï¿½dito para que no tenga mas de dos una fuente sin fecha
            if (validarNoFuentesLineasCredito()) {

                // RN02: Para poder finalizar la tarea el "Monto total en USD" de cada lï¿½nea debe ser igual al total asignado a cada una.
    ...
    private Boolean validarNoFuentesLineasCredito() {
        ...
        try {
            ...
            JSFUtils.addFacesErrorMessage("No se pudo validar el nï¿½mero de fuentes de las lï¿½neas de crï¿½dito.");
            logger.warning(e);
        }
        return result;
    }
    ...
    private Boolean validarNumeroLineasCredito() {
        ...
        if (JSFUtils.resolveExpression("#{bindings.LineaCreditoVO.estimatedRowCount}") != null) {
            ...
            if (numLineasCredito < 1) {
                esDatosCorrectos = Boolean.FALSE;
                JSFUtils.addFacesErrorMessage("Debe registrar al menos una lï¿½nea de crï¿½dito.");
            }
        }
    ...
    private Boolean propagarFuente() {
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            if ((response.getResultado().getResult() != null) &&
                ...
                JSFUtils.addFacesErrorMessage("No se puede realizar la asignaciï¿½n de la lï¿½nea en ARE.");
                ...
                //                JSFUtils.addFacesErrorMessage(message + "Cï¿½digo de error: "
                //                                              + response.getResultado().getError().getErrorCode() + ". Descripciï¿½n: "
    ...
    private void agregarModificarFuenteActionListener(Boolean esModificar) {
        ...
        // Pre-llenamos datos si la operaciï¿½n es modificar
        if (esModificar) {
            formalizacionBean =
                (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
    ...
    private Boolean validarMontoAsignar() {
        // NI07: El valor del monto a asignar no puede ser mayor a la disponibilidad de la lï¿½nea.
        Boolean esDatosCorrectos = Boolean.TRUE;
        DCIteratorBinding voAgregarFuentePorLineaCredito = null;
        ...
        if ((disponibilidad != null) && (montoAsignar != null) && (montoAsignar.compareTo(disponibilidad) > 0) &&
            ...
            esDatosCorrectos = Boolean.FALSE;
            JSFUtils.addFacesErrorMessage("El monto de asignaciï¿½n no puede ser mayor a la cantidad disponible.");
        }
    ...
    public void cancelarAgregarModificarFuenteActionListener(ActionEvent actionEvent) {
        ...
        // Cerramos popup de Agregar/Modificar fuente en pantalla Asignaciï¿½n de recursos
        getPopupAgregarModificarFuente().hide();
    }
    ...
    public void descripcionValueChangeListener(ValueChangeEvent valueChangeEvent) {
        ...
        // Guardamos la descripciï¿½n de la lï¿½nea de crï¿½dito si se modificï¿½
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance()); // update model values
        this.commitData();
    }
    ...
    /*** END Pantalla Asignaciï¿½n de recursos ***/


    /*** START acciones genï¿½ricas ***/
    ...
    /* Overview: Se mdifica el metodo para agregar la acciï¿½n proveniente de ELABORACION DE CONTRATO FINAL
     * Developer: RUGM
     * Date: 01/06/2021
     * */
    public void confirmarFinalizarTareaElaborarBorradorContratoActionListener(ActionEvent actionEvent, String accion) {
        ...
        if (existeCondicion) {
            ...
            /**
             * @Overview: Renderizaciï¿½n de mensajes en PopupConfirmarFinalizarTarea para escenario FinalizarTareaSinCondicion 
             * @Developer RUGM
             * @Date: 01/06/2021
             * */
            switch(accion){
                ...
                case "enviarResponsableOperacion":
                    ...
                    break;
                case "finalizarTarea":
                    // Si no existe condicion es false mostrarï¿½ el mensaje de finalizar tarea sin condicion 
...
//jenamorado 15/07/2021
    ...
    public void cancelarFinalizarTareaSCPFActionListener(ActionEvent actionEvent) {
        // Cerramos popup de confirmar finalizar tarea en todas las pantallas
        logger.log(ADFLogger.WARNING,"Inside cancelarFinalizarTareaSCPFActionListener: " + actionEvent.getComponent().getId());
        getPopupConfirmarFinalizarTareaSC().hide();
    }
    ...
    private Map validarCondicionesFormalizacion(Long idOperacion) {
        ...
        Long idContrato=null;
        // Validamos Condiciones a formalizar y Plazo de formalizaciï¿½n
        ...
        if (!operationBinding.getErrors().isEmpty()) {
            ...
            if ((response.getResultado().getResult() != null) &&
                ...
                JSFUtils.addFacesErrorMessage("Error al validar Condiciones de Formalizaciï¿½n: " + mensajeError);
                logger.log(ADFLogger.ERROR, "Error al validar Condiciones de Formalizaciï¿½n: " + mensajeError);
            }
        } else {
    ...
    public RichOutputText getOtInitForm() {
        ...
        // Usado cuando queremos acceder a un componente que existe hasta que se desplegï¿½ en pantalla
        FormalizacionBean formalizacionBean = (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
        ...
        if (formalizacionBean.getIdTarea().equalsIgnoreCase(FenixConstants.PC05_DEFINIR_RECURSOS_EXTERNOS)) {
            ...
            if (JSFUtils.resolveExpression("#{bindings.FuentesExternasVO.estimatedRowCount}") != null) {
                ...
                if (fuentesExternasRowCount > 0)
                    formalizacionBean.setCuentaConRecExternos(1); // Sï¿½ cuenta con recursos externos
            }
        }
    ...
    private BigDecimal obtenerMontoAprobado() {
        ...
        operationBinding.getParamsMap().put("varTipoMonto",
                                            new Integer(3)); // Monto Aprobado Id = 3, segï¿½n tabla TCA_TIPO_MONTO_OPERACION
        operationBinding.execute();
        if (operationBinding.getErrors().isEmpty()) {
            // Asignaciï¿½n para Monto aprobado
    ...
    private Boolean montoTotalValido() {
        ...
        if (stMontoEscriturado == null || stMontoEscriturado == "") {
            ...
            if ((montoEscriturado == null) || (montoTotal == null) ||
                ...
                esDatosCorrectos = Boolean.FALSE;
                JSFUtils.addFacesErrorMessage("El monto total de las lï¿½neas registradas supera el monto escriturado.");
                //JSFUtils.addFacesErrorMessage("La suma total del monto de cada Lï¿½nea debe corresponder al monto escriturado.");
            }
    ...
    private String finalizarAsignarRecursos() {
        String returnAction = null;
        // Propagamos fuentes de las lï¿½neas de crï¿½dito
        if (propagarFuente()) {
            // Invocar mï¿½todo para cargar documentos en onBase
            cargarDocumentosOnBase();
            // Invocar aprovacion del humman task
    ...
    public void adjuntarContratoMasInformacionAction() {
        ...
        if (validateComentario(Long.valueOf(formalizacionBean.getIdOperacion()), formalizacionBean.getIdTarea(),
                               getInstanciaTarea())) {
            // Abre popup de confirmar mï¿½s informacion en todas las pantallas
            RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
            getPopupConfirmarMasInformacion().show(popupHints);
        } else {
            // Se notifica que se debe ingresar un comentario
    ...
    /**
     * Mï¿½todo para cancelar la acciï¿½n de mï¿½s informaciï¿½n de
    ...
    /**
     * Mï¿½todo para validar la TCC 101 - Fecha de vencimiento
     * @author Jonathan Ruiz
    ...
    /**
     * Mï¿½todo para validar si se adjunto el contrato firmado y se
    ...
    /**
     * Mï¿½todo para recuperar la fecha de vencimiento por binding
     * @author Jonathan Ruiz
     * @return true en caso de ser verdadero
     */
    @SuppressWarnings("unchecked")
     ...
     * ya que de acuerdo con QA, la fecha de creaciï¿½n de la operaciï¿½n es la que se debe cambiar a horas
     * en cero y asi ya no se modificarian las fechas que se muestran en la lï¿½nea del tiempo del gestor
     * de operciones.
     *
    ...
    public void finalizarTareaFormalizacion(ActionEvent actionEvent) {
        ...
        //Previo a finalizar, obtiene si las lineas fueron actualizadas para setear actualizoLineasCredito.
        FormalizacionBean formalizacionBean = (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
        ADFContext adfCtx = ADFContext.getCurrent();
        Map sessionScope = adfCtx.getSessionScope();
        Object val = sessionScope.get("CambiolineasCreditos");
        
        String cambioLineasCredito;
        
        if (val != null){
            cambioLineasCredito = val.toString();
            logger.log(ADFLogger.WARNING, "FinalizarTarea >>> CambiolineasCreditos Val: No Nulo : " + cambioLineasCredito);
        }else{
            cambioLineasCredito = "N";
            logger.log(ADFLogger.WARNING, "FinalizarTarea >>> CambiolineasCreditos Val: Nulo : " + cambioLineasCredito);
        }
        
        if (cambioLineasCredito == "N") {
            formalizacionBean.setActualizoLineaCredito(Boolean.FALSE);
            actualizarPayloadElement("actualizoLineaCredito", Boolean.FALSE);
            //ADFUtils.setBoundAttributeValue("actualizoLineaCredito", Boolean.FALSE);
        }
        else{
            formalizacionBean.setActualizoLineaCredito(Boolean.TRUE);
            actualizarPayloadElement("actualizoLineaCredito", Boolean.TRUE);
            //ADFUtils.setBoundAttributeValue("actualizoLineaCredito", Boolean.TRUE);
        }
        
        logger.log(ADFLogger.WARNING, "FinalizarTarea >>> actualizoLineaCredito: " + formalizacionBean.getActualizoLineaCredito());
        
        logger.warning("Termina finalizarTareaFormalizacion.");
    }

    /**
     * Mï¿½todo que realiza funciï¿½n de enviar tarea a COFI
     * @funtion: replica la funcionalidad de finalizar tarea
     * @restriccciones: Se ejecuta logica en el constructor para determianr mostrar o no el botï¿½n
     * @since 04/05/2021
     * @by Raul Garcia
     */
    ...
    /**
     * Mï¿½todo que realiza funciï¿½n de enviar tarea a Responsable de la operaciï¿½n
    ...
    * 
    ...
    public void validarDocumentosTareaFormalizacion(ActionEvent actionEvent, String p_accion) {
        ...
        switch (codigoTarea.toString()) {
            ...
            case FenixConstants.PC05_COMUNICAR_VIGENCIA_CONTRATO:
                ...
                break;
            case FenixConstants.PC05_VALIDAR_DERIVADOR_IMPLICITOS:
                logger.warning("Tarea: PC05_VALIDAR_DERIVADOR_IMPLICITOS");
                if(accion=="finalizarTarea"){ 
                      this.confirmarFinalizarValidarDerivadosImplicitos(actionEvent);
                } 
                logger.warning("Finaliza Tarea: PC05_VALIDAR_DERIVADOR_IMPLICITOS");
            break; 
            }
        } else {
    ...
    /*** START Pantalla Registrar datos de la línea de crédito ***/
    public String aceptarValidarDerivadosImplicitos() {
        logger.log(ADFLogger.TRACE, "Inside aceptarValidarDerivadosImplicitos.");
        InvokeActionBean invokeActionBean = null;
        String returnAction = null;
        FormalizacionBean formalizacionBean = (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
        // Validación de datos en header
  
        if(validarDerivadosImplicitos() ){
            // Invocar método para cargar documentos en onBase
            invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue("invokeActionBean");
            returnAction = invokeActionBean.invokeOperation();
        }
    ...
    public String aceptarValidarDerivadosImplicitosComentarios() {
        logger.log(ADFLogger.TRACE, "Inside aceptarValidarDerivadosImplicitos.");
        InvokeActionBean invokeActionBean = null;
        String returnAction = null;
        FormalizacionBean formalizacionBean = (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
        ...
        if(validarDerivadosImplicitos()  && validateComentario(Long.valueOf(formalizacionBean.getIdOperacion()),formalizacionBean.getIdTarea(), getInstanciaTarea())){
            // Invocar método para cargar documentos en onBase
            invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue("invokeActionBean");
            returnAction = invokeActionBean.invokeOperation();
        }else {
    ...
    public Boolean validarDerivadosImplicitos(){
        Boolean res= Boolean.TRUE;
        JSFUtils.setExpressionValue("#{bindings.requiereValidarFormulario.inputValue}", Boolean.FALSE);
        return res;
    }
    public void confirmarFinalizarValidarDerivadosImplicitos(ActionEvent actionEvent) {
        // Abre popup de confirmar finalizar tarea en todas las pantallas
        logger.warning("Inside confirmarFinalizarValidarDerivadosImplicitos");
        RichPopup.PopupHints popupHints = new RichPopup.PopupHints();
        getPopupConfirmarFinalizarTarea().show(popupHints);
        logger.warning("Leave confirmarFinalizarValidarDerivadosImplicitos");
    }
  /*
   * jenamorado
   * 20/07/2021
   ...
   * */
    ...
    public boolean ValidarFechasFinalizarTareaAdjuntarContratoFirmaAction() {
        logger.warning("ValidarFechasFinalizarTareaAdjuntarContratoFirmaAction ");
        FormalizacionBean formalizacionBean =
            (FormalizacionBean) JSFUtils.resolveExpression("#{pageFlowScope.FormalizacionManagedBean}");
        DCIteratorBinding voAdjuntarContratoFirmado = null;
        Integer codigoTarea = Integer.parseInt(formalizacionBean.getIdTarea());
        logger.warning("Tarea: "+codigoTarea);
        
        voAdjuntarContratoFirmado =
            ADFUtils.getDCBindingContainer().findIteratorBinding("AdjuntarContratoFirmadoVOIterator");
        Date fechaFirma;
        Date fechaVencimiento= (Date) obtenerFechaVencimientoBinding() ;
        logger.warning("Fecha Vencimiento: "+fechaVencimiento);
        fechaFirma =  (Date) voAdjuntarContratoFirmado.getRowAtRangeIndex(0).getAttribute("FechaFirma");
        logger.warning("Fecha FechaFirma: "+fechaFirma);
        Boolean Resultado =Boolean.TRUE;
 ...
 * */
    public Long ObtenerContratoporIntancia(){
        ...
        logger.log(ADFLogger.WARNING, "idOperacion: " + formalizacionBean.getIdOperacion());
        
        operationBinding.getParamsMap().put("instanciaProceso", formalizacionBean.getInstanciaProceso());
        
        logger.log(ADFLogger.WARNING, "instanciaProceso : " + formalizacionBean.getInstanciaProceso());
