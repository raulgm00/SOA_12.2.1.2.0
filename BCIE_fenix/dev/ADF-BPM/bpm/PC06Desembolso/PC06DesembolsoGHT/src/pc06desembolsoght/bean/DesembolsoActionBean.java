...
public class DesembolsoActionBean extends FenixActionBean {
    ...
    private static final String CLASS_NAME = DesembolsoActionBean.class.getName();
    ...
    private RichPopup popupRetornarResponsable;
    ...
    private RichPopup popupCancelValidacionRaroc;
    ...
    /**
     * Se agrega una tarea generia para no depender de los IDS de las tareas,
     * ya que cambian entre los diferentes ambientes.
     * @Date: 2020/12/09
     * @Author: Kruger
     * @return action
     */
    public String invocarFinalizarTareaValidarRAROC() {
        final String METHOD_NAME = "invocarFinalizarTareaValidarRAROC";
        ...
        if (validacionesCA()) {
            ...
            existeDocumento = validarDocumentosTarea(idOpearcion, codigoTarea, null);
            if(existeDocumento){
                isValidFinalizarTarea = Boolean.TRUE;
            }else{
                budleKeyErroList.add("MSG_ERROR_DOCUMENTO_VALIDAR_RAROC");;
            }
    ...
    public String invocarFinalizarTarea() {
        if (validacionesCA()) {
            ...
            case FenixConstants.CGD_RESERVAR_FONDOS:
                LOGGER.warning("Case CGD_RESERVAR_FONDOS - invocarFinalizarTarea");
    ...
    public Boolean validarReglasPorTarea(){
        ...
        LOGGER.warning("A?adir datos a mapa de parametros de entrada.");
                    ...
                    if(null != idRegla){
                        ...
                        switch (codigoTarea) {
                            case FenixConstants.CGD_APROBAR_DESEMBOLSO:
                                ...
                                /**
                                 * @Developer: RUGM, DSALGADO
                                 * @Date: 14/06/2021
                                 * @US: 2012
                                 * */
                                mensajeErrorValidacion = obtenerEstadoReglasValidadas(estado, idRegla, mapaReglasValidadas);
                                break;
                            case FenixConstants.CGD_GESTIONAR_PROGRAMACION:
                                LOGGER.warning("Ingresa al case para obtener estado reglas validadas Aprobar Desembolso"); 
                                mensajeErrorValidacion = obtenerEstadoReglasValidadasGestionarProgramacion(estado, idRegla, mapaReglasValidadas);
                                break;
                            case FenixConstants.CGD_RESERVAR_FONDOS:
                                LOGGER.warning("Case CGD_RESERVAR_FONDOS - validarReglasPorTarea");
                                LOGGER.warning("Ingresa al case para obtener estado reglas validadas Aprobar Desembolso"); 
                                mensajeErrorValidacion = obtenerEstadoReglasValidadasReservarFondos(estado, idRegla, mapaReglasValidadas);
                                break;
                            case FenixConstants.CGD_REALIZAR_AJUSTES_A_DESEMBOLSO:
                                LOGGER.warning("Ingresa al case para obtener estado reglas validadas Realizar ajustes a desembolso"); 
                                mensajeErrorValidacion = obtenerEstadoReglasValidadasRealizarAjustesDesembolso(estado, idRegla, mapaReglasValidadas);
                                try{
                                    if(idRegla.compareTo(FenixConstants.ID_REGLA_TRANSACCION_PROGRAMACION)==0){
                                        if(null != mensajeErrorValidacion){
                                            LOGGER.warning("Es error de PROGRAMACION, se invocar? POPUP.");
                        ...
                        if(null != mensajeErrorValidacion){
                            LOGGER.warning("A?adiendo listado de mensajes.");
        ...
        switch(codigoTarea){
            ...
            break;
            case FenixConstants.CGD_RESERVAR_FONDOS:
                LOGGER.warning("Case CGD_RESERVAR_FONDOS - validarReglasPorTarea 2");
            ...
            //Evaluando RN_12: Para poder finalizar la tarea el monto de la asignaci?n de recursos
        ...
        if(listaErrores.size() > 0){
            ...
            LOGGER.warning("La lista de errores es vac?a.");
            if(esErrorProgramacion){
                desembolsoBean.setListaErrores(listaErrores);
                desembolsoBean.setEsErrorProgramacion(Boolean.FALSE);
                mostrarPopupProgramacion();
    ...
    public String validarAprobador(){
        ...
        try{
            ...
            LOGGER.warning("Usuario que cre? el contrato: " + creadorContrato);
        }catch(Exception e){
     ...
     * @US: 2012
     * */
    public String obtenerEstadoReglasValidadas(String descripcionEstadoRegla, Long idReglaPorValidar, Map mapaReglasValidadas){
        ...
        if(null != descripcionEstadoRegla){
            ...
            else if(descripcionEstadoRegla.equalsIgnoreCase(getStringFromBundle("ESTADO_VALIDACION_REGLA_ERROR_FLEXCUBE")))
            {
                ...
                if(FenixConstants.ID_REGLA_VALIDACION_TEMPRANA_FLEXCUBE.compareTo(idReglaPorValidar)==0){
                    if(descripcionEstadoRegla.equalsIgnoreCase("NO_CUMPLIDA_ADVERTENCIA"))
                    {
                        LOGGER.warning("mensajeErrorRegla24 : " + (String) mapaReglasValidadas.get("mensajeErrorRegla24"));
                        listaAdvertencias.add((String) mapaReglasValidadas.get("mensajeErrorRegla24"));
                    }
                    else
                    {
                        LOGGER.warning("mensajeErrorRegla24 : " + (String) mapaReglasValidadas.get("mensajeErrorRegla24"));
                        mensajeError = (String) mapaReglasValidadas.get("mensajeErrorRegla24");
                    }
                }
                /**
                 * @Developer: RUGM, DSALGADO
    ...
    public String obtenerEstadoReglasValidadasRealizarAjustesDesembolso(String descripcionEstadoRegla, Long idReglaPorValidar, Map mapaReglasValidadas){
    ...
    public String obtenerEstadoReglasValidadasGestionarProgramacion(String descripcionEstadoRegla, Long idReglaPorValidar, Map mapaReglasValidadas){
    ...
    public String obtenerEstadoReglasValidadasReservarFondos(String descripcionEstadoRegla, Long idReglaPorValidar, Map mapaReglasValidadas){
    ...
    /**
     * Finaliza la tarea de validacion RAROC
     * @return
     */
    public String finalizarTareaValidarRAROC() {
        final String METHOD_NAME = "finalizarTareaValidarRAROC";
        LOGGER.entering(CLASS_NAME, METHOD_NAME);
    ...
    public String finalizarTarea() {
        ...
        switch (codigoTarea) {
        case FenixConstants.PC06_GESTIONAR_DESEMBOLSO_FUENTES_EXTERNAS:
        //Cambio para finalizar una tarea de Gestionar desembolsos con fuentes externas sin la linea pasiva y sin validar los campos ticket #7138 //
           ...
           if (null != voFuentesExtContratoDesembolso) {
                LOGGER.warning("Registros de Fuentes Externas con Transferencias de recursos: " +
                     voFuentesExtContratoDesembolso.getEstimatedRowCount());           
                   if (voFuentesExtContratoDesembolso.getEstimatedRowCount()>0)
                                   { 
        ...
        case FenixConstants.CGD_GESTIONAR_PROGRAMACION:
            ...
            break;
            case FenixConstants.CGD_RESERVAR_FONDOS:
                LOGGER.warning("Case CGD_RESERVAR_FONDOS - finalizarTarea");
                final String estado=JSFUtils.resolveExpression("#{bindings.IdTcaEstado.inputValue}").toString();
                LOGGER.warning("Ingresa al switch reserva de fondos - estado actual: "+estado+" - "+desembolsoBean.getIdOperacion());
                if(estado.equals(Integer.toString(FenixConstants.ESTADO_SOLICITUD_APROBADO_POR_GERENTE)) && !estado.equals(Integer.toString(FenixConstants.ESTADO_SOLICITUD_REGISTRADO))){
                    LOGGER.warning("Ingreso al if cambiar condicion - "+desembolsoBean.getIdOperacion());
    ...
    public String invocarMasInformacion() { 
        ...
        switch (codigoTarea)
        {
            ...
            break;
            case FenixConstants.CGD_RESERVAR_FONDOS:
               LOGGER.warning("Aplica validacion para tarea CGD_RESERVAR_FONDOS - invocarMasInformacion");
                esComentario =
                    validateComentario(idOpearcion,codigoTarea.toString() , getInstanciaTarea());                          
                if(esComentario){
                    isValidSolicitarMasInformacion = Boolean.TRUE;
      ...
      * @return action
      */
    public String invocarMasInformacionValidarRAROC() { 
        final String METHOD_NAME = "invocarMasInformacionValidarRAROC";
        LOGGER.entering(CLASS_NAME, METHOD_NAME);
    ...
    public void solicitarRetornarResponsablePopup()
    {
        RichPopup.PopupHints hints = new RichPopup.PopupHints();
        popupRetornarResponsable.show(hints);
    }
    ...
    */
    ...
    public String solicitarMasInformacionGestionarProgramacion() {
        ...
        if(!resultado){
            LOGGER.warning("No fu? posible actualizar el estado del contrato de desembolso a En Transito.");
    ...
    /**
     * Cancelar Retornar Responsable
     * @return
     */
    public String cancelarRetornarResponsable() {
        popupRetornarResponsable.hide();
        return null;
    }
    ...
    public void setPopupRetornarResponsable(RichPopup popupRetornaResponsable)
    {
    this.popupRetornarResponsable = popupRetornaResponsable;
    }
    
    public RichPopup getPopupRetornarResponsable()
    {
    return popupRetornarResponsable;
    }
    ...
    public String invokeFinalizarTarea(){
            ...
            case FenixConstants.CGD_REGISTRAR_DESEMBOLSO:
                LOGGER.warning("Case CGD_REGISTRAR_DESEMBOLSO invokeFinalizarTarea");
    ...
    public String invokeMasInformacion() {
            ...
            switch (codigoTarea) {
                ...
                isValidMasInformacion=validateComentario(getIdOperacion(), getCodigoTarea().toString(), getInstanciaTarea());
                    ...
                    break;
                case FenixConstants.CGD_REGISTRAR_DESEMBOLSO:
                    LOGGER.warning("Case CGD_REGISTRAR_DESEMBOLSO invokeMasInformacion");
        ...
        popupConfFinalizarTarea.hide();
            ...
            switch (codigoTarea) {
                ...
                if(null!=condicion1){
                    ...
                    break;
                case FenixConstants.CGD_REGISTRAR_DESEMBOLSO:
                    LOGGER.warning("Case CGD_REGISTRAR_DESEMBOLSO aceptarFinalizarTarea");
                    final String estado=JSFUtils.resolveExpression("#{bindings.IdTcaEstado.inputValue}").toString();
                    LOGGER.warning("Ingresa al switch registrr desembolso - estado actual "+estado+" - "+desembolsoBean.getIdOperacion());
                    if(!estado.equals(Integer.toString(FenixConstants.ESTADO_SOLICITUD_REGISTRADO))){
                        LOGGER.warning("Ingreso al if cambiar condicion - "+desembolsoBean.getIdOperacion());
                        Boolean isEstadoActualizado =   actualizarEstadoContrato(idContratoDesembolso, FenixConstants.ESTADO_SOLICITUD_DESEMBOLSADO);
    ...
    public String aceptarMasInformacion(){
		...
		switch (codigoTarea) {
			...
			isValidMasInformacion = Boolean.TRUE;
				break;
			case FenixConstants.CGD_REGISTRAR_DESEMBOLSO:
				LOGGER.warning("Case CGD_REGISTRAR_DESEMBOLSO aceptarMasInformacion");
				final String estado=JSFUtils.resolveExpression("#{bindings.IdTcaEstado.inputValue}").toString();
				LOGGER.warning("Ingresa al switch registrr desembolso 2 - estado actual "+estado+" - "+desembolsoBean.getIdOperacion());
				if(!estado.equals(Integer.toString(FenixConstants.ESTADO_SOLICITUD_REGISTRADO))){	
					LOGGER.warning("Ingreso al if cambiar condicion - "+desembolsoBean.getIdOperacion());
    ...
    public String aceptarRetornarResponsable(){
        final String METHOD_NAME = "aceptarRetornarResponsable";
        LOGGER.entering(CLASS_NAME,METHOD_NAME);
        ...
        isValidMasInformacion=true;
        // 2021-01-22 Se remueve actualizar estado, la VO esta publicada en ambos Application modules
        //isValidMasInformacion=actualizarEstadoDeContrato(desembolsoBean.getContrato(), FenixConstants.ESTADO_SOLICITUD_EN_TRANSITO);
        
        if(isValidMasInformacion){
        
            actualizarPayloadElement("estadoDesembolso", FenixConstants.ESTADO_SOLICITUD_EN_TRANSITO);
            cargarDocumentosOnBase();
            invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue("invokeActionBean");
            return invokeActionBean.invokeOperation();// VAPT
        } else {
            bundleKeyErrorList.add("MSG_ERROR_ACTUALIZAR_ESTADO_CONTRATO"); 
        ...
        LOGGER.exiting(CLASS_NAME, METHOD_NAME);
        return null;//invokeActionBean.invokeOperation();        
    }
...
//                        JSFUtils.addFacesErrorMessage("Error al Consultar datos de la operacion. Por favor intente m?s tarde.");
//                }
    ...
    public Boolean aceptarAgregarEvidencia(ActionEvent actionEvent){
        ...
        if(bandera == false){ 
            ...
            // Invocamos m?todo que agrega las evidencias seleccionadas
    ...
    private void actualizarEvidenciasSolicitud(){
        ...
        // Invocamos m?todo que asigna la bind variable IdCondicion en EvidenciasCondicionesVO
    ...
    public Boolean eliminarAgregarEvidencia(){
        ...
        if(bandera == false){            
            ...
            // Invocamos m?todo que elimina la evidencia seleccionada
...
/*
  ...
  */    
    ...
    /**
     * M?todo de validaci?n de las excepciones del caso de uso para 
        ...
        if (null != voFuentesExtContratoDesembolso) {
            ...
            else 
            {
                if (this.validarCamposRequeridosTransferenciaRecursos() &&
                    this.validaGestorDesembolsoFuentesExternasEX02() && this.validarFormaTransferencia() &&
     ...
     * por el respnsable de la operaci?n.
     * EX03 del CU
     * @return valido Boolean
     */
                    ...
                    if(null != row){
                        ...
                        if(null != idFormaTransferencia){
                            ...
                            if (forma != null && origen != null) {
                                ...
                                LOGGER.warning("La Forma de transferencia de Recursos o el or?gen de la transferencia son NULL.");
                                JSFUtils.addFacesErrorMessage(getStringFromBundle("MSG_ERROR_VALIDACION_FORMA_TRANSFERENCIA"));
                                break;
                            }
                        }else{
        ...
        if (ADFUtils.getDCBindingContainer().findIteratorBinding("FuentesExternasContratoDesembolsoVOIterator") != null) {
            ...
            if (row != null) {
                ...
                if (row.getAttribute("TcaFormaTransferencia") != null) {
                    ...
                    LOGGER.warning("No se encontr? el TcaFormaTransferencia");
                }
            } else {
                LOGGER.warning("El Row es null");
            }
        } else {
            LOGGER.warning("No se encontr? el TransferenciaRecursosVOIterator");
        }
        return idForma;
    }
    ...
    private String getOrigenTransferenciaFuente() {
        ...
        if (row != null) {
            if (row.getAttribute("OrigenTranferenciaCliente") != null) {
                ...
                LOGGER.warning("No se encontr? el OrigenTranferenciaCliente");
            }
        } else {
            LOGGER.warning("El Row es null");
        }
       ...
       // if(aplicarCompensacionDesembolso()){   
              ...
              if(desembolsoBean != null){   
                 ...
                 actualizarEstado=true;
                    // 2021-01-22 Se remueve actualizar estado, la VO esta publicada en ambos Application modules
                     //actualizarEstadoContrato(desembolsoBean.getContrato(), FenixConstants.ESTADO_SOLICITUD_EN_TRANSITO);                        
    ...
    public String invocarRetornarResponsable() { 
    
            LOGGER.warning("Inicia proceso de Invocar Retornar Responsable");
    
            List<String> budleKeyErroList = new ArrayList<String>();
          
            Boolean isValidRetornarResponsable = Boolean.FALSE;
            ...
            switch (codigoTarea)
            {
                ...
                case FenixConstants.CGD_REGISTRAR_DESEMBOLSO:
                    ...
                    if(esDesembolsoRegistradoFlex() == null ){
                        ...
                        isValidRetornarResponsable = Boolean.FALSE;
                        JSFUtils.addFacesInformationMessage("No se pueden requerir ajustes de un desembolso ya registrado");                     
                    }else{
                        esComentario =
                            validateComentario(idOpearcion,codigoTarea.toString() , getInstanciaTarea());
                        if(esComentario){
                            isValidRetornarResponsable = Boolean.TRUE;
                        }else{
                            budleKeyErroList.add("MSG_VALIDACION_COMENTARIO");
                        }
                    }
                ...
                default:
                    LOGGER.log(ADFLogger.ERROR, "invocarRetornarResponsable(): Codigo de tarea no valido - " + codigoTarea);
                break;
            }
            
            if (!isValidRetornarResponsable)
            {
                if(budleKeyErroList.size()>0)
                {
                  for(String bundleKey : budleKeyErroList)
            ...
            {
                solicitarRetornarResponsablePopup();
            }
    ...
    public Boolean actualizarEstadoDeContrat(String contrato, Integer estado){
        Boolean resultado=Boolean.TRUE;
        BindingContainer bindings = ADFUtils.getBindingContainer();
        OperationBinding operationBinding = null;
        try{
                // Invocamos metodo y asignamos las variables correspondientes
                operationBinding = bindings.getOperationBinding("actualizarEstadoContrato");
    ...
    /**
     * Valida que se indic? que si se cuenta con los recursos para cubrir el monto del contrato de desembolso.
     */
        ...
        if(null != listaErroresModelo && listaErroresModelo.size() > 0){
            ...
            for(Object mensaje : listaErroresModelo){
                ...
                if(!(mensajeError.equalsIgnoreCase("T102")) && !(mensajeError.equalsIgnoreCase("T103"))){
                    LOGGER.warning("No es mensaje de Validacion. Ocurri? un error.");
                    listaErrores.add(mensajeError);
                }else{
    ...
    public Boolean validarLimitesTasaMonto(){
        ...
        if(null != montoMinimoDesembolso ){
            if(null != montoEnDolares){
                ...
                JSFUtils.addFacesErrorMessage("ERROR no se puede realizar la validaci?n de l?mites de montos de la l?nea de cr?dito." +
                    "Alg?n valor es NULL.");
            }
        }else{
                                ...
                                if(null != formaTransferencia){
                                    if(formaTransferencia.compareTo(1)==0){
                                        if(null == row.getAttribute("NumeroCuentaClientePasivo")){
                                            ...
                                            //CAMBIO REALIZADO EL 19/10/2021 POR JFLORES Ya no sera obligatorio llenar este campo, ver ticket #7138 //
                                            //resultado = Boolean.FALSE; //
                                            // esCampoNull = Boolean.TRUE; //                                         
                                        }
                                    }
    ...
    /**
     * M?todo empleado para la validacion de los componentes de Clasificacion Ambiental al finalizar la tarea.
    ...
    /**
     * M?todo auxiliar empleado para validar si la seccion de Clasificacion Ambiental es editable o no.
     * @author : S&P Solutions
     * @param  : 
     * @return : boolean
     * @version: v1.0
    ...
    /**
     * M?todo auxiliar empleado para obtener el # de elementos en la seccion de Clasificacion Ambiental.
     * @author : S&P Solutions
     * @param  : 
     * @return : int
     * @version: v1.0
    ...
    /**
     * M?todo que busca y retorna un componente dado su id.
     * @author : S&P Solutions
     * @param  : String
     * @return : UIComponent
     * @version: v1.0
    ...
    /**
     * M?todo que busca y retorna un componente dado un componente base y el id del componente a encontrar.
     * @author :
     * @param  : UIComponent
     * @param  : String
     * @return : UIComponent
    ...
    /**
     * M?todo que obtiene los valores de cada componente de los formularios din?micos.
     * @author : S&P Solutions
     * @param  : int index
     * @version: v1.0
     * @Fecha  : 24/09/2019
    ...
    public Boolean obtenerValoresComponentes(int index) {
       ...
       // logger.warning("\n=======  obtenerValoresComponentes index:" + index);
        ...
        if (categoria.equals("-1")) {
            esDatosCorrectos = Boolean.FALSE;
          // JSFUtils.addFacesErrorMessage("El campo Categor?a es requerido.");
          erroresCA.add("El campo Categor?a es requerido.");
        } 
        //SubCategoria
        String subcategoria = getValorTxtSubCategoria(obtenerComponente(ID_PANELCMP), index);
        if (subcategoria.equals("-1")) {
            esDatosCorrectos = Boolean.FALSE;
         //   JSFUtils.addFacesErrorMessage("El campo Subcategor?a es requerido.");
         erroresCA.add("El campo Subcategor?a es requerido.");
        } 
    ...
    /**
     * M?todo que obtiene el valor del componente Sector Ambiental.
     * @author : S&P Solutions
    ...
    public String invocarCancelarValidarRAROC() {
        final String METHOD_NAME = "invocarCancelarValidarRAROC";
        try{
            
            LOGGER.entering(CLASS_NAME, METHOD_NAME);
            
            final Integer codigoTarea = getCodigoTarea();
            final Long idOpearcion = getIdOperacion();
            Boolean validacion = Boolean.TRUE;
            
            validacion =validateComentario(idOpearcion,codigoTarea.toString() , getInstanciaTarea());
            if(!validacion){
                JSFUtils.addFacesErrorMessage(ADFUtils.getStringFromBundle("MSG_VALIDACION_COMENTARIO", BUNDLE));
            }
            if(validacion){
                final RichPopup.PopupHints hints = new RichPopup.PopupHints();
                this.popupCancelValidacionRaroc.show(hints);
            }
        }catch(Exception e){
    ...
    public String cancelarValidarRAROC() {
        final String METHOD_NAME = "cancelarValidarRAROC";
        try{
            LOGGER.entering(CLASS_NAME, METHOD_NAME);
            popupConfMasInfo.hide();
            InvokeActionBean invokeActionBean = (InvokeActionBean) JSFUtils.getManagedBeanValue("invokeActionBean");
    ...
    public String cancelarPopUpValidarRAROC(){
        try{
            this.popupCancelValidacionRaroc.hide();
        }catch(Exception e){
            JSFUtils.addFacesErrorMessage(e.getMessage());
            LOGGER.severe(e);
        }
    ...
    public void setPopupCancelValidacionRaroc(RichPopup popupCancelValidacionRaroc) {
        this.popupCancelValidacionRaroc = popupCancelValidacionRaroc;
    }

    public RichPopup getPopupCancelValidacionRaroc() {
        return popupCancelValidacionRaroc;
    }
