...
declare function local:func($ErrorResponse as element() (:: schema-element(ns1:responseMapeoErrorMessage) ::),
                                                          $message as xs:string) as element() (:: schema-element(ns2:ConsultarAgrupadorByCondicionResponse) ::) {
    <ns2:ConsultarAgrupadorByCondicionResponse>
        <ns2:Resultado>
            <res:result>ERROR</res:result>
            <res:message>{fn:data($message)}</res:message>
            <res:error>
                <err:errorCode>{fn:data($ErrorResponse/ns1:Result/res:error/err:errorCode)}</err:errorCode>
                <err:errorDescription>{fn:data($ErrorResponse/ns1:Result/res:error/err:errorDescription)}</err:errorDescription>
                <err:errorType>{fn:data($ErrorResponse/ns1:Result/res:error/err:errorType)}</err:errorType>
            </res:error>
        </ns2:Resultado>
    </ns2:ConsultarAgrupadorByCondicionResponse>
};
